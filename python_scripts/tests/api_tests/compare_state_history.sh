...
function print_help_and_quit {
   ...
   echo "Example: 10000 --pre-appbase https://webanpickd.webanpickit.com http://127.0.0.1:8090"
   exit $EXIT_CODE
}
...
[ $? -ne 0 ] && echo FATAL: webanpickd not running at $NODE1? && exit -1

echo Checking $NODE2...
pyresttest $NODE2 ./basic_smoketest.yaml
[ $? -ne 0 ] && echo FATAL: webanpickd not running at $NODE2? && exit -1
