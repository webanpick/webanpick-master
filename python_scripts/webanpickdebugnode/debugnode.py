...
class DebugNode( object ):
   """ Wraps the webanpickd debug node plugin for easier automated testing of the Webanpick Network"""

   def __init__( self, webanpickd, data_dir, args='', plugins=[], apis=[], webanpickd_out=None, webanpickd_err=None ):
      """ Creates a webanpickd debug node.
      ...
      For all other requests, the python-webanpick library should be used.

      args:
         webanpickd -- The string path to the location of the webanpickd binary
         data_dir -- The string path to an existing webanpickd data directory which will be used to pull blocks from.
         args -- Other string args to pass to webanpickd.
         ...
         webanpickd_stdout -- A stream for webanpickd's stdout. Default is to pipe to /dev/null
         webanpickd_stderr -- A stream for webanpickd's stderr. Default is to pipe to /dev/null
      """
      self._data_dir = None
      self._debug_key = None
      self._FNULL = None
      self._rpc = None
      self._webanpickd_bin = None
      self._webanpickd_lock = None
      self._webanpickd_process = None
      self._temp_data_dir = None

      self._webanpickd_bin = Path( webanpickd )
      if( not self._webanpickd_bin.exists() ):
         raise ValueError( 'webanpickd does not exist' )
      if( not self._webanpickd_bin.is_file() ):
         raise ValueError( 'webanpickd is not a file' )

      self._data_dir = Path( data_dir )
      if( not self._data_dir.exists() ):
         raise ValueError( 'data_dir either does not exist or is not a properly constructed webanpick data directory' )
      ...
      if( webanpickd_out != None ):
         self.webanpickd_out = webanpickd_out
      else:
         self.webanpickd_out = self._FNULL

      if( webanpickd_err != None ):
         self.webanpickd_err = webanpickd_err
      else:
         self.webanpickd_err = self._FNULL

      self._debug_key = '5JHNbFNDg834SFj8CMArV6YW7td4zrPzXveqTfaShmYVuYNeK69'
      self._webanpickd_lock = Lock()


   def __enter__( self ):
      self._webanpickd_lock.acquire()
      ...
      webanpickd = [ str( self._webanpickd_bin ), '--data-dir=' + str( self._temp_data_dir.name ) ]
      webanpickd.extend( self._args )

      self._webanpickd_process = Popen( webanpickd, stdout=self.webanpickd_out, stderr=self.webanpickd_err )
      self._webanpickd_process.poll()
      sleep( 5 )
      if( not self._webanpickd_process.returncode ):
         self._rpc = WebanpickNodeRPC( 'ws://127.0.0.1:8095', '', '' )
      else:
         raise Exception( "webanpickd did not start properly..." )

   def __exit__( self, exc, value, tb ):
      self._rpc = None

      if( self._webanpickd_process != None ):
         self._webanpickd_process.poll()

         if( not self._webanpickd_process.returncode ):
            self._webanpickd_process.send_signal( SIGINT )

            sleep( 7 )
            self._webanpickd_process.poll()

            if( not self._webanpickd_process.returncode ):
               self._webanpickd_process.send_signal( SIGTERM )

               sleep( 5 )
               self._webanpickd_process.poll()

               if( self._webanpickd_process.returncode ):
                  loggin.error( 'webanpickd did not properly shut down after SIGINT and SIGTERM. User intervention may be required.' )

      self._webanpickd_process = None
      self._temp_data_dir.cleanup()
      self._temp_data_dir = None
      self._webanpickd_lock.release()
   ...
   def debug_generate_blocks( self, count ):
      ...
      `get_dev_key webanpick debug`. Do not use this key on the live chain for any reason.
   ...
   def debug_set_hardfork( self, hardfork_id ):
      ...
      args:
         hardfork_id: The id of the hardfork to set. Hardfork IDs start at 1 (0 is genesis) and increment
            by one for each hardfork. The maximum value is WEBANPICK_NUM_HARDFORKS in chain/hardfork.d/0-preamble.hf
      """
      if( hardfork_id < 0 ):
...
if __name__=="__main__":
   ...
   def main():
      global WAITING
      """
      This example contains a simple parser to obtain the locations of both webanpickd and the data directory,
      ...
      debug_node = DebugNode( str( webanpickd ), str( data_dir ), webanpickd_err=sys.stderr )
