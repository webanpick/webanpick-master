...
This is developer documentation for creating brand-new operations on the WEBANPICK blockchain.
...
- (4) Add operation to `webanpick::protocol::operation`
...
- (4a) The file `operations.hpp` defines the `webanpick::protocol::operation`
...
- (5a) You must add `WEBANPICK_DEFINE_EVALUATOR` macro in `evaluator.hpp` to
...
[implementation](https://github.com/webanpickit/webanpick/blob/a0a69b10189d93a9be4da7e7fd9d5358af956e34/libraries/chain/database.cpp#L364)
of `get_account(name)`.
...
- (9a) Add `smt_token_object_type` to `enum object_type` in `webanpick_objects.hpp` and add
to `FC_REFLECT_ENUM` bubble list at the bottom of that file
- (9b) Declare (but do not define) `class smt_token_object;` in `webanpick_objects.hpp`
- (9c) Define `typedef oid< smt_token_object > smt_token_id_type` in `webanpick_objects.hpp`
...
- (9e) All SMT objects are consensus, and therefore should exist in `webanpick::chain` namespace
...
defined by an `enum` in `webanpick_object_types.hpp`, any new objects must be added here.  In SQL
...
alias `typedef oid< smt_token_object > smt_id_type` is added to `webanpick_object_types.hpp`.
...
is designed as a reusable library, not tightly coupled to Webanpick, it contains no reference
to any `webanpick` namespaces.  So the name `by_id` must refer to `chainbase::by_id`.  If you
define a `struct by_id;` in the `webanpick::chain` namespace, the result will be that every
...
- (9m) All indexes used in Webanpick must be `ordered_unique`.  In theory, hashed or non-unique
