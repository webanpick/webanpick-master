...
of a running Webanpick Debug Node. The Debug Node is a plugin for Webanpick that allows realtime
...
[Python Webanpick library](https://github.com/xeroc/python-webanpicklib).
...
To use the script include `from webanpickdebugnode import DebugNode`

There are a couple of examples already made that you can try modifying yourself.

[debug_hardforks.py](https://github.com/webanpickit/webanpick/python_scripts/tests/debug_hardforks.py)
...
[debugnode.py](https://github.com/webanpickit/webanpick/python_scripts/webanpickdebugnode/debugnode.py#L212)
...
specify the webanpickd binary location and a populated data directory. For most configurations
this will be `programs/webanpickd/webanpickd` and `witness_node_data_dir` respectively, from the
git root directory for Webanpick.
