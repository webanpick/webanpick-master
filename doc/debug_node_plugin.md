...
Anyone (even you!) can edit their account to contain 1 million WEBANPICK.  It's really
...
yourself 1 million WEBANPICK.  Just like it's really impossible for your bank to physically stop
...
the fake WEBANPICK or fake dollars you "gave" yourself).  So you can believe whatever you want
about your balance and rewrite the rules of your own bookkeeping system to show you whatever balance you
want to be shown, but as soon as you try to actually spend WEBANPICK (or dollars) that you don't actually have,
...
(see `libraries/plugins/debug_node/include/webanpick/plugins/debug_node/debug_node_api.hpp`
...
Okay, let's run `webanpickd`.  It should start immediately with no blocks.  We can ask it to read blocks from the directory we saved earlier:
...
If we want to take control of an account we can do so by editing its key with `debug_update_object` command like this:
    ...
    curl --data '{"jsonrpc": "2.0", "method": "call", "params": [0,"lookup_account_names",[["webanpickit"]]], "id": 8}' http://127.0.0.1:8090/rpc    # find out ID of account we want is 2.2.28
...
Now that we've reset its key, we can take control of it in the wallet:
    ...
    list_my_accounts
    transfer webanpickit dantheman "1.234 WEBANPICK" "make -j100 money" true
    list_my_accounts
    get_account_history webanpickit -1 1000
