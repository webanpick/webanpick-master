...
All plugins in the `libraries/plugins` directory are iterated over by `CMakeLists.txt`. The manifest directory iterates through all plugins, adding them to the `webanpick_plugins` build target. Any other build target wanting to access all plugins
...
- By default, webanpickd runs the `chain`, `p2p`, and `webserver` plugins.
