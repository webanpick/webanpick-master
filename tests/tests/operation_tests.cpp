...
BOOST_AUTO_TEST_CASE( account_create_apply )
{
   try
   {
      ...
      const account_object& init = db->get_account( WEBANPICK_INIT_MINER_NAME );
      ...
      op.fee = asset( 100, WEBANPICK_SYMBOL );
      op.new_account_name = "alice";
      op.creator = WEBANPICK_INIT_MINER_NAME;
      ...
      auto vests = gpo.total_vesting_fund_webanpick;
      ...
      /// because init_witness has created vesting shares and blocks have been produced, 100 WEBANPICK is worth less than 100 vesting shares due to rounding
      ...
      tx.signatures.clear();
      tx.operations.clear();
      op.fee = asset( db->get_account( WEBANPICK_INIT_MINER_NAME ).balance.amount + 1, WEBANPICK_SYMBOL );
      op.new_account_name = "bob";
      tx.operations.push_back( op );
      tx.sign( init_account_priv_key, db->get_chain_id() );
...
BOOST_AUTO_TEST_CASE( account_update_authorities )
{
   try
   {
      ...
      WEBANPICK_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_missing_active_auth );
      ...
      tx.sign( alice_private_key, db->get_chain_id() );
      WEBANPICK_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_irrelevant_sig );

      BOOST_TEST_MESSAGE( "--- Test failure when containing duplicate signatures" );
      tx.signatures.clear();
      tx.sign( active_key, db->get_chain_id() );
      tx.sign( active_key, db->get_chain_id() );
      WEBANPICK_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_duplicate_sig );
      ...
      tx.operations.push_back( op );
      tx.sign( active_key, db->get_chain_id() );
      WEBANPICK_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_missing_owner_auth );
...
BOOST_AUTO_TEST_CASE( comment_authorities )
{
   try
   {
      ...
      ACTORS( (alice)(bob) );
      generate_blocks( 60 / WEBANPICK_BLOCK_INTERVAL );
      ...
      WEBANPICK_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_missing_posting_auth );
      ...
      tx.sign( bob_private_key, db->get_chain_id() );
      WEBANPICK_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_irrelevant_sig );

      BOOST_TEST_MESSAGE( "--- Test failure when signed by a signature not in the creator's authority" );
      tx.signatures.clear();
      tx.sign( bob_private_key, db->get_chain_id() );
      WEBANPICK_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_missing_posting_auth );
...
BOOST_AUTO_TEST_CASE( comment_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice_comment.cashout_time == fc::time_point_sec( db->head_block_time() + fc::seconds( WEBANPICK_CASHOUT_WINDOW_SECONDS ) ) );
      ...
      BOOST_REQUIRE( bob_comment.cashout_time == bob_comment.created + WEBANPICK_CASHOUT_WINDOW_SECONDS );
      ...
      BOOST_REQUIRE( sam_comment.cashout_time == sam_comment.created + WEBANPICK_CASHOUT_WINDOW_SECONDS );
      BOOST_REQUIRE( sam_comment.root_comment == alice_comment.id );
      validate_database();

      generate_blocks( 60 * 5 / WEBANPICK_BLOCK_INTERVAL + 1 );
      ...
      {
         o.total_reward_shares2 = webanpick::chain::util::evaluate_reward_curve( 10 );
      });
      ...
      BOOST_REQUIRE( mod_sam_comment.cashout_time == mod_sam_comment.created + WEBANPICK_CASHOUT_WINDOW_SECONDS );
      validate_database();
      ...
      generate_blocks( 60 * 5 / WEBANPICK_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( comment_delete_apply )
{
   try
   {
      ...
      tx.set_expiration( db->head_block_time() + WEBANPICK_MIN_TRANSACTION_EXPIRATION_LIMIT );
      tx.sign( alice_private_key, db->get_chain_id() );
      db->push_transaction( tx, 0 );
      ...
      tx.operations.push_back( op );
      tx.sign( alice_private_key, db->get_chain_id() );
      WEBANPICK_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::assert_exception );


      BOOST_TEST_MESSAGE( "--- Test success deleting a comment with negative rshares" );

      generate_block();
      vote.weight = -1 * WEBANPICK_100_PERCENT;
      tx.clear();
      tx.operations.push_back( vote );
      tx.operations.push_back( op );
      tx.sign( alice_private_key, db->get_chain_id() );
      ...
      generate_blocks( WEBANPICK_MIN_ROOT_COMMENT_INTERVAL.to_seconds() / WEBANPICK_BLOCK_INTERVAL );
      ...
      generate_blocks( WEBANPICK_CASHOUT_WINDOW_SECONDS / WEBANPICK_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( vote_apply )
{
   try
   {
...

      {
         ...
         op.author = "bob";
         op.permlink = "foo";
         op.weight = WEBANPICK_100_PERCENT;
         tx.operations.push_back( op );
         tx.sign( alice_private_key, db->get_chain_id() );
         ...
         int64_t max_vote_denom = ( db->get_dynamic_global_properties().vote_power_reserve_rate * WEBANPICK_VOTE_REGENERATION_SECONDS ) / (60*60*24);
         ...
         BOOST_REQUIRE( alice_comment.net_rshares.value == alice.vesting_shares.amount.value * ( old_voting_power - alice.voting_power ) / WEBANPICK_100_PERCENT - WEBANPICK_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( alice_comment.cashout_time == alice_comment.created + WEBANPICK_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr->rshares == alice.vesting_shares.amount.value * ( old_voting_power - alice.voting_power ) / WEBANPICK_100_PERCENT - WEBANPICK_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();

         BOOST_TEST_MESSAGE( "--- Test reduced power for quick voting" );

         generate_blocks( db->head_block_time() + WEBANPICK_MIN_VOTE_INTERVAL_SEC );
         ...
         op.weight = WEBANPICK_100_PERCENT / 2;
         op.voter = "alice";
         op.author = "bob";
         op.permlink = "foo";
         tx.operations.clear();
         ...
         BOOST_REQUIRE( db->get_account( "alice" ).voting_power == old_voting_power - ( ( old_voting_power + max_vote_denom - 1 ) * WEBANPICK_100_PERCENT / ( 2 * max_vote_denom * WEBANPICK_100_PERCENT ) ) );
         BOOST_REQUIRE( bob_comment.net_rshares.value == alice.vesting_shares.amount.value * ( old_voting_power - db->get_account( "alice" ).voting_power ) / WEBANPICK_100_PERCENT - WEBANPICK_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( bob_comment.cashout_time == bob_comment.created + WEBANPICK_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         generate_blocks( db->head_block_time() + fc::seconds( ( WEBANPICK_CASHOUT_WINDOW_SECONDS / 2 ) ), true );
         ...
         uint128_t new_cashout_time = db->head_block_time().sec_since_epoch() + WEBANPICK_CASHOUT_WINDOW_SECONDS;

         BOOST_REQUIRE( new_bob.voting_power == WEBANPICK_100_PERCENT - ( ( WEBANPICK_100_PERCENT + max_vote_denom - 1 ) / max_vote_denom ) );
         BOOST_REQUIRE( new_alice_comment.net_rshares.value == old_abs_rshares + new_bob.vesting_shares.amount.value * ( old_voting_power - new_bob.voting_power ) / WEBANPICK_100_PERCENT - WEBANPICK_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( new_alice_comment.cashout_time == new_alice_comment.created + WEBANPICK_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         op.weight = -1 * WEBANPICK_100_PERCENT / 2;
         op.voter = "sam";
         op.author = "bob";
         op.permlink = "foo";
         tx.operations.clear();
         ...
         new_cashout_time = db->head_block_time().sec_since_epoch() + WEBANPICK_CASHOUT_WINDOW_SECONDS;
         auto sam_weight /*= ( ( uint128_t( new_sam.vesting_shares.amount.value ) ) / 400 + 1 ).to_uint64();*/
                         = ( ( uint128_t( new_sam.vesting_shares.amount.value ) * ( ( WEBANPICK_100_PERCENT + max_vote_denom - 1 ) / ( 2 * max_vote_denom ) ) ) / WEBANPICK_100_PERCENT ).to_uint64() - WEBANPICK_VOTE_DUST_THRESHOLD;

         BOOST_REQUIRE( new_sam.voting_power == WEBANPICK_100_PERCENT - ( ( WEBANPICK_100_PERCENT + max_vote_denom - 1 ) / ( 2 * max_vote_denom ) ) );
         ...
         BOOST_REQUIRE( new_bob_comment.cashout_time == new_bob_comment.created + WEBANPICK_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         int64_t regenerated_power = (WEBANPICK_100_PERCENT * ( db->head_block_time() - db->get_account( "alice").last_vote_time ).to_seconds() ) / WEBANPICK_VOTE_REGENERATION_SECONDS;
         ...
         auto new_rshares = ( ( fc::uint128_t( db->get_account( "alice" ).vesting_shares.amount.value ) * used_power ) / WEBANPICK_100_PERCENT ).to_uint64() - WEBANPICK_VOTE_DUST_THRESHOLD;

         BOOST_REQUIRE( db->get_comment( "alice", string( "foo" ) ).cashout_time == db->get_comment( "alice", string( "foo" ) ).created + WEBANPICK_CASHOUT_WINDOW_SECONDS );
         ...
         old_abs_rshares = new_bob_comment.abs_rshares.value;
         used_power = ( ( WEBANPICK_1_PERCENT * 25 * ( new_alice.voting_power ) / WEBANPICK_100_PERCENT ) + max_vote_denom - 1 ) / max_vote_denom;
         auto alice_voting_power = new_alice.voting_power - used_power;

         op.voter = "alice";
         op.weight = WEBANPICK_1_PERCENT * 25;
         op.author = "bob";
         op.permlink = "foo";
         tx.operations.clear();
         tx.signatures.clear();
         ...
         new_rshares = ( ( fc::uint128_t( new_alice.vesting_shares.amount.value ) * used_power ) / WEBANPICK_100_PERCENT ).to_uint64() - WEBANPICK_VOTE_DUST_THRESHOLD;
         ...
         old_abs_rshares = new_bob_comment.abs_rshares.value;
         used_power = ( uint64_t( WEBANPICK_1_PERCENT ) * 75 * uint64_t( alice_voting_power ) ) / WEBANPICK_100_PERCENT;
         used_power = ( used_power + max_vote_denom - 1 ) / max_vote_denom;
         alice_voting_power -= used_power;

         op.weight = WEBANPICK_1_PERCENT * -75;
         tx.operations.clear();
         tx.signatures.clear();
         tx.operations.push_back( op );
         tx.sign( alice_private_key, db->get_chain_id() );
         ...
         generate_blocks( fc::time_point_sec( ( new_bob_comment.cashout_time - WEBANPICK_UPVOTE_LOCKOUT_HF17 ).sec_since_epoch() + WEBANPICK_BLOCK_INTERVAL ), true );
         ...
         op.weight = -1 * WEBANPICK_100_PERCENT;
         tx.operations.clear();
         tx.signatures.clear();
         tx.operations.push_back( op );
         tx.sign( alice_private_key, db->get_chain_id() );
...
BOOST_AUTO_TEST_CASE( transfer_apply )
{
   try
   {
      ...
      tx.set_expiration( db->head_block_time() + WEBANPICK_MAX_TIME_UNTIL_EXPIRATION );
      tx.sign( alice_private_key, db->get_chain_id() );
      WEBANPICK_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), fc::exception );
...
BOOST_AUTO_TEST_CASE( transfer_to_vesting_apply )
{
   try
   {
      ...
      auto vests = asset( gpo.total_vesting_fund_webanpick.amount, WEBANPICK_SYMBOL );
      ...
      BOOST_REQUIRE( gpo.total_vesting_fund_webanpick.amount.value == vests.amount.value );
      BOOST_REQUIRE( gpo.total_vesting_shares.amount.value == shares.amount.value );
      validate_database();

      op.to = "bob";
      op.amount = asset( 2000, WEBANPICK_SYMBOL );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      tx.set_expiration( db->head_block_time() + WEBANPICK_MAX_TIME_UNTIL_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( withdraw_vesting_authorities )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- Test failure when no signature." );
      WEBANPICK_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_missing_active_auth );
      ...
      tx.sign( alice_private_key, db->get_chain_id() );
      WEBANPICK_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_duplicate_sig );
...
BOOST_AUTO_TEST_CASE( withdraw_vesting_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice.vesting_withdraw_rate.amount.value == ( old_vesting_shares.amount / ( WEBANPICK_VESTING_WITHDRAW_INTERVALS * 2 ) ).value );
      BOOST_REQUIRE( alice.to_withdraw.value == op.vesting_shares.amount.value );
      BOOST_REQUIRE( alice.next_vesting_withdrawal == db->head_block_time() + WEBANPICK_VESTING_WITHDRAW_INTERVAL_SECONDS );
      validate_database();
      ...
      BOOST_REQUIRE( alice.vesting_shares.amount.value == old_vesting_shares.amount.value );
      BOOST_REQUIRE( alice.vesting_withdraw_rate.amount.value == ( old_vesting_shares.amount / ( WEBANPICK_VESTING_WITHDRAW_INTERVALS * 3 ) ).value );
      BOOST_REQUIRE( alice.to_withdraw.value == op.vesting_shares.amount.value );
      BOOST_REQUIRE( alice.next_vesting_withdrawal == db->head_block_time() + WEBANPICK_VESTING_WITHDRAW_INTERVAL_SECONDS );
      validate_database();
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         ...
         {
            gpo.current_supply += wso.median_props.account_creation_fee - ASSET( "0.001 TESTS" ) - gpo.total_vesting_fund_webanpick;
            gpo.total_vesting_fund_webanpick = wso.median_props.account_creation_fee - ASSET( "0.001 TESTS" );
         });
...
BOOST_AUTO_TEST_CASE( witness_update_apply )
{
   try
   {
      ...
      op.fee = ASSET( "1.000 TESTS" );
      op.block_signing_key = signing_key.get_public_key();
      op.props.account_creation_fee = legacy_webanpick_asset::from_asset( asset(WEBANPICK_MIN_ACCOUNT_CREATION_FEE + 10, WEBANPICK_SYMBOL) );
      op.props.maximum_block_size = WEBANPICK_MIN_BLOCK_SIZE_LIMIT + 100;
...
BOOST_AUTO_TEST_CASE( account_witness_proxy_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice.proxy == WEBANPICK_PROXY_TO_SELF_ACCOUNT );
      ...
      BOOST_REQUIRE( sam.proxy == WEBANPICK_PROXY_TO_SELF_ACCOUNT );
      ...
      BOOST_REQUIRE( sam.proxied_vsf_votes_total() == bob.vesting_shares.amount );
      BOOST_REQUIRE( dave.proxy == WEBANPICK_PROXY_TO_SELF_ACCOUNT );
      ...
      tx.operations.clear();
      tx.signatures.clear();
      op.proxy = WEBANPICK_PROXY_TO_SELF_ACCOUNT;
      op.account = "bob";
      tx.operations.push_back( op );
      tx.sign( bob_private_key, db->get_chain_id() );
      ...
      BOOST_REQUIRE( alice.proxy == "sam" );
      BOOST_REQUIRE( alice.proxied_vsf_votes_total().value == 0 );
      BOOST_REQUIRE( bob.proxy == WEBANPICK_PROXY_TO_SELF_ACCOUNT );
      BOOST_REQUIRE( bob.proxied_vsf_votes_total().value == 0 );
      BOOST_REQUIRE( sam.proxy == "dave" );
      ...
      vote.account= "bob";
      vote.witness = WEBANPICK_INIT_MINER_NAME;
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( vote );
      tx.sign( bob_private_key, db->get_chain_id() );
      ...
      BOOST_REQUIRE( db->get_witness( WEBANPICK_INIT_MINER_NAME ).votes == ( alice.vesting_shares + bob.vesting_shares ).amount );
      validate_database();
      ...
      BOOST_REQUIRE( db->get_witness( WEBANPICK_INIT_MINER_NAME ).votes == bob.vesting_shares.amount );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( feed_publish_apply )
{
   try
   {
      ...
      feed_publish_operation op;
      op.publisher = "alice";
      op.exchange_rate = price( ASSET( "1.000 TBD" ), ASSET( "1000.000 TESTS" ) ); // 1000 WEBANPICK : 1 SBD
...
BOOST_AUTO_TEST_CASE( convert_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( convert_request->conversion_date == db->head_block_time() + WEBANPICK_CONVERSION_DELAY );
...
BOOST_AUTO_TEST_CASE( fixture_convert_checks_balance )
{
   ...
   try
   {
      ...
      fund( "dany", 5000 );
      WEBANPICK_REQUIRE_THROW( convert( "dany", ASSET( "5000.000 TESTS" ) ), fc::exception );
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( limit_order_create_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( limit_order->get_market() == std::make_pair( SBD_SYMBOL, WEBANPICK_SYMBOL ) );
...
BOOST_AUTO_TEST_CASE( limit_order_create2_apply )
{
   try
   {
      ...
      /// First check validation on price constructor level:
      {
        price broken_price;
        /// Invalid base value
        WEBANPICK_REQUIRE_THROW(broken_price=price(ASSET("0.000 TESTS"), ASSET("1.000 TBD")),
          fc::exception);
        /// Invalid quote value
        WEBANPICK_REQUIRE_THROW(broken_price=price(ASSET("1.000 TESTS"), ASSET("0.000 TBD")),
          fc::exception);
        /// Invalid symbol (same in base & quote)
        WEBANPICK_REQUIRE_THROW(broken_price=price(ASSET("1.000 TESTS"), ASSET("0.000 TESTS")),
          fc::exception);
      }
...
BOOST_AUTO_TEST_CASE( account_recovery )
{
   try
   {
      ...
      generate_blocks( db->head_block_time() + WEBANPICK_OWNER_UPDATE_LIMIT );
      ...
      generate_blocks( db->head_block_time() + WEBANPICK_OWNER_UPDATE_LIMIT + fc::seconds( WEBANPICK_BLOCK_INTERVAL ) );
      ...
      BOOST_REQUIRE( req_itr->expires == db->head_block_time() + WEBANPICK_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD );
      auto expires = req_itr->expires;
      ++req_itr;
      BOOST_REQUIRE( req_itr == request_idx.end() );

      generate_blocks( time_point_sec( expires - WEBANPICK_BLOCK_INTERVAL ), true );
      ...
      generate_blocks( db->head_block_time() + ( WEBANPICK_OWNER_AUTH_RECOVERY_PERIOD - WEBANPICK_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD ) );
      generate_block();
...
BOOST_AUTO_TEST_CASE( change_recovery_account )
{
   try
   {
      ...
      WEBANPICK_REQUIRE_THROW( change_recovery_account("alice", "nobody"), fc::exception );
      WEBANPICK_REQUIRE_THROW( change_recovery_account("haxer", "sam"   ), fc::exception );
      WEBANPICK_REQUIRE_THROW( change_recovery_account("haxer", "nobody"), fc::exception );
      ...
      generate_blocks( db->head_block_time() + WEBANPICK_OWNER_AUTH_RECOVERY_PERIOD - fc::seconds( WEBANPICK_BLOCK_INTERVAL ), true );
      // cannot request account recovery until recovery account is approved
      WEBANPICK_REQUIRE_THROW( request_account_recovery( "sam", sam_private_key, "alice", alice_pub1 ), fc::exception );
      generate_blocks(1);
      // cannot finish account recovery until requested
      WEBANPICK_REQUIRE_THROW( recover_account( "alice", alice_priv1, alice_private_key ), fc::exception );
      // do the request
...
BOOST_AUTO_TEST_CASE( escrow_transfer_validate )
{
   try
   {
      ...
      op.webanpick_amount = ASSET( "1.000 TESTS" );
      op.escrow_id = 0;
      op.agent = "sam";
      ...
      op.sbd_amount.symbol = WEBANPICK_SYMBOL;
      WEBANPICK_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when webanpick symbol != WEBANPICK" );
      op.sbd_amount.symbol = SBD_SYMBOL;
      op.webanpick_amount.symbol = SBD_SYMBOL;
      WEBANPICK_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when fee symbol != SBD and fee symbol != WEBANPICK" );
      op.webanpick_amount.symbol = WEBANPICK_SYMBOL;
      op.fee.symbol = VESTS_SYMBOL;
      WEBANPICK_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when sbd == 0 and webanpick == 0" );
      op.fee.symbol = WEBANPICK_SYMBOL;
      op.sbd_amount.amount = 0;
      op.webanpick_amount.amount = 0;
      WEBANPICK_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when sbd < 0" );
      op.sbd_amount.amount = -100;
      op.webanpick_amount.amount = 1000;
      WEBANPICK_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when webanpick < 0" );
      op.sbd_amount.amount = 1000;
      op.webanpick_amount.amount = -100;
      WEBANPICK_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( escrow_transfer_apply )
{
   try
   {
      ...
      op.sbd_amount.amount = 0;
      op.webanpick_amount.amount = 10000;
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      tx.sign( alice_private_key, db->get_chain_id() );
      ...
      auto alice_webanpick_balance = alice.balance - op.webanpick_amount - op.fee;
      auto alice_sbd_balance = alice.sbd_balance - op.sbd_amount;
      auto bob_webanpick_balance = bob.balance;
      auto bob_sbd_balance = bob.sbd_balance;
      auto sam_webanpick_balance = sam.balance;
      ...
      BOOST_REQUIRE( escrow.webanpick_balance == op.webanpick_amount );
      ...
      BOOST_REQUIRE( alice.balance == alice_webanpick_balance );
      BOOST_REQUIRE( alice.sbd_balance == alice_sbd_balance );
      BOOST_REQUIRE( bob.balance == bob_webanpick_balance );
      BOOST_REQUIRE( bob.sbd_balance == bob_sbd_balance );
      BOOST_REQUIRE( sam.balance == sam_webanpick_balance );
...
BOOST_AUTO_TEST_CASE( escrow_approve_apply )
{
   try
   {
      ...
      et_op.to = "bob";
      et_op.agent = "sam";
      et_op.webanpick_amount = ASSET( "1.000 TESTS" );
      ...
      BOOST_REQUIRE( escrow.webanpick_balance == ASSET( "1.000 TESTS" ) );
      ...
      tx.set_expiration( db->head_block_time() + WEBANPICK_BLOCK_INTERVAL );
      tx.sign( bob_private_key, db->get_chain_id() );
      WEBANPICK_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
      ...
      WEBANPICK_REQUIRE_THROW( db->get_escrow( op.from, op.escrow_id ), fc::exception );
      BOOST_REQUIRE( alice.balance == ASSET( "10.000 TESTS" ) );
      validate_database();
      ...
      generate_blocks( et_op.ratification_deadline + WEBANPICK_BLOCK_INTERVAL, true );
...
BOOST_AUTO_TEST_CASE( escrow_dispute_apply )
{
   try
   {
      ...
      et_op.webanpick_amount = ASSET( "1.000 TESTS" );
      et_op.fee = ASSET( "0.100 TESTS" );
      et_op.ratification_deadline = db->head_block_time() + WEBANPICK_BLOCK_INTERVAL;
      et_op.escrow_expiration = db->head_block_time() + 2 * WEBANPICK_BLOCK_INTERVAL;
      ...
      BOOST_REQUIRE( escrow.webanpick_balance == et_op.webanpick_amount );
...
BOOST_AUTO_TEST_CASE( escrow_release_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- failure when webanpick == 0 and sbd == 0" );
      op.sbd_amount.amount = 0;
      WEBANPICK_REQUIRE_THROW( op.validate(), fc::exception );
      ...
      BOOST_TEST_MESSAGE( "--- failure when webanpick is not webanpick symbol" );
      op.sbd_amount.symbol = SBD_SYMBOL;
      op.webanpick_amount = ASSET( "1.000 TBD" );
      WEBANPICK_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( escrow_release_apply )
{
   try
   {
      ...
      op.webanpick_amount = ASSET( "0.100 TESTS" );
      ...
      BOOST_REQUIRE( db->get_escrow( op.from, op.escrow_id ).webanpick_balance == ASSET( "0.900 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( op.from, op.escrow_id ).webanpick_balance == ASSET( "0.800 TESTS" ) );
      ...
      BOOST_TEST_MESSAGE( "--- failure when releasing less webanpick than available" );
      op.webanpick_amount = ASSET( "0.000 TESTS" );
      op.sbd_amount = ASSET( "1.000 TBD" );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).webanpick_balance == ASSET( "0.700 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).webanpick_balance == ASSET( "0.600 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).webanpick_balance == ASSET( "0.500 TESTS" ) );


      BOOST_TEST_MESSAGE( "--- success deleting escrow when balances are both zero" );
      tx.clear();
      op.webanpick_amount = ASSET( "0.500 TESTS" );
      tx.operations.push_back( op );
      tx.sign( sam_private_key, db->get_chain_id() );
      db->push_transaction( tx, 0 );

      BOOST_REQUIRE( db->get_account( "alice" ).balance == ASSET( "9.700 TESTS" ) );
      WEBANPICK_REQUIRE_THROW( db->get_escrow( et_op.from, et_op.escrow_id ), fc::exception );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).webanpick_balance == ASSET( "0.900 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).webanpick_balance == ASSET( "0.800 TESTS" ) );
      ...
      tx.clear();
      op.webanpick_amount = ASSET( "0.600 TESTS" );
      tx.operations.push_back( op );
      tx.sign( alice_private_key, db->get_chain_id() );
      db->push_transaction( tx, 0 );
...
BOOST_AUTO_TEST_CASE( transfer_to_savings_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "success when amount is WEBANPICK" );
      op.amount = ASSET( "1.000 TESTS" );
      op.validate();
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( transfer_to_savings_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- success transferring WEBANPICK to self" );
      op.to = "alice";
      ...
      BOOST_TEST_MESSAGE( "--- success transferring WEBANPICK to other" );
      op.to = "bob";
      op.amount = ASSET( "1.000 TESTS" );
...
BOOST_AUTO_TEST_CASE( transfer_from_savings_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- success withdrawing WEBANPICK to self" );
      op.to = "alice";
      ...
      BOOST_REQUIRE( db->get_savings_withdraw( "alice", op.request_id ).complete == db->head_block_time() + WEBANPICK_SAVINGS_WITHDRAW_TIME );
      validate_database();
      ...
      BOOST_TEST_MESSAGE( "--- success withdrawing WEBANPICK to other" );
      op.to = "bob";
      op.amount = ASSET( "1.000 TESTS" );
      op.request_id = 3;
      ...
      generate_blocks( db->head_block_time() + WEBANPICK_SAVINGS_WITHDRAW_TIME - fc::seconds( WEBANPICK_BLOCK_INTERVAL ), true );
      ...
      for( int i = 0; i < WEBANPICK_SAVINGS_WITHDRAW_REQUEST_LIMIT; i++ )
      {
         op.request_id = i;
         tx.clear();
         tx.operations.push_back( op );
      ...
      op.request_id = WEBANPICK_SAVINGS_WITHDRAW_REQUEST_LIMIT;
      tx.clear();
      tx.operations.push_back( op );
      tx.sign( alice_private_key, db->get_chain_id() );
      WEBANPICK_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
      BOOST_REQUIRE( db->get_account( "alice" ).savings_withdraw_requests == WEBANPICK_SAVINGS_WITHDRAW_REQUEST_LIMIT );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( decline_voting_rights_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( itr->effective_date == db->head_block_time() + WEBANPICK_OWNER_AUTH_RECOVERY_PERIOD );
      ...
      vote.weight = WEBANPICK_1_PERCENT * 50;
      tx.clear();
      tx.operations.push_back( vote );
      tx.sign( alice_private_key, db->get_chain_id() );
      WEBANPICK_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
...
BOOST_AUTO_TEST_CASE( account_bandwidth )
{
   try
   {
      ...
      BOOST_REQUIRE( average_bandwidth == fc::raw::pack_size( tx ) * 10 * WEBANPICK_BANDWIDTH_PRECISION );
      ...
      BOOST_REQUIRE( last_bandwidth_update == db->head_block_time() );
      BOOST_REQUIRE( average_bandwidth == total_bandwidth + fc::raw::pack_size( tx ) * 10 * WEBANPICK_BANDWIDTH_PRECISION );
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( claim_reward_balance_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "Testing wrong WEBANPICK symbol" );
      op.reward_webanpick = ASSET( "1.000 TBD" );
      WEBANPICK_REQUIRE_THROW( op.validate(), fc::assert_exception );


      BOOST_TEST_MESSAGE( "Testing wrong SBD symbol" );
      op.reward_webanpick = ASSET( "1.000 TESTS" );
...
BOOST_AUTO_TEST_CASE( claim_reward_balance_apply )
{
   try
   {
      ...
      db_plugin->debug_update( []( database& db )
      {
         ...
         {
            a.reward_webanpick_balance = ASSET( "10.000 TESTS" );
            a.reward_sbd_balance = ASSET( "10.000 TBD" );
            a.reward_vesting_balance = ASSET( "10.000000 VESTS" );
            a.reward_vesting_webanpick = ASSET( "10.000 TESTS" );
         });
         ...
         db.modify( db.get_dynamic_global_properties(), []( dynamic_global_property_object& gpo )
         {
            ...
            gpo.pending_rewarded_vesting_webanpick += ASSET( "10.000 TESTS" );
         });
      });
      ...
      auto alice_webanpick = db->get_account( "alice" ).balance;
      auto alice_sbd = db->get_account( "alice" ).sbd_balance;
      auto alice_vests = db->get_account( "alice" ).vesting_shares;


      BOOST_TEST_MESSAGE( "--- Attempting to claim more WEBANPICK than exists in the reward balance." );

      claim_reward_balance_operation op;
      signed_transaction tx;

      op.account = "alice";
      op.reward_webanpick = ASSET( "20.000 TESTS" );
      op.reward_sbd = ASSET( "0.000 TBD" );
      op.reward_vests = ASSET( "0.000000 VESTS" );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).balance == alice_webanpick + op.reward_webanpick );
      BOOST_REQUIRE( db->get_account( "alice" ).reward_webanpick_balance == ASSET( "10.000 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).reward_vesting_webanpick == ASSET( "5.000 TESTS" ) );
      validate_database();
      ...
      op.reward_webanpick = ASSET( "10.000 TESTS" );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).balance == alice_webanpick + op.reward_webanpick );
      BOOST_REQUIRE( db->get_account( "alice" ).reward_webanpick_balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).sbd_balance == alice_sbd + op.reward_sbd );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).reward_vesting_webanpick == ASSET( "0.000 TESTS" ) );
            validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( delegate_vesting_shares_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice_comment.net_rshares.value == bob_acc.effective_vesting_shares().amount.value * ( old_voting_power - bob_acc.voting_power ) / WEBANPICK_100_PERCENT - WEBANPICK_VOTE_DUST_THRESHOLD);
      BOOST_REQUIRE( itr->rshares == bob_acc.effective_vesting_shares().amount.value * ( old_voting_power - bob_acc.voting_power ) / WEBANPICK_100_PERCENT - WEBANPICK_VOTE_DUST_THRESHOLD );
      ...
      tx.set_expiration( db->head_block_time() + WEBANPICK_MAX_TIME_UNTIL_EXPIRATION );
      tx.sign( sam_private_key, db->get_chain_id() );
      WEBANPICK_REQUIRE_THROW( db->push_transaction( tx ), fc::assert_exception );
      ...
      BOOST_REQUIRE( exp_obj->expiration == db->head_block_time() + WEBANPICK_CASHOUT_WINDOW_SECONDS );
      ...
      generate_blocks( exp_obj->expiration + WEBANPICK_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( comment_beneficiaries_validate )
{
   try
   {
      ...
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), WEBANPICK_100_PERCENT + 1 ) );
      op.extensions.insert( b );
      WEBANPICK_REQUIRE_THROW( op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( "--- Testing more than 100% total weight" );
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), WEBANPICK_1_PERCENT * 75 ) );
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "sam" ), WEBANPICK_1_PERCENT * 75 ) );
      op.extensions.clear();
      op.extensions.insert( b );
      WEBANPICK_REQUIRE_THROW( op.validate(), fc::assert_exception );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( "bob", WEBANPICK_1_PERCENT * 2 ) );
      b.beneficiaries.push_back( beneficiary_route_type( "bob", WEBANPICK_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      WEBANPICK_REQUIRE_THROW( op.validate(), fc::assert_exception );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( "bob", WEBANPICK_1_PERCENT ) );
      b.beneficiaries.push_back( beneficiary_route_type( "alice", WEBANPICK_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      WEBANPICK_REQUIRE_THROW( op.validate(), fc::assert_exception );
...
BOOST_AUTO_TEST_CASE( comment_beneficiaries_apply )
{
   try
   {
      ...
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), WEBANPICK_1_PERCENT ) );

      for( size_t i = 0; i < 8; i++ )
      {
         b.beneficiaries.push_back( beneficiary_route_type( account_name_type( WEBANPICK_INIT_MINER_NAME + fc::to_string( i ) ), WEBANPICK_1_PERCENT ) );
      }
      ...
      tx.operations.push_back( op );
      tx.sign( alice_private_key, db->get_chain_id() );
      WEBANPICK_REQUIRE_THROW( db->push_transaction( tx ), chain::plugin_exception );


      BOOST_TEST_MESSAGE( "--- Test specifying a non-existent benefactor" );
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "doug" ), WEBANPICK_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      tx.clear();
      tx.operations.push_back( op );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), 25 * WEBANPICK_1_PERCENT ) );
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "sam" ), 50 * WEBANPICK_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "dave" ), 25 * WEBANPICK_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      tx.sign( alice_private_key, db->get_chain_id() );
      WEBANPICK_REQUIRE_THROW( db->push_transaction( tx ), fc::assert_exception );
      ...
      generate_blocks( db->get_comment( "alice", string( "test" ) ).cashout_time - WEBANPICK_BLOCK_INTERVAL );

      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_dynamic_global_properties(), [=]( dynamic_global_property_object& gpo )
         {
            gpo.current_supply -= gpo.total_reward_fund_webanpick;
            gpo.total_reward_fund_webanpick = ASSET( "100.000 TESTS" );
            gpo.current_supply += gpo.total_reward_fund_webanpick;
         });
      });
      ...
      BOOST_REQUIRE( db->get_account( "bob" ).reward_webanpick_balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "bob" ).reward_sbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( "bob" ).reward_vesting_webanpick.amount + db->get_account( "sam" ).reward_vesting_webanpick.amount == db->get_comment( "alice", string( "test" ) ).beneficiary_payout_value.amount );
      BOOST_REQUIRE( ( db->get_account( "alice" ).reward_sbd_balance.amount + db->get_account( "alice" ).reward_vesting_webanpick.amount ) == db->get_account( "bob" ).reward_vesting_webanpick.amount + 2 );
      BOOST_REQUIRE( ( db->get_account( "alice" ).reward_sbd_balance.amount + db->get_account( "alice" ).reward_vesting_webanpick.amount ) * 2 == db->get_account( "sam" ).reward_vesting_webanpick.amount + 3 );
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( witness_set_properties_validate )
{
   try
   {
      ...
      prop_op.owner = "alice";
      WEBANPICK_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );
      ...
      BOOST_TEST_MESSAGE( "--- failure when setting maximum_block_size below WEBANPICK_MIN_BLOCK_SIZE_LIMIT" );
      prop_op.props.erase( "account_creation_fee" );
      prop_op.props[ "maximum_block_size" ] = fc::raw::pack_to_vector( WEBANPICK_MIN_BLOCK_SIZE_LIMIT - 1 );
      WEBANPICK_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );
      ...
      BOOST_TEST_MESSAGE( "--- failure when setting sbd_interest_rate to WEBANPICK_100_PERCENT + 1" );
      prop_op.props[ "sbd_interest_rate" ].clear();
      prop_op.props[ "sbd_interest_rate" ] = fc::raw::pack_to_vector( WEBANPICK_100_PERCENT + 1 );
      WEBANPICK_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( "--- failure when setting new sbd_exchange_rate with SBD / WEBANPICK" );
...
BOOST_AUTO_TEST_CASE( witness_set_properties_authorities )
{
   try
   {
      ...
      expected_keys.push_back( authority( 1, WEBANPICK_NULL_ACCOUNT, 1 ) );
      op.get_required_authorities( key_auths );
      BOOST_REQUIRE( key_auths == expected_keys );
...
BOOST_AUTO_TEST_CASE( witness_set_properties_apply )
{
   try
   {
      ...
      tx.sign( signing_key, db->get_chain_id() );
      db->push_transaction( tx, 0 );
      BOOST_REQUIRE( alice_witness.props.maximum_block_size == WEBANPICK_MIN_BLOCK_SIZE_LIMIT + 1 );
