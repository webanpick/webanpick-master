...
BOOST_AUTO_TEST_CASE( asset_symbol_validate )
{
   try
   {
      ...
      // specific cases in https://github.com/webanpickit/webanpick/issues/1738
      check_validate( "0", 0 );
      check_validate( "d2", 1 );
...
BOOST_AUTO_TEST_CASE( smt_create_validate )
{
   try
   {
      ...
      op.smt_creation_fee.amount = WEBANPICK_MAX_SHARE_SUPPLY;
      op.validate();
...
BOOST_AUTO_TEST_CASE( smt_create_apply )
{
   try
   {
      ...
      // Fund with WEBANPICK, and set fee with SBD.
      ...
      // Now fund with SBD, and set fee with WEBANPICK.
      convert( "alice", asset( test_amount, WEBANPICK_SYMBOL ) );
      // Declare fee in WEBANPICK though alice has none.
      op.smt_creation_fee = asset( test_amount, WEBANPICK_SYMBOL );
      // Throw due to insufficient balance of WEBANPICK.
      FAIL_WITH_OP(op, alice_private_key, fc::assert_exception);
      ...
      // Check too low fee in WEBANPICK.
      FUND( "bob", too_low_fee_amount );
      op.smt_creation_fee = asset( too_low_fee_amount, WEBANPICK_SYMBOL );
      FAIL_WITH_OP(op, bob_private_key, fc::assert_exception);

      // Check too low fee in SBD.
      convert( "bob", asset( too_low_fee_amount, WEBANPICK_SYMBOL ) );
...
BOOST_AUTO_TEST_CASE( setup_emissions_validate )
{
   try
   {
      ...
      op.control_account = "alice";
      // schedule_time <= WEBANPICK_GENESIS_TIME;
      WEBANPICK_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( set_setup_parameters_validate )
{
   try
   {
      ...
      op.control_account = "####";
      WEBANPICK_REQUIRE_THROW( op.validate(), fc::exception ); // invalid account name
...
BOOST_AUTO_TEST_CASE( setup_emissions_apply )
{
   try
   {
      ...
      auto smts = create_smt_3("alice", alice_private_key);
      {
         ...
         const webanpick::chain::smt_token_object* smt = db->find< webanpick::chain::smt_token_object, by_symbol >( fail_op.symbol );
         FC_ASSERT( smt != nullptr, "The SMT has just been created!" );
         FC_ASSERT( smt->phase < webanpick::chain::smt_token_object::smt_phase::setup_completed, "Who closed setup phase?!" );
         db->modify( *smt, [&]( webanpick::chain::smt_token_object& token )
         {
            token.phase = webanpick::chain::smt_token_object::smt_phase::setup_completed;
         });
...
BOOST_AUTO_TEST_CASE( smt_setup_validate )
{
   try
   {
      ...
      // Do minimal operation setup that allows successful validatation.
      {
         ...
         gpolicy.pre_soft_cap_unit.webanpick_unit.emplace( "founder", 100 );
...
BOOST_AUTO_TEST_CASE( comment_votable_assers_validate )
{
   try
   {
...

      {
         ...
         BOOST_TEST_MESSAGE( "--- Testing invalid configuration of votable_assets - WEBANPICK added to container" );
         allowed_vote_assets ava;
         const auto& smt = smts.front();
         ava.add_votable_asset(smt, share_type(20), false);
         ava.add_votable_asset(WEBANPICK_SYMBOL, share_type(20), true);
         op.extensions.insert( ava );
         WEBANPICK_REQUIRE_THROW( op.validate(), fc::assert_exception );
      }
   }
...
BOOST_AUTO_TEST_CASE( asset_symbol_vesting_methods )
{
   try
   {
      ...
      asset_symbol_type Webanpick = WEBANPICK_SYMBOL;
      FC_ASSERT( Webanpick.is_vesting() == false );
      FC_ASSERT( Webanpick.get_paired_symbol() == VESTS_SYMBOL );

      asset_symbol_type Vests = VESTS_SYMBOL;
      FC_ASSERT( Vests.is_vesting() );
      FC_ASSERT( Vests.get_paired_symbol() == WEBANPICK_SYMBOL );
