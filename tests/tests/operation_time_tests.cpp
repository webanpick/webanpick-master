...
#include <webanpick/protocol/exceptions.hpp>
...
#include <webanpick/plugins/debug_node/debug_node_plugin.hpp>
...
BOOST_AUTO_TEST_CASE( comment_payout_equalize )
{
   try
   {
      ...
      // set a ridiculously high WEBANPICK price ($1 / satoshi) to disable dust threshold
      ...
      for( const auto& author : authors )
      {
         ...
         com.permlink = "mypost";
         com.parent_author = WEBANPICK_ROOT_POST_PARENT;
         ...
         tx.set_expiration( db->head_block_time() + WEBANPICK_MAX_TIME_UNTIL_EXPIRATION );
      ...
      for( const auto& voter : voters )
      {
         ...
         vote.weight = WEBANPICK_100_PERCENT;
      ...
      //auto reward_webanpick = db->get_dynamic_global_properties().total_reward_fund_webanpick;

      // generate a few blocks to seed the reward fund
      generate_blocks(10);
      //const auto& rf = db->get< reward_fund_object, by_name >( WEBANPICK_POST_REWARD_FUND_NAME );
      //idump( (rf) );
      ...
      for( const auto& author : authors )
      {
         ...
         ilog( "${n} : ${webanpick} ${sbd}", ("n", author.name)("webanpick", a.reward_webanpick_balance)("sbd", a.reward_sbd_balance) );
      }
      for( const auto& voter : voters )
      {
         const account_object& a = db->get_account(voter.name);
         ilog( "${n} : ${webanpick} ${sbd}", ("n", voter.name)("webanpick", a.reward_webanpick_balance)("sbd", a.reward_sbd_balance) );
      }
      */
...
BOOST_AUTO_TEST_CASE( comment_payout_dust )
{
   try
   {
      ...
      vote.author = "alice";
      vote.permlink = "test";
      vote.weight = 81 * WEBANPICK_1_PERCENT;
      ...
      vote.voter = "bob";
      vote.author = "bob";
      vote.weight = 59 * WEBANPICK_1_PERCENT;
      ...
      // If comments are paid out independent of order, then the last satoshi of WEBANPICK cannot be divided among them
      const auto rf = db->get< reward_fund_object, by_name >( WEBANPICK_POST_REWARD_FUND_NAME );
...
BOOST_AUTO_TEST_CASE( reward_funds )
{
   try
   {
      ...
      {
         const auto& post_rf = db->get< reward_fund_object, by_name >( WEBANPICK_POST_REWARD_FUND_NAME );
         const auto& comment_rf = db->get< reward_fund_object, by_name >( WEBANPICK_COMMENT_REWARD_FUND_NAME );
...
BOOST_AUTO_TEST_CASE( recent_claims_decay )
{
   try
   {
      ...
      auto alice_vshares = util::evaluate_reward_curve( db->get_comment( "alice", string( "test" ) ).net_rshares.value,
         db->get< reward_fund_object, by_name >( WEBANPICK_POST_REWARD_FUND_NAME ).author_reward_curve,
         db->get< reward_fund_object, by_name >( WEBANPICK_POST_REWARD_FUND_NAME ).content_constant );
      ...
      {
         alice_vshares -= ( alice_vshares * WEBANPICK_BLOCK_INTERVAL ) / WEBANPICK_RECENT_RSHARES_DECAY_TIME_HF19.to_seconds();
         const auto& post_rf = db->get< reward_fund_object, by_name >( WEBANPICK_POST_REWARD_FUND_NAME );
...
/*BOOST_AUTO_TEST_CASE( comment_payout )
{
   try
   {
      ...
      //generate_blocks( db->get_comment( "bob", string( "test" ) ).cashout_time - WEBANPICK_BLOCK_INTERVAL, true );

      auto reward_webanpick = db->get_dynamic_global_properties().total_reward_fund_webanpick + ASSET( "1.667 TESTS" );
      ...
      auto bob_comment_payout = asset( ( ( uint128_t( bob_comment_rshares.value ) * bob_comment_rshares.value * reward_webanpick.amount.value ) / total_rshares2 ).to_uint64(), WEBANPICK_SYMBOL );
      auto bob_comment_discussion_rewards = asset( bob_comment_payout.amount / 4, WEBANPICK_SYMBOL );
      bob_comment_payout -= bob_comment_discussion_rewards;
      auto bob_comment_sbd_reward = db->to_sbd( asset( bob_comment_payout.amount / 2, WEBANPICK_SYMBOL ) );
      auto bob_comment_vesting_reward = ( bob_comment_payout - asset( bob_comment_payout.amount / 2, WEBANPICK_SYMBOL) ) * db->get_dynamic_global_properties().get_vesting_share_price();

      BOOST_TEST_MESSAGE( "Cause first payout" );

      generate_block();

      BOOST_REQUIRE( db->get_dynamic_global_properties().total_reward_fund_webanpick == reward_webanpick - bob_comment_payout );
      ...
      vote.voter = "dave";
      vote.author = "bob";
      vote.weight = WEBANPICK_1_PERCENT;
      tx.operations.push_back( vote );
      tx.sign( dave_private_key, db->get_chain_id() );
      db->push_transaction( tx, 0 );
...
BOOST_AUTO_TEST_CASE( comment_payout )
{
   try
   {
      ...
      generate_blocks( fc::time_point_sec( db->head_block_time().sec_since_epoch() + WEBANPICK_CASHOUT_WINDOW_SECONDS / 2 ), true );
      ...
      generate_blocks( db->get_comment( "bob", string( "test" ) ).cashout_time - ( WEBANPICK_BLOCK_INTERVAL / 2 ), true );
      ...
      auto reward_webanpick = db->get_dynamic_global_properties().total_reward_fund_webanpick + ASSET( "2.000 TESTS" );
      auto total_rshares2 = db->get_dynamic_global_properties().total_reward_shares2;
      ...
      auto bob_comment_payout = asset( ( ( uint128_t( bob_comment_rshares.value ) * bob_comment_rshares.value * reward_webanpick.amount.value ) / total_rshares2 ).to_uint64(), WEBANPICK_SYMBOL );
      auto bob_comment_vote_rewards = asset( bob_comment_payout.amount / 2, WEBANPICK_SYMBOL );
      bob_comment_payout -= bob_comment_vote_rewards;
      auto bob_comment_sbd_reward = asset( bob_comment_payout.amount / 2, WEBANPICK_SYMBOL ) * exchange_rate;
      auto bob_comment_vesting_reward = ( bob_comment_payout - asset( bob_comment_payout.amount / 2, WEBANPICK_SYMBOL ) ) * db->get_dynamic_global_properties().get_vesting_share_price();
      auto unclaimed_payments = bob_comment_vote_rewards;
      auto alice_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "alice" ) ).id ) )->weight ) * bob_comment_vote_rewards.amount.value ) / bob_comment_vote_total ), WEBANPICK_SYMBOL );
      auto alice_vote_vesting = alice_vote_reward * db->get_dynamic_global_properties().get_vesting_share_price();
      auto bob_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "bob" ) ).id ) )->weight ) * bob_comment_vote_rewards.amount.value ) / bob_comment_vote_total ), WEBANPICK_SYMBOL );
      auto bob_vote_vesting = bob_vote_reward * db->get_dynamic_global_properties().get_vesting_share_price();
      auto sam_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "sam" ) ).id ) )->weight ) * bob_comment_vote_rewards.amount.value ) / bob_comment_vote_total ), WEBANPICK_SYMBOL );
      ...
      BOOST_REQUIRE( db->get_dynamic_global_properties().total_reward_fund_webanpick.amount.value == reward_webanpick.amount.value - ( bob_comment_payout + bob_comment_vote_rewards - unclaimed_payments ).amount.value );
      ...
      generate_blocks( db->get_comment( "alice", string( "test" ) ).cashout_time - ( WEBANPICK_BLOCK_INTERVAL / 2 ), true );
      ...
      reward_webanpick = db->get_dynamic_global_properties().total_reward_fund_webanpick + ASSET( "2.000 TESTS" );
      ...
      u256 rf( reward_webanpick.amount.value );
      u256 trs2 = total_rshares2.hi;
      trs2 = ( trs2 << 64 ) + total_rshares2.lo;
      auto rs2 = rs*rs;

      auto alice_comment_payout = asset( static_cast< uint64_t >( ( rf * rs2 ) / trs2 ), WEBANPICK_SYMBOL );
      auto alice_comment_vote_rewards = asset( alice_comment_payout.amount / 2, WEBANPICK_SYMBOL );
      alice_comment_payout -= alice_comment_vote_rewards;
      auto alice_comment_sbd_reward = asset( alice_comment_payout.amount / 2, WEBANPICK_SYMBOL ) * exchange_rate;
      auto alice_comment_vesting_reward = ( alice_comment_payout - asset( alice_comment_payout.amount / 2, WEBANPICK_SYMBOL ) ) * db->get_dynamic_global_properties().get_vesting_share_price();
      unclaimed_payments = alice_comment_vote_rewards;
      alice_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "alice", string( "test" ).id, db->get_account( "alice" ) ).id ) )->weight ) * alice_comment_vote_rewards.amount.value ) / alice_comment_vote_total ), WEBANPICK_SYMBOL );
      alice_vote_vesting = alice_vote_reward * db->get_dynamic_global_properties().get_vesting_share_price();
      bob_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "alice", string( "test" ).id, db->get_account( "bob" ) ).id ) )->weight ) * alice_comment_vote_rewards.amount.value ) / alice_comment_vote_total ), WEBANPICK_SYMBOL );
      bob_vote_vesting = bob_vote_reward * db->get_dynamic_global_properties().get_vesting_share_price();
      sam_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "alice", string( "test" ).id, db->get_account( "sam" ) ).id ) )->weight ) * alice_comment_vote_rewards.amount.value ) / alice_comment_vote_total ), WEBANPICK_SYMBOL );
      sam_vote_vesting = sam_vote_reward * db->get_dynamic_global_properties().get_vesting_share_price();
      auto dave_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "alice", string( "test" ).id, db->get_account( "dave" ) ).id ) )->weight ) * alice_comment_vote_rewards.amount.value ) / alice_comment_vote_total ), WEBANPICK_SYMBOL );
      ...
      BOOST_REQUIRE( ( db->get_dynamic_global_properties().total_reward_fund_webanpick + alice_comment_payout + alice_comment_vote_rewards - unclaimed_payments ).amount.value == reward_webanpick.amount.value );
...
BOOST_AUTO_TEST_CASE( nested_comments )
{
   try
   {
      ...
      vote_op.weight = WEBANPICK_100_PERCENT;
      ...
      tx.operations.push_back( vote_op );
      vote_op.author = "dave";
      vote_op.weight = WEBANPICK_1_PERCENT * 20;
      tx.operations.push_back( vote_op );
      tx.sign( bob_private_key, db->get_chain_id() );
      db->push_transaction( tx, 0 );
      ...
      generate_blocks( db->get_comment( "alice", string( "test" ) ).cashout_time - fc::seconds( WEBANPICK_BLOCK_INTERVAL ), true );

      auto gpo = db->get_dynamic_global_properties();
      uint128_t reward_webanpick = gpo.total_reward_fund_webanpick.amount.value + ASSET( "2.000 TESTS" ).amount.value;
      ...
      auto alice_comment_reward = ( ( reward_webanpick * alice_comment.net_rshares.value * alice_comment.net_rshares.value ) / total_rshares2 ).to_uint64();
      total_rshares2 -= uint128_t( alice_comment.net_rshares.value ) * ( alice_comment.net_rshares.value );
      reward_webanpick -= alice_comment_reward;
      auto alice_comment_vote_rewards = alice_comment_reward / 2;
      alice_comment_reward -= alice_comment_vote_rewards;

      auto alice_vote_alice_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "alice", string( "test" ).id, db->get_account( "alice" ) ).id ) )->weight ) * alice_comment_vote_rewards ) / alice_comment.total_vote_weight ), WEBANPICK_SYMBOL );
      auto bob_vote_alice_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "alice", string( "test" ).id, db->get_account( "bob" ) ).id ) )->weight ) * alice_comment_vote_rewards ) / alice_comment.total_vote_weight ), WEBANPICK_SYMBOL );
      reward_webanpick += alice_comment_vote_rewards - ( alice_vote_alice_reward + bob_vote_alice_reward ).amount.value;

      auto bob_comment_reward = ( ( reward_webanpick * bob_comment.net_rshares.value * bob_comment.net_rshares.value ) / total_rshares2 ).to_uint64();
      total_rshares2 -= uint128_t( bob_comment.net_rshares.value ) * bob_comment.net_rshares.value;
      reward_webanpick -= bob_comment_reward;
      auto bob_comment_vote_rewards = bob_comment_reward / 2;
      bob_comment_reward -= bob_comment_vote_rewards;

      auto alice_vote_bob_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "alice" ) ).id ) )->weight ) * bob_comment_vote_rewards ) / bob_comment.total_vote_weight ), WEBANPICK_SYMBOL );
      auto bob_vote_bob_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "bob" ) ).id ) )->weight ) * bob_comment_vote_rewards ) / bob_comment.total_vote_weight ), WEBANPICK_SYMBOL );
      auto sam_vote_bob_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "sam" ) ).id ) )->weight ) * bob_comment_vote_rewards ) / bob_comment.total_vote_weight ), WEBANPICK_SYMBOL );
      reward_webanpick += bob_comment_vote_rewards - ( alice_vote_bob_reward + bob_vote_bob_reward + sam_vote_bob_reward ).amount.value;

      auto dave_comment_reward = ( ( reward_webanpick * dave_comment.net_rshares.value * dave_comment.net_rshares.value ) / total_rshares2 ).to_uint64();
      total_rshares2 -= uint128_t( dave_comment.net_rshares.value ) * dave_comment.net_rshares.value;
      reward_webanpick -= dave_comment_reward;
      auto dave_comment_vote_rewards = dave_comment_reward / 2;
      dave_comment_reward -= dave_comment_vote_rewards;

      auto bob_vote_dave_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "dave", string( "test" ).id, db->get_account( "bob" ) ).id ) )->weight ) * dave_comment_vote_rewards ) / dave_comment.total_vote_weight ), WEBANPICK_SYMBOL );
      reward_webanpick += dave_comment_vote_rewards - bob_vote_dave_reward.amount.value;
      ...
      auto alice_comment_total_payout = db->to_sbd( asset( alice_pays_alice_sbd + alice_pays_alice_vest, WEBANPICK_SYMBOL ) );
      alice_comment_total_payout += db->to_sbd( asset( bob_pays_alice_sbd + bob_pays_alice_vest, WEBANPICK_SYMBOL ) );
      alice_comment_total_payout += db->to_sbd( asset( dave_pays_alice_sbd + dave_pays_alice_vest, WEBANPICK_SYMBOL ) );
      auto bob_comment_total_payout = db->to_sbd( asset( bob_pays_bob_sbd + bob_pays_bob_vest, WEBANPICK_SYMBOL ) );
      bob_comment_total_payout += db->to_sbd( asset( dave_pays_bob_sbd + dave_pays_bob_vest, WEBANPICK_SYMBOL ) );
      auto sam_comment_total_payout = db->to_sbd( asset( dave_pays_sam_sbd + dave_pays_sam_vest, WEBANPICK_SYMBOL ) );
      auto dave_comment_total_payout = db->to_sbd( asset( dave_pays_dave_sbd + dave_pays_dave_vest, WEBANPICK_SYMBOL ) );
      ...
      auto alice_total_sbd = alice_starting_sbd + asset( alice_pays_alice_sbd + bob_pays_alice_sbd + dave_pays_alice_sbd, WEBANPICK_SYMBOL ) * exchange_rate;
      auto alice_total_vesting = alice_starting_vesting + asset( alice_pays_alice_vest + bob_pays_alice_vest + dave_pays_alice_vest + alice_vote_alice_reward.amount + alice_vote_bob_reward.amount, WEBANPICK_SYMBOL ) * gpo.get_vesting_share_price();
      ...
      auto bob_total_sbd = bob_starting_sbd + asset( bob_pays_bob_sbd + dave_pays_bob_sbd, WEBANPICK_SYMBOL ) * exchange_rate;
      auto bob_total_vesting = bob_starting_vesting + asset( bob_pays_bob_vest + dave_pays_bob_vest + bob_vote_alice_reward.amount + bob_vote_bob_reward.amount + bob_vote_dave_reward.amount, WEBANPICK_SYMBOL ) * gpo.get_vesting_share_price();
      ...
      auto sam_total_sbd = sam_starting_sbd + asset( dave_pays_sam_sbd, WEBANPICK_SYMBOL ) * exchange_rate;
      auto sam_total_vesting = bob_starting_vesting + asset( dave_pays_sam_vest + sam_vote_bob_reward.amount, WEBANPICK_SYMBOL ) * gpo.get_vesting_share_price();
      ...
      auto dave_total_sbd = dave_starting_sbd + asset( dave_pays_dave_sbd, WEBANPICK_SYMBOL ) * exchange_rate;
      auto dave_total_vesting = dave_starting_vesting + asset( dave_pays_dave_vest, WEBANPICK_SYMBOL ) * gpo.get_vesting_share_price();
...
BOOST_AUTO_TEST_CASE( vesting_withdrawals )
{
   try
   {
      ...
      auto next_withdrawal = db->head_block_time() + WEBANPICK_VESTING_WITHDRAW_INTERVAL_SECONDS;
      ...
      generate_blocks( next_withdrawal - ( WEBANPICK_BLOCK_INTERVAL / 2 ), true);
      ...
      for( int i = 1; i < WEBANPICK_VESTING_WITHDRAW_INTERVALS - 1; i++ )
      {
         generate_blocks( db->head_block_time() + WEBANPICK_VESTING_WITHDRAW_INTERVAL_SECONDS );
         ...
         if ( i == WEBANPICK_VESTING_WITHDRAW_INTERVALS - 1 )
            BOOST_REQUIRE( alice.next_vesting_withdrawal == fc::time_point_sec::maximum() );
         else
            BOOST_REQUIRE( alice.next_vesting_withdrawal.sec_since_epoch() == ( old_next_vesting + WEBANPICK_VESTING_WITHDRAW_INTERVAL_SECONDS ).sec_since_epoch() );
      ...
      if (  to_withdraw.amount.value % withdraw_rate.amount.value != 0 )
      {
         ...
         generate_blocks( db->head_block_time() + WEBANPICK_VESTING_WITHDRAW_INTERVAL_SECONDS, true );
         fill_op = get_last_operations( 1 )[0].get< fill_vesting_withdraw_operation >();
         gpo = db->get_dynamic_global_properties();

         BOOST_REQUIRE( db->get_account( "alice" ).next_vesting_withdrawal.sec_since_epoch() == ( old_next_vesting + WEBANPICK_VESTING_WITHDRAW_INTERVAL_SECONDS ).sec_since_epoch() );
         BOOST_REQUIRE( fill_op.from_account == "alice" );
         BOOST_REQUIRE( fill_op.to_account == "alice" );
         BOOST_REQUIRE( fill_op.withdrawn.amount.value == withdraw_rate.amount.value );
         BOOST_REQUIRE( std::abs( ( fill_op.deposited - fill_op.withdrawn * gpo.get_vesting_share_price() ).amount.value ) <= 1 );
...
BOOST_AUTO_TEST_CASE( vesting_withdraw_route )
{
   try
   {
      ...
      op.to_account = "bob";
      op.percent = WEBANPICK_1_PERCENT * 50;
      op.auto_vest = true;
      tx.operations.push_back( op );

      BOOST_TEST_MESSAGE( "Setting up sam destination" );
      op.to_account = "sam";
      op.percent = WEBANPICK_1_PERCENT * 30;
      op.auto_vest = false;
      tx.operations.push_back( op );
      tx.sign( alice_private_key, db->get_chain_id() );
      db->push_transaction( tx, 0 );
...

      {
         ...
         BOOST_REQUIRE( alice.balance == old_alice_balance + asset( ( vesting_withdraw_rate.amount * WEBANPICK_1_PERCENT * 20 ) / WEBANPICK_100_PERCENT, VESTS_SYMBOL ) * db->get_dynamic_global_properties().get_vesting_share_price() );
         BOOST_REQUIRE( bob.vesting_shares == old_bob_vesting + asset( ( vesting_withdraw_rate.amount * WEBANPICK_1_PERCENT * 50 ) / WEBANPICK_100_PERCENT, VESTS_SYMBOL ) );
         BOOST_REQUIRE( bob.balance == old_bob_balance );
         BOOST_REQUIRE( sam.vesting_shares == old_sam_vesting );
         BOOST_REQUIRE( sam.balance ==  old_sam_balance + asset( ( vesting_withdraw_rate.amount * WEBANPICK_1_PERCENT * 30 ) / WEBANPICK_100_PERCENT, VESTS_SYMBOL ) * db->get_dynamic_global_properties().get_vesting_share_price() );
      ...
      op.to_account = "sam";
      op.percent = WEBANPICK_1_PERCENT * 50 + 1;
      tx.operations.push_back( op );
      tx.set_expiration( db->head_block_time() + WEBANPICK_MAX_TIME_UNTIL_EXPIRATION );
      tx.sign( alice_private_key, db->get_chain_id() );
      WEBANPICK_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
      ...
      generate_blocks( db->get_account( "alice" ).next_vesting_withdrawal, true );
      {
         ...
         BOOST_REQUIRE( bob.balance == old_bob_balance );
         BOOST_REQUIRE( sam.vesting_shares == old_sam_vesting );
         BOOST_REQUIRE( sam.balance ==  old_sam_balance + asset( ( vesting_withdraw_rate.amount * WEBANPICK_1_PERCENT * 50 ) / WEBANPICK_100_PERCENT, VESTS_SYMBOL ) * db->get_dynamic_global_properties().get_vesting_share_price() );
      }
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( feed_publish_mean )
{
   try
   {
      ...
      generate_blocks( 30 / WEBANPICK_BLOCK_INTERVAL );
      ...
      for( int i = 0; i < 7; i++ )
      {
         transfer( WEBANPICK_INIT_MINER_NAME, accounts[i], asset( 10000, WEBANPICK_SYMBOL ) );
      ...
      ops[0].exchange_rate = price( asset( 1000, SBD_SYMBOL ), asset( 100000, WEBANPICK_SYMBOL ) );
      ops[1].exchange_rate = price( asset( 1000, SBD_SYMBOL ), asset( 105000, WEBANPICK_SYMBOL ) );
      ops[2].exchange_rate = price( asset( 1000, SBD_SYMBOL ), asset(  98000, WEBANPICK_SYMBOL ) );
      ops[3].exchange_rate = price( asset( 1000, SBD_SYMBOL ), asset(  97000, WEBANPICK_SYMBOL ) );
      ops[4].exchange_rate = price( asset( 1000, SBD_SYMBOL ), asset(  99000, WEBANPICK_SYMBOL ) );
      ops[5].exchange_rate = price( asset( 1000, SBD_SYMBOL ), asset(  97500, WEBANPICK_SYMBOL ) );
      ops[6].exchange_rate = price( asset( 1000, SBD_SYMBOL ), asset( 102000, WEBANPICK_SYMBOL ) );

      for( int i = 0; i < 7; i++ )
      {
         txs[i].set_expiration( db->head_block_time() + WEBANPICK_MAX_TIME_UNTIL_EXPIRATION );
      ...
      generate_blocks( WEBANPICK_BLOCKS_PER_HOUR ); // Jump forward 1 hour
      ...
      BOOST_REQUIRE( feed_history.current_median_history == price( asset( 1000, SBD_SYMBOL ), asset( 99000, WEBANPICK_SYMBOL) ) );
      BOOST_REQUIRE( feed_history.price_history[ 0 ] == price( asset( 1000, SBD_SYMBOL ), asset( 99000, WEBANPICK_SYMBOL) ) );
      validate_database();
      ...
      for ( int i = 0; i < 23; i++ )
      {
         ...
         for( int j = 0; j < 7; j++ )
         {
            ...
            ops[j].exchange_rate = price( ops[j].exchange_rate.base, asset( ops[j].exchange_rate.quote.amount + 10, WEBANPICK_SYMBOL ) );
            txs[j].set_expiration( db->head_block_time() + WEBANPICK_MAX_TIME_UNTIL_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( convert_delay )
{
   try
   {
      ...
      generate_blocks( db->head_block_time() + WEBANPICK_CONVERSION_DELAY - fc::seconds( WEBANPICK_BLOCK_INTERVAL / 2 ), true );
...
BOOST_AUTO_TEST_CASE( webanpick_inflation )
{
   try
   {
   /*
      BOOST_TEST_MESSAGE( "Testing WEBANPICK Inflation until the vesting start block" );
      ...
      auto new_rewards = std::max( WEBANPICK_MIN_CONTENT_REWARD, asset( ( WEBANPICK_CONTENT_APR * gpo.virtual_supply.amount ) / ( WEBANPICK_BLOCKS_PER_YEAR * 100 ), WEBANPICK_SYMBOL ) )
         + std::max( WEBANPICK_MIN_CURATE_REWARD, asset( ( WEBANPICK_CURATE_APR * gpo.virtual_supply.amount ) / ( WEBANPICK_BLOCKS_PER_YEAR * 100 ), WEBANPICK_SYMBOL ) );
      auto witness_pay = std::max( WEBANPICK_MIN_PRODUCER_REWARD, asset( ( WEBANPICK_PRODUCER_APR * gpo.virtual_supply.amount ) / ( WEBANPICK_BLOCKS_PER_YEAR * 100 ), WEBANPICK_SYMBOL ) );
      auto witness_pay_shares = asset( 0, VESTS_SYMBOL );
      auto new_vesting_webanpick = asset( 0, WEBANPICK_SYMBOL );
      auto new_vesting_shares = gpo.total_vesting_shares;

      if ( db->get_account( witness_name ).vesting_shares.amount.value == 0 )
      {
         new_vesting_webanpick += witness_pay;
         new_vesting_shares += witness_pay * ( gpo.total_vesting_shares / gpo.total_vesting_fund_webanpick );
      }

      auto new_supply = gpo.current_supply + new_rewards + witness_pay + new_vesting_webanpick;
      new_rewards += gpo.total_reward_fund_webanpick;
      new_vesting_webanpick += gpo.total_vesting_fund_webanpick;
      ...
      BOOST_REQUIRE( gpo.total_reward_fund_webanpick.amount.value == new_rewards.amount.value );
      BOOST_REQUIRE( gpo.total_vesting_fund_webanpick.amount.value == new_vesting_webanpick.amount.value );
      ...
      while( db->head_block_num() < WEBANPICK_START_VESTING_BLOCK - 1)
      {
         ...
         new_rewards = std::max( WEBANPICK_MIN_CONTENT_REWARD, asset( ( WEBANPICK_CONTENT_APR * gpo.virtual_supply.amount ) / ( WEBANPICK_BLOCKS_PER_YEAR * 100 ), WEBANPICK_SYMBOL ) )
            + std::max( WEBANPICK_MIN_CURATE_REWARD, asset( ( WEBANPICK_CURATE_APR * gpo.virtual_supply.amount ) / ( WEBANPICK_BLOCKS_PER_YEAR * 100 ), WEBANPICK_SYMBOL ) );
         witness_pay = std::max( WEBANPICK_MIN_PRODUCER_REWARD, asset( ( WEBANPICK_PRODUCER_APR * gpo.virtual_supply.amount ) / ( WEBANPICK_BLOCKS_PER_YEAR * 100 ), WEBANPICK_SYMBOL ) );
         new_vesting_webanpick = asset( 0, WEBANPICK_SYMBOL );
         ...
         if ( db->get_account( witness_name ).vesting_shares.amount.value == 0 )
         {
            ...
            witness_pay = asset( 0, WEBANPICK_SYMBOL );
         }

         new_supply = gpo.current_supply + new_rewards + witness_pay + new_vesting_webanpick;
         new_rewards += gpo.total_reward_fund_webanpick;
         new_vesting_webanpick += gpo.total_vesting_fund_webanpick;
      ...
      while( db->head_block_num() < WEBANPICK_START_MINER_VOTING_BLOCK - 1 )
      {
         virtual_supply = gpo.virtual_supply;
         witness_name = db->get_scheduled_witness(1);
         old_witness_balance = db->get_account( witness_name ).balance;
         ...
         witness_pay = std::max( WEBANPICK_MIN_PRODUCER_REWARD, asset( ( WEBANPICK_PRODUCER_APR * gpo.virtual_supply.amount ) / ( WEBANPICK_BLOCKS_PER_YEAR * 100 ), WEBANPICK_SYMBOL ) );
         auto witness_pay_shares = asset( 0, VESTS_SYMBOL );
         new_vesting_webanpick = asset( ( witness_pay + new_rewards ).amount * 9, WEBANPICK_SYMBOL );
         new_vesting_shares = gpo.total_vesting_shares;
      ...
      for( int i = 0; i < WEBANPICK_BLOCKS_PER_DAY; i++ )
      {
         virtual_supply = gpo.virtual_supply;
         witness_name = db->get_scheduled_witness(1);
         old_witness_balance = db->get_account( witness_name ).balance;
         ...
         witness_pay = std::max( WEBANPICK_MIN_PRODUCER_REWARD, asset( ( WEBANPICK_PRODUCER_APR * gpo.virtual_supply.amount ) / ( WEBANPICK_BLOCKS_PER_YEAR * 100 ), WEBANPICK_SYMBOL ) );
         witness_pay_shares = witness_pay * gpo.get_vesting_share_price();
         new_vesting_webanpick = asset( ( witness_pay + new_rewards ).amount * 9, WEBANPICK_SYMBOL ) + witness_pay;
         new_vesting_shares = gpo.total_vesting_shares + witness_pay_shares;
         new_supply = gpo.current_supply + new_rewards + new_vesting_webanpick;
         new_rewards += gpo.total_reward_fund_webanpick;
         new_vesting_webanpick += gpo.total_vesting_fund_webanpick;
      ...
      vesting_webanpick = gpo.total_vesting_fund_webanpick;
      reward_webanpick = gpo.total_reward_fund_webanpick;
      ...
      BOOST_REQUIRE_EQUAL( gpo.total_vesting_fund_webanpick.amount.value,
         ( vesting_webanpick.amount.value
            + ( ( ( uint128_t( virtual_supply.amount.value ) / 10 ) / WEBANPICK_BLOCKS_PER_YEAR ) * 9 )
            + ( uint128_t( virtual_supply.amount.value ) / 100 / WEBANPICK_BLOCKS_PER_YEAR ) ).to_uint64() );
      BOOST_REQUIRE_EQUAL( gpo.total_reward_fund_webanpick.amount.value,
         reward_webanpick.amount.value + virtual_supply.amount.value / 10 / WEBANPICK_BLOCKS_PER_YEAR + virtual_supply.amount.value / 10 / WEBANPICK_BLOCKS_PER_DAY );
      BOOST_REQUIRE_EQUAL( db->get_account( witness_name ).vesting_shares.amount.value,
         old_witness_shares.amount.value + ( asset( ( ( virtual_supply.amount.value / WEBANPICK_BLOCKS_PER_YEAR ) * WEBANPICK_1_PERCENT ) / WEBANPICK_100_PERCENT, WEBANPICK_SYMBOL ) * ( vesting_shares / vesting_webanpick ) ).amount.value );
      validate_database();
      */
   }
   FC_LOG_AND_RETHROW();
...
BOOST_AUTO_TEST_CASE( sbd_interest )
{
   try
   {
      ...
      generate_blocks( db->head_block_time() + fc::seconds( WEBANPICK_SBD_INTEREST_COMPOUND_INTERVAL_SEC ), true );
      ...
      BOOST_REQUIRE( static_cast<uint64_t>(db->get_account( "alice" ).sbd_balance.amount.value) == alice_sbd.amount.value - ASSET( "1.000 TBD" ).amount.value + ( ( ( ( uint128_t( alice_sbd.amount.value ) * ( db->head_block_time() - start_time ).to_seconds() ) / WEBANPICK_SECONDS_PER_YEAR ) * gpo.sbd_interest_rate ) / WEBANPICK_100_PERCENT ).to_uint64() );
      ...
      generate_blocks( db->head_block_time() + fc::seconds( WEBANPICK_SBD_INTEREST_COMPOUND_INTERVAL_SEC / 2 ), true );
      ...
      generate_blocks( db->head_block_time() + fc::seconds( ( WEBANPICK_SBD_INTEREST_COMPOUND_INTERVAL_SEC * 7 ) / 3 ), true );
...
BOOST_AUTO_TEST_CASE( liquidity_rewards )
{
   ...
   try
   {
      ...
      generate_blocks( db->head_block_time() + WEBANPICK_MIN_LIQUIDITY_REWARD_PERIOD_SEC_HF10, true );
      ...
      alice_webanpick_volume += ( asset( alice_sbd.amount / 20, SBD_SYMBOL ) * exchange_rate ).amount.value;
      alice_reward_last_update = db->head_block_time();
      bob_webanpick_volume -= ( asset( alice_sbd.amount / 20, SBD_SYMBOL ) * exchange_rate ).amount.value;
      ...
      BOOST_REQUIRE( reward->webanpick_volume == alice_webanpick_volume );
      ...
      BOOST_REQUIRE( reward->webanpick_volume == bob_webanpick_volume );
      ...
      op.owner = "sam";
      op.amount_to_sell = asset( ( alice_sbd.amount.value / 20 ), WEBANPICK_SYMBOL );
      ...
      op.owner = "bob";
      op.orderid = 4;
      op.amount_to_sell = asset( ( alice_sbd.amount.value / 10 ) * 3 - alice_sbd.amount.value / 20, WEBANPICK_SYMBOL );
      ...
      op.min_to_receive = asset( ( alice_sbd.amount.value / 10 ) * 3, WEBANPICK_SYMBOL );
      ...
      BOOST_REQUIRE( fill_order_op.open_pays.amount.value == asset( ( alice_sbd.amount.value / 10 ) * 3 - alice_sbd.amount.value / 20, WEBANPICK_SYMBOL ).amount.value );
      ...
      BOOST_REQUIRE( fill_order_op.open_pays.amount.value == asset( alice_sbd.amount.value / 20, WEBANPICK_SYMBOL ).amount.value );
      BOOST_REQUIRE( fill_order_op.current_owner == "alice" );
      BOOST_REQUIRE( fill_order_op.current_orderid == 5 );
      ...
      BOOST_REQUIRE( reward->webanpick_volume == sam_webanpick_volume );
      ...
      op.min_to_receive = asset( alice_sbd.amount.value / 20 * 2, WEBANPICK_SYMBOL );
      ...
      generate_blocks( db->head_block_time() + fc::seconds( WEBANPICK_MIN_LIQUIDITY_REWARD_PERIOD_SEC_HF10.to_seconds() / 2 ), true );
      ...
      BOOST_REQUIRE( fill_order_op.current_pays.amount.value == asset( alice_sbd.amount.value / 20, WEBANPICK_SYMBOL ).amount.value );
      ...
      alice_webanpick_volume += alice_sbd.amount.value / 20;
      alice_reward_last_update = db->head_block_time();
      sam_webanpick_volume -= alice_sbd.amount.value / 20;
      sam_reward_last_update = db->head_block_time();
      ...
      op.owner = "alice";
      op.amount_to_sell = asset( 8 * ( alice_sbd.amount.value / 20 ), WEBANPICK_SYMBOL );
      ...
      op.min_to_receive = asset( op.amount_to_sell.amount, WEBANPICK_SYMBOL );
      op.orderid = 10;
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      ...
      BOOST_REQUIRE( reward->webanpick_volume == dave_webanpick_volume );
      ...
      bob_webanpick_volume += op.amount_to_sell.amount.value;
      bob_reward_last_update = db->head_block_time();
      dave_webanpick_volume -= op.amount_to_sell.amount.value;
      dave_reward_last_update = db->head_block_time();
      ...
      generate_blocks( WEBANPICK_LIQUIDITY_REWARD_BLOCKS - ( db->head_block_num() % WEBANPICK_LIQUIDITY_REWARD_BLOCKS ) - 1 );

      BOOST_REQUIRE( db->head_block_num() % WEBANPICK_LIQUIDITY_REWARD_BLOCKS == WEBANPICK_LIQUIDITY_REWARD_BLOCKS - 1 );
      ...
      //alice_balance += WEBANPICK_MIN_LIQUIDITY_REWARD;
      ...
      WEBANPICK_REQUIRE_THROW( ops[0].get< liquidity_reward_operation>(), fc::exception );
      //BOOST_REQUIRE( ops[0].get< liquidity_reward_operation>().payout.amount.value == WEBANPICK_MIN_LIQUIDITY_REWARD.amount.value );

      generate_blocks( WEBANPICK_LIQUIDITY_REWARD_BLOCKS );

      //bob_balance += WEBANPICK_MIN_LIQUIDITY_REWARD;
      ...
      alice_webanpick_volume = 0;
      alice_sbd_volume = 0;
      bob_webanpick_volume = 0;
      bob_sbd_volume = 0;

      BOOST_TEST_MESSAGE( "Testing liquidity timeout" );

      generate_blocks( sam_reward_last_update + WEBANPICK_LIQUIDITY_TIMEOUT_SEC - fc::seconds( WEBANPICK_BLOCK_INTERVAL / 2 ) - WEBANPICK_MIN_LIQUIDITY_REWARD_PERIOD_SEC , true );
      ...
      generate_blocks( db->head_block_time() + ( WEBANPICK_BLOCK_INTERVAL / 2 ) + WEBANPICK_LIQUIDITY_TIMEOUT_SEC, true );
      ...
      op.min_to_receive.symbol = WEBANPICK_SYMBOL;
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      tx.set_expiration( db->head_block_time() + WEBANPICK_MAX_TIME_UNTIL_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( post_rate_limit )
{
   try
   {
      ...
      BOOST_REQUIRE( db->get_comment( "alice", string( "test1" ) ).reward_weight == WEBANPICK_100_PERCENT );

      tx.operations.clear();
      tx.signatures.clear();

      generate_blocks( db->head_block_time() + WEBANPICK_MIN_ROOT_COMMENT_INTERVAL + fc::seconds( WEBANPICK_BLOCK_INTERVAL ), true );
      ...
      BOOST_REQUIRE( db->get_comment( "alice", string( "test2" ) ).reward_weight == WEBANPICK_100_PERCENT );
      ...
      BOOST_REQUIRE( db->get_comment( "alice", string( "test3" ) ).reward_weight == WEBANPICK_100_PERCENT );
      ...
      BOOST_REQUIRE( db->get_comment( "alice", string( "test4" ) ).reward_weight == WEBANPICK_100_PERCENT );
      ...
      BOOST_REQUIRE( db->get_comment( "alice", string( "test5" ) ).reward_weight == WEBANPICK_100_PERCENT );
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( comment_freeze )
{
   try
   {
      ...
      vote.voter = "bob";
      vote.weight = WEBANPICK_100_PERCENT * -1;
...
BOOST_AUTO_TEST_CASE( sbd_stability )
{
   try
   {
      ...
      BOOST_REQUIRE( db->get_dynamic_global_properties().sbd_print_rate == WEBANPICK_100_PERCENT );
      ...
      db_plugin->debug_generate_blocks_until( debug_key, fc::time_point_sec( db->get_comment( comment.author, comment.permlink ).cashout_time.sec_since_epoch() - 2 * WEBANPICK_BLOCK_INTERVAL ), true, database::skip_witness_signature );

      auto& gpo = db->get_dynamic_global_properties();

      BOOST_TEST_MESSAGE( "Changing sam and gpo to set up market cap conditions" );

      asset sbd_balance = asset( ( gpo.virtual_supply.amount * ( WEBANPICK_SBD_STOP_PERCENT + 30 ) ) / WEBANPICK_100_PERCENT, WEBANPICK_SYMBOL ) * exchange_rate;
      ...
      auto comment_reward = ( gpo.total_reward_fund_webanpick.amount + 2000 ) - ( ( gpo.total_reward_fund_webanpick.amount + 2000 ) * 25 * WEBANPICK_1_PERCENT ) / WEBANPICK_100_PERCENT ;
      comment_reward /= 2;
      auto sbd_reward = ( comment_reward * gpo.sbd_print_rate ) / WEBANPICK_100_PERCENT;
      auto alice_sbd = db->get_account( "alice" ).sbd_balance + db->get_account( "alice" ).reward_sbd_balance + asset( sbd_reward, WEBANPICK_SYMBOL ) * exchange_rate;
      auto alice_webanpick = db->get_account( "alice" ).balance + db->get_account( "alice" ).reward_webanpick_balance ;

      BOOST_TEST_MESSAGE( "Checking printing SBD has slowed" );
      BOOST_REQUIRE( db->get_dynamic_global_properties().sbd_print_rate < WEBANPICK_100_PERCENT );

      BOOST_TEST_MESSAGE( "Pay out comment and check rewards are paid as WEBANPICK" );
      db_plugin->debug_generate_blocks( debug_key, 1, database::skip_witness_signature );

      validate_database();

      BOOST_REQUIRE( db->get_account( "alice" ).sbd_balance + db->get_account( "alice" ).reward_sbd_balance == alice_sbd );
      BOOST_REQUIRE( db->get_account( "alice" ).balance + db->get_account( "alice" ).reward_webanpick_balance > alice_webanpick );
      ...
      while( ( db->get_dynamic_global_properties().current_sbd_supply * exchange_rate ).amount >= ( db->get_dynamic_global_properties().virtual_supply.amount * WEBANPICK_SBD_START_PERCENT ) / WEBANPICK_100_PERCENT )
      {
         auto& gpo = db->get_dynamic_global_properties();
...
BOOST_AUTO_TEST_CASE( sbd_price_feed_limit )
{
   try
   {
      ...
      auto new_exchange_rate = price( gpo.current_sbd_supply, asset( ( WEBANPICK_100_PERCENT ) * gpo.current_supply.amount, WEBANPICK_SYMBOL ) );
...
BOOST_AUTO_TEST_CASE( clear_null_account )
{
   try
   {
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_account( WEBANPICK_NULL_ACCOUNT ), [&]( account_object& a )
         {
            a.reward_webanpick_balance = ASSET( "1.000 TESTS" );
            a.reward_sbd_balance = ASSET( "1.000 TBD" );
            a.reward_vesting_balance = ASSET( "1.000000 VESTS" );
            a.reward_vesting_webanpick = ASSET( "1.000 TESTS" );
         });
         ...
         db.modify( db.get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
         {
            ...
            gpo.pending_rewarded_vesting_webanpick += ASSET( "1.000 TESTS" );
         });
      });

      validate_database();

      BOOST_REQUIRE( db->get_account( WEBANPICK_NULL_ACCOUNT ).balance == ASSET( "1.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( WEBANPICK_NULL_ACCOUNT ).sbd_balance == ASSET( "2.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( WEBANPICK_NULL_ACCOUNT ).vesting_shares > ASSET( "0.000000 VESTS" ) );
      BOOST_REQUIRE( db->get_account( WEBANPICK_NULL_ACCOUNT ).savings_balance == ASSET( "4.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( WEBANPICK_NULL_ACCOUNT ).savings_sbd_balance == ASSET( "5.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( WEBANPICK_NULL_ACCOUNT ).reward_sbd_balance == ASSET( "1.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( WEBANPICK_NULL_ACCOUNT ).reward_webanpick_balance == ASSET( "1.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( WEBANPICK_NULL_ACCOUNT ).reward_vesting_balance == ASSET( "1.000000 VESTS" ) );
      BOOST_REQUIRE( db->get_account( WEBANPICK_NULL_ACCOUNT ).reward_vesting_webanpick == ASSET( "1.000 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_account( WEBANPICK_NULL_ACCOUNT ).balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( WEBANPICK_NULL_ACCOUNT ).sbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( WEBANPICK_NULL_ACCOUNT ).vesting_shares == ASSET( "0.000000 VESTS" ) );
      BOOST_REQUIRE( db->get_account( WEBANPICK_NULL_ACCOUNT ).savings_balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( WEBANPICK_NULL_ACCOUNT ).savings_sbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( WEBANPICK_NULL_ACCOUNT ).reward_sbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( WEBANPICK_NULL_ACCOUNT ).reward_webanpick_balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( WEBANPICK_NULL_ACCOUNT ).reward_vesting_balance == ASSET( "0.000000 VESTS" ) );
      BOOST_REQUIRE( db->get_account( WEBANPICK_NULL_ACCOUNT ).reward_vesting_webanpick == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).balance == ASSET( "2.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).sbd_balance == ASSET( "3.000 TBD" ) );
   }
   FC_LOG_AND_RETHROW()
