...
BOOST_AUTO_TEST_CASE( smt_limit_order_create_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( db->get_balance( bob_account, WEBANPICK_SYMBOL ).amount.value == bob_balance.amount.value );
      ...
      BOOST_REQUIRE( limit_order->get_market() == std::make_pair( alice_symbol, WEBANPICK_SYMBOL ) );
      BOOST_REQUIRE( db->get_balance( alice_account, alice_symbol ).amount.value == alice_smt_balance.amount.value );
      BOOST_REQUIRE( db->get_balance( alice_account, WEBANPICK_SYMBOL ).amount.value == alice_balance.amount.value );
      validate_database();
...
BOOST_AUTO_TEST_CASE( claim_reward_balance2_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).balance == alice_webanpick + ASSET( "0.000 TESTS" ) );
      ...
      // Legacy symbols
      asset full_webanpick = ASSET( "10.000 TESTS" );
      asset full_sbd = ASSET( "10.000 TBD" );
      op.reward_tokens.push_back( full_sbd );
      op.reward_tokens.push_back( full_webanpick );
      op.reward_tokens.push_back( partial_vests );
      PUSH_OP(op, alice_private_key);
      BOOST_REQUIRE( db->get_account( "alice" ).balance == alice_webanpick + full_webanpick );
