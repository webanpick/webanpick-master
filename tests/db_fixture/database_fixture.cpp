...
#include <webanpick/utilities/tempdir.hpp>

#include <webanpick/chain/webanpick_objects.hpp>
#include <webanpick/chain/history_object.hpp>
#include <webanpick/plugins/account_history/account_history_plugin.hpp>
#include <webanpick/plugins/witness/witness_plugin.hpp>
#include <webanpick/plugins/chain/chain_plugin.hpp>
#include <webanpick/plugins/webserver/webserver_plugin.hpp>
#include <webanpick/plugins/condenser_api/condenser_api_plugin.hpp>
...
uint32_t WEBANPICK_TESTING_GENESIS_TIMESTAMP = 1431700000;

using namespace webanpick::plugins::webserver;
using namespace webanpick::plugins::database_api;
using namespace webanpick::plugins::block_api;
using webanpick::plugins::condenser_api::condenser_api_plugin;
...
clean_database_fixture::clean_database_fixture()
{
   ...
   appbase::app().register_plugin< webanpick::plugins::account_history::account_history_plugin >();
   db_plugin = &appbase::app().register_plugin< webanpick::plugins::debug_node::debug_node_plugin >();
   appbase::app().register_plugin< webanpick::plugins::witness::witness_plugin >();

   db_plugin->logging = false;
   appbase::app().initialize<
      webanpick::plugins::account_history::account_history_plugin,
      webanpick::plugins::debug_node::debug_node_plugin,
      webanpick::plugins::witness::witness_plugin
      >( argc, argv );

   db = &appbase::app().get_plugin< webanpick::plugins::chain::chain_plugin >().db();
   BOOST_REQUIRE( db );
   ...
   generate_block();
   db->set_hardfork( WEBANPICK_BLOCKCHAIN_VERSION.minor() );
   generate_block();

   vest( "initminer", 10000 );

   // Fill up the rest of the required miners
   for( int i = WEBANPICK_NUM_INIT_MINERS; i < WEBANPICK_MAX_WITNESSES; i++ )
   {
      account_create( WEBANPICK_INIT_MINER_NAME + fc::to_string( i ), init_account_pub_key );
      fund( WEBANPICK_INIT_MINER_NAME + fc::to_string( i ), WEBANPICK_MIN_PRODUCER_REWARD.amount.value );
      witness_create( WEBANPICK_INIT_MINER_NAME + fc::to_string( i ), init_account_priv_key, "foo.bar", init_account_pub_key, WEBANPICK_MIN_PRODUCER_REWARD.amount );
   }
...
asset_symbol_type database_fixture::name_to_asset_symbol( const std::string& name, uint8_t decimal_places )
{
   ...
   FC_ASSERT( decimal_places <= WEBANPICK_ASSET_MAX_DECIMALS, "Invalid decimal_places" );
...
void database_fixture::open_database()
{
   if( !data_dir )
   {
      data_dir = fc::temp_directory( webanpick::utilities::temp_directory_path() );
...
{
   skip |= default_skip;
   db_plugin->debug_generate_blocks( webanpick::utilities::key_to_wif( key ), 1, skip, miss_blocks );
}
...
void database_fixture::generate_blocks(fc::time_point_sec timestamp, bool miss_intermediate_blocks)
{
   ...
   BOOST_REQUIRE( ( db->head_block_time() - timestamp ).to_seconds() < WEBANPICK_BLOCK_INTERVAL );
}
   ...
   )
{
   try
   {
      ...
      op.creator = creator;
      op.fee = asset( fee, WEBANPICK_SYMBOL );
      ...
      trx.set_expiration( db->head_block_time() + WEBANPICK_MAX_TIME_UNTIL_EXPIRATION );
...
)
{
   try
   {
      return account_create(
         name,
         WEBANPICK_INIT_MINER_NAME,
         init_account_priv_key,
         std::max( db->get_witness_schedule_object().median_props.account_creation_fee.amount * WEBANPICK_CREATE_ACCOUNT_WITH_WEBANPICK_MODIFIER, share_type( 100 ) ),
         key,
         post_key,
         "" );
   }
   ...
   )
{
   try
   {
      transfer( WEBANPICK_INIT_MINER_NAME, account_name, asset( amount, WEBANPICK_SYMBOL ) );
   ...
   )
{
   try
   {
      db_plugin->debug_update( [=]( database& db)
      {
         ...
         {
            if( amount.symbol == WEBANPICK_SYMBOL )
               a.balance += amount;
         ...
         if( amount.symbol == SBD_SYMBOL )
         {
            ...
            if( median_feed.current_median_history.is_null() )
               ...
               {
                  f.current_median_history = price( asset( 1, SBD_SYMBOL ), asset( 1, WEBANPICK_SYMBOL ) );
               });
         }
   ...
   const asset& amount )
{
   try
   {
      ...
      {
         db->adjust_balance( account_name, -amount );
         db->adjust_balance( account_name, db->to_webanpick( amount ) );
         db->adjust_supply( -amount );
         db->adjust_supply( db->to_webanpick( amount ) );
      }
   } FC_CAPTURE_AND_RETHROW( (account_name)(amount) )
}
...
void database_fixture::vest( const string& from, const share_type& amount )
{
   try
   {
      ...
      op.from = from;
      op.to = "";
      op.amount = asset( amount, WEBANPICK_SYMBOL );
...
void database_fixture::set_witness_props( const flat_map< string, vector< char > >& props )
{
   for( size_t i = 1; i < 8; i++ )
   {
      ...
      op.owner = WEBANPICK_INIT_MINER_NAME + fc::to_string( i );
      op.props = props;
   ...
   generate_blocks( WEBANPICK_BLOCKS_PER_HOUR );
}
...
vector< operation > database_fixture::get_last_operations( uint32_t num_ops )
{
   ...
   while( itr != acc_hist_idx.begin() && ops.size() < num_ops )
   {
      ...
      ops.push_back( fc::raw::unpack_from_vector< webanpick::chain::operation >( serialized_op ) );
   }
   ...
   uint8_t token_decimal_places )
{
   ...
   try
   {
      ...
      tx.set_expiration( this->db->head_block_time() + WEBANPICK_MAX_TIME_UNTIL_EXPIRATION );
...
void push_invalid_operation(const operation& invalid_op, const fc::ecc::private_key& key, database* db)
{
   ...
   WEBANPICK_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), fc::assert_exception );
}
...
void t_smt_database_fixture< T >::create_invalid_smt( const char* control_account_name, const fc::ecc::private_key& key )
{
   ...
   WEBANPICK_REQUIRE_THROW( set_create_op(this->db, &op_precision, control_account_name, WEBANPICK_ASSET_MAX_DECIMALS + 1), fc::assert_exception );
}
...
json_rpc_database_fixture::json_rpc_database_fixture()
{
   ...
   db_plugin = &appbase::app().register_plugin< webanpick::plugins::debug_node::debug_node_plugin >();
   appbase::app().register_plugin< webanpick::plugins::witness::witness_plugin >();
   rpc_plugin = &appbase::app().register_plugin< webanpick::plugins::json_rpc::json_rpc_plugin >();
   appbase::app().register_plugin< webanpick::plugins::block_api::block_api_plugin >();
   appbase::app().register_plugin< webanpick::plugins::database_api::database_api_plugin >();
   appbase::app().register_plugin< webanpick::plugins::condenser_api::condenser_api_plugin >();
   ...
   appbase::app().initialize<
      ...
      webanpick::plugins::block_api::block_api_plugin,
      webanpick::plugins::database_api::database_api_plugin,
      webanpick::plugins::condenser_api::condenser_api_plugin
      >( argc, argv );

   appbase::app().get_plugin< webanpick::plugins::condenser_api::condenser_api_plugin >().plugin_startup();
