...
extern uint32_t ( WEBANPICK_TESTING_GENESIS_TIMESTAMP );

#define PUSH_TX \
   webanpick::chain::test::_push_transaction

#define PUSH_BLOCK \
   webanpick::chain::test::_push_block
...
/*#define WEBANPICK_REQUIRE_THROW( expr, exc_type )          \
{                                                         \
   ...
   if( fc::enable_record_assert_trip )                    \
      std::cout << "WEBANPICK_REQUIRE_THROW begin "        \
         << req_throw_info << std::endl;                  \
   BOOST_REQUIRE_THROW( expr, exc_type );                 \
   if( fc::enable_record_assert_trip )                    \
      std::cout << "WEBANPICK_REQUIRE_THROW end "          \
         << req_throw_info << std::endl;                  \
}*/
...
#define WEBANPICK_CHECK_THROW( expr, exc_type )            \
{                                                         \
   std::string req_throw_info = fc::json::to_string(      \
      fc::mutable_variant_object()                        \
      ("source_file", __FILE__)                           \
   ...
   if( fc::enable_record_assert_trip )                    \
      std::cout << "WEBANPICK_CHECK_THROW begin "          \
         << req_throw_info << std::endl;                  \
   BOOST_CHECK_THROW( expr, exc_type );                   \
   if( fc::enable_record_assert_trip )                    \
      std::cout << "WEBANPICK_CHECK_THROW end "            \
         << req_throw_info << std::endl;                  \
}
...
{ \
   const auto temp = op.field; \
   op.field = value; \
   WEBANPICK_REQUIRE_THROW( op.validate(), exc_type ); \
   op.field = temp; \
}
...
{ \
   ...
   trx.operations.back() = op; \
   op.field = bak; \
   WEBANPICK_REQUIRE_THROW(db.push_transaction(trx, ~0), exc_type); \
}
...
#define ASSET( s ) \
   webanpick::plugins::condenser_api::legacy_asset::from_string( s ).to_asset()
...
{ \
   signed_transaction tx; \
   OP2TX(OP,tx,KEY) \
   WEBANPICK_REQUIRE_THROW( db->push_transaction( tx, 0 ), EXCEPTION ); \
}
...
struct database_fixture {
   ...
   string debug_key = webanpick::utilities::key_to_wif( init_account_priv_key );
...
{
   private:
      webanpick::plugins::json_rpc::json_rpc_plugin* rpc_plugin;
