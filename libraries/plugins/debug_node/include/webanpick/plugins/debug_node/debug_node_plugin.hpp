...
#define WEBANPICK_DEBUG_NODE_PLUGIN_NAME "debug_node"
...
struct debug_generate_blocks_args
{
   ...
   uint32_t                                  count = 0;
   uint32_t                                  skip = webanpick::chain::database::skip_nothing;
...
class debug_node_plugin : public plugin< debug_node_plugin >
{
   public:
      ...
      static const std::string& name() { static std::string name = WEBANPICK_DEBUG_NODE_PLUGIN_NAME; return name; }
      ...
      void debug_update( Lambda&& callback, uint32_t skip = webanpick::chain::database::skip_nothing )
      {
...
FC_REFLECT( webanpick::plugins::debug_node::debug_generate_blocks_args,
            (debug_key)
            (count)
            (skip)
            (miss_blocks)
...
FC_REFLECT( webanpick::plugins::debug_node::debug_generate_blocks_return,
            (blocks)
          )
