...
#define WEBANPICK_JSON_RPC_PLUGIN_NAME "json_rpc"

#define JSON_RPC_REGISTER_API( API_NAME )                                                       \
{                                                                                               \
   webanpick::plugins::json_rpc::detail::register_api_method_visitor vtor( API_NAME );              \
   for_each_api( vtor );                                                                        \
}
...
namespace webanpick { namespace plugins { namespace json_rpc {
...
class json_rpc_plugin : public appbase::plugin< json_rpc_plugin >
{
   public:
      ...
      static const std::string& name() { static std::string name = WEBANPICK_JSON_RPC_PLUGIN_NAME; return name; }
   ...
   class register_api_method_visitor
   {
      public:
         register_api_method_visitor( const std::string& api_name )
            : _api_name( api_name ),
              _json_rpc_plugin( appbase::app().get_plugin< webanpick::plugins::json_rpc::json_rpc_plugin >() )
         {}
      ...
      private:
         ...
         webanpick::plugins::json_rpc::json_rpc_plugin& _json_rpc_plugin;
   };

}

} } } // webanpick::plugins::json_rpc

FC_REFLECT( webanpick::plugins::json_rpc::api_method_signature, (args)(ret) )
