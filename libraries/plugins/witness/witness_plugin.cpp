...
#include <webanpick/utilities/plugin_utilities.hpp>
...
namespace detail {
   ...
   public:
      ...
      void pre_transaction( const webanpick::protocol::signed_transaction& trx );
      ...
      uint32_t _required_witness_participation = 33 * WEBANPICK_1_PERCENT;
      uint32_t _production_skip_flags = chain::database::skip_nothing;

      std::map< webanpick::protocol::public_key_type, fc::ecc::private_key > _private_keys;
      std::set< webanpick::protocol::account_name_type >                     _witnesses;
   ...
   struct comment_options_extension_visitor
   {
      ...
      {
         WEBANPICK_ASSERT( cpb.beneficiaries.size() <= 8,
            plugin_exception,
   ...
   void check_memo( const string& memo, const chain::account_object& account, const account_authority_object& auth )
   {
      ...
      {
         for( auto& key : keys )
            WEBANPICK_ASSERT( key_weight_pair.first != key,  plugin_exception,
      ...
      for( auto& key : keys )
         WEBANPICK_ASSERT( memo_key != key,  plugin_exception,
   ...
   struct operation_visitor
   {
      ...
      void operator()( const comment_operation& o )const
      {
         if( o.parent_author != WEBANPICK_ROOT_POST_PARENT )
         {
            ...
            if( parent != nullptr )
            WEBANPICK_ASSERT( parent->depth < WEBANPICK_SOFT_MAX_COMMENT_DEPTH,
               plugin_exception,
               "Comment is nested ${x} posts deep, maximum depth is ${y}.", ("x",parent->depth)("y",WEBANPICK_SOFT_MAX_COMMENT_DEPTH) );
         }
      }
   ...
   void witness_plugin_impl::pre_transaction( const webanpick::protocol::signed_transaction& trx )
   {
   ...
   { try {
      ...
      if( BOOST_UNLIKELY( reserve_ratio_ptr == nullptr ) )
      {
         _db.create< reserve_ratio_object >( [&]( reserve_ratio_object& r )
         {
            ...
            r.current_reserve_ratio = WEBANPICK_MAX_RESERVE_RATIO * RESERVE_RATIO_PRECISION;
            r.max_virtual_bandwidth = ( static_cast<uint128_t>( WEBANPICK_MAX_BLOCK_SIZE) * WEBANPICK_MAX_RESERVE_RATIO
                                       * WEBANPICK_BANDWIDTH_PRECISION * WEBANPICK_BANDWIDTH_AVERAGE_WINDOW_SECONDS )
                                       / WEBANPICK_BLOCK_INTERVAL;
         });
      }
      else
      {
            ...
            */
            if( _db.head_block_num() % 20 == 0 )
            {
               ...
               else
               {
                  ...
                  if( r.current_reserve_ratio > WEBANPICK_MAX_RESERVE_RATIO * RESERVE_RATIO_PRECISION )
                     r.current_reserve_ratio = WEBANPICK_MAX_RESERVE_RATIO * RESERVE_RATIO_PRECISION;
               }
               ...
               r.max_virtual_bandwidth = ( uint128_t( max_block_size ) * uint128_t( r.current_reserve_ratio )
                                          * uint128_t( WEBANPICK_BANDWIDTH_PRECISION * WEBANPICK_BANDWIDTH_AVERAGE_WINDOW_SECONDS ) )
                                          / ( WEBANPICK_BLOCK_INTERVAL * RESERVE_RATIO_PRECISION );
            }
         });
      }
   } FC_LOG_AND_RETHROW() }
   ...
   void witness_plugin_impl::update_account_bandwidth( const chain::account_object& a, uint32_t trx_size, const bandwidth_type type )
   {
      ...
      if( props.total_vesting_shares.amount > 0 )
      {
         ...
         share_type trx_bandwidth = trx_size * WEBANPICK_BANDWIDTH_PRECISION;
         auto delta_time = ( _db.head_block_time() - band->last_bandwidth_update ).to_seconds();

         if( delta_time > WEBANPICK_BANDWIDTH_AVERAGE_WINDOW_SECONDS )
            new_bandwidth = 0;
         else
            new_bandwidth = ( ( ( WEBANPICK_BANDWIDTH_AVERAGE_WINDOW_SECONDS - delta_time ) * fc::uint128( band->average_bandwidth.value ) )
               / WEBANPICK_BANDWIDTH_AVERAGE_WINDOW_SECONDS ).to_uint64();
         ...
         if( _db.is_producing() )
            WEBANPICK_ASSERT( has_bandwidth,  plugin_exception,
               "Account: ${account} bandwidth limit exceeded. Please wait to transact or power up WEBANPICK.",
               ("account", a.name)
   ...
   {
      if( fc::time_point::now() < fc::time_point(WEBANPICK_GENESIS_TIME) )
      {
         wlog( "waiting until genesis time to produce block: ${t}", ("t",WEBANPICK_GENESIS_TIME) );
   ...
   block_production_condition::block_production_condition_enum witness_plugin_impl::maybe_produce_block(fc::mutable_variant_object& capture)
   {
      ...
      {
         capture("pct", uint32_t(100*uint64_t(prate) / WEBANPICK_1_PERCENT));
      ...
      appbase::app().get_plugin< webanpick::plugins::p2p::p2p_plugin >().broadcast_block( block );
...
{ try {
   ...
   WEBANPICK_LOAD_VALUE_SET( options, "witness", my->_witnesses, webanpick::protocol::account_name_type )
   ...
   if( options.count("private-key") )
   {
      ...
      {
         fc::optional<fc::ecc::private_key> private_key = webanpick::utilities::wif_to_key(wif_key);
   ...
   {
      my->_required_witness_participation = WEBANPICK_1_PERCENT * options.at( "required-participation" ).as< uint32_t >();
   }
   ...
   appbase::app().get_plugin< webanpick::plugins::p2p::p2p_plugin >().set_block_production( true );
} FC_LOG_AND_RETHROW() }

void witness_plugin::plugin_startup()
{ try {
   ilog("witness plugin:  plugin_startup() begin");
   chain::database& d = appbase::app().get_plugin< webanpick::plugins::chain::chain_plugin >().db();
