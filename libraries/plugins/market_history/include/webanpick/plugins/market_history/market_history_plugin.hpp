...
// time.
//
#ifndef WEBANPICK_MARKET_HISTORY_SPACE_ID
#define WEBANPICK_MARKET_HISTORY_SPACE_ID 7
#endif

#ifndef WEBANPICK_MARKET_HISTORY_PLUGIN_NAME
#define WEBANPICK_MARKET_HISTORY_PLUGIN_NAME "market_history"
#endif
...
{
   bucket_object_type        = ( WEBANPICK_MARKET_HISTORY_SPACE_ID << 8 ),
   order_history_object_type = ( WEBANPICK_MARKET_HISTORY_SPACE_ID << 8 ) + 1
};
...
class market_history_plugin : public plugin< market_history_plugin >
{
   public:
      ...
      static const std::string& name() { static std::string name = WEBANPICK_MARKET_HISTORY_PLUGIN_NAME; return name; }
...
struct bucket_object : public object< bucket_object_type, bucket_object >
{
   ...
   bucket_object_details webanpick;
   bucket_object_details non_webanpick;

#ifdef WEBANPICK_ENABLE_SMT
   asset_symbol_type symbol = SBD_SYMBOL;

   price high()const { return asset( non_webanpick.high, symbol ) / asset( webanpick.high, WEBANPICK_SYMBOL ); }
   price low()const { return asset( non_webanpick.low, symbol ) / asset( webanpick.low, WEBANPICK_SYMBOL ); }
#else
   price high()const { return asset( non_webanpick.high, SBD_SYMBOL ) / asset( webanpick.high, WEBANPICK_SYMBOL ); }
   price low()const { return asset( non_webanpick.low, SBD_SYMBOL ) / asset( webanpick.low, WEBANPICK_SYMBOL ); }
#endif
};
...
FC_REFLECT( webanpick::plugins::market_history::bucket_object_details,
            (high)
            (low)
            (open)
            (close)
...
#if defined WEBANPICK_ENABLE_SMT
FC_REFLECT( webanpick::plugins::market_history::bucket_object,
                     (id)
                     (open)(seconds)
                     (webanpick)(symbol)(non_webanpick)
         )
#else
FC_REFLECT( webanpick::plugins::market_history::bucket_object,
                     (id)
                     (open)(seconds)
                     (webanpick)(non_webanpick)
         )
#endif

CHAINBASE_SET_INDEX_TYPE( webanpick::plugins::market_history::bucket_object, webanpick::plugins::market_history::bucket_index )

FC_REFLECT( webanpick::plugins::market_history::order_history_object,
                     (id)
                     (time)
                     (op) )
CHAINBASE_SET_INDEX_TYPE( webanpick::plugins::market_history::order_history_object, webanpick::plugins::market_history::order_history_index )
