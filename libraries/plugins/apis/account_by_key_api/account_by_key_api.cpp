...
#include <webanpick/plugins/account_by_key/account_by_key_objects.hpp>
...
{
   public:
      account_by_key_api_impl() : _db( appbase::app().get_plugin< webanpick::plugins::chain::chain_plugin >().db() ) {}
...
get_key_references_return account_by_key_api_impl::get_key_references( const get_key_references_args& args )const
{
   ...
   {
      std::vector< webanpick::protocol::account_name_type > result;
...
{
   JSON_RPC_REGISTER_API( WEBANPICK_ACCOUNT_BY_KEY_API_PLUGIN_NAME );
}
