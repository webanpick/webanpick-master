...
{
   public:
      tags_api_impl() : _db( appbase::app().get_plugin< webanpick::plugins::chain::chain_plugin >().db() ) {}
      ...
      chain::database& _db;
      std::shared_ptr< webanpick::plugins::follow::follow_api > _follow_api;
};
...
DEFINE_API_IMPL( tags_api_impl, get_discussions_by_promoted )
{
   ...
   auto tidx_itr = tidx.lower_bound( boost::make_tuple( tag, parent, share_type( WEBANPICK_MAX_SHARE_SUPPLY ) )  );
...
{
   JSON_RPC_REGISTER_API( WEBANPICK_TAGS_API_PLUGIN_NAME );
}
...
{
   auto follow_api_plugin = appbase::app().find_plugin< webanpick::plugins::follow::follow_api_plugin >();
