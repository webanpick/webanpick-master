...
#include <webanpick/protocol/transaction_util.hpp>
...
{
   JSON_RPC_REGISTER_API( WEBANPICK_DATABASE_API_PLUGIN_NAME );
}
...
{
   return webanpick::protocol::get_config();
}
...
DEFINE_API_IMPL( database_api_impl, get_order_book )
{
   ...
   auto max_sell = price::max( SBD_SYMBOL, WEBANPICK_SYMBOL );
   auto max_buy = price::max( WEBANPICK_SYMBOL, SBD_SYMBOL );
   ...
   while( sell_itr != end && sell_itr->sell_price.base.symbol == SBD_SYMBOL && result.bids.size() < args.limit )
   {
      ...
      cur.webanpick = ( asset( itr->for_sale, SBD_SYMBOL ) * cur.order_price ).amount;
      cur.created = itr->created;
      result.bids.push_back( cur );
      ++sell_itr;
   }
   while( buy_itr != end && buy_itr->sell_price.base.symbol == WEBANPICK_SYMBOL && result.asks.size() < args.limit )
   {
      auto itr = buy_itr;
      order cur;
      cur.order_price = itr->sell_price;
      ...
      cur.sbd     = ( asset( itr->for_sale, WEBANPICK_SYMBOL ) * cur.order_price ).amount;
      cur.created = itr->created;
...
DEFINE_API_IMPL( database_api_impl, verify_signatures )
{
   ...
   try
   {
      webanpick::protocol::verify_authority< verify_signatures_args >(
         { args },
         sig_keys,
