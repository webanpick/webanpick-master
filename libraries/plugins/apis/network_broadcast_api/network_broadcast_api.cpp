   ...
   class network_broadcast_api_impl
   {
      public:
         network_broadcast_api_impl() :
            _p2p( appbase::app().get_plugin< webanpick::plugins::p2p::p2p_plugin >() ),
            _chain( appbase::app().get_plugin< webanpick::plugins::chain::chain_plugin >() )
         {
         ...
         webanpick::plugins::p2p::p2p_plugin&                      _p2p;
         webanpick::plugins::chain::chain_plugin&                  _chain;
...
{
   JSON_RPC_REGISTER_API( WEBANPICK_NETWORK_BROADCAST_API_PLUGIN_NAME );
}
