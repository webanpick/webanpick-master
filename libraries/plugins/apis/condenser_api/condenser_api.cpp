#include <webanpick/plugins/condenser_api/condenser_api.hpp>
...
#include <webanpick/plugins/account_history_api/account_history_api_plugin.hpp>
#include <webanpick/plugins/account_by_key_api/account_by_key_api_plugin.hpp>
#include <webanpick/plugins/network_broadcast_api/network_broadcast_api_plugin.hpp>
#include <webanpick/plugins/tags_api/tags_api_plugin.hpp>
#include <webanpick/plugins/follow_api/follow_api_plugin.hpp>
...
namespace detail
{
   ...
   DEFINE_API_IMPL( condenser_api_impl, get_version )
   {
      ...
      (
         fc::string( WEBANPICK_BLOCKCHAIN_VERSION ),
         fc::string( webanpick::utilities::git_revision_sha ),
   ...
   {
      FC_ASSERT( false, "condenser_api::get_account_references --- Needs to be refactored for Webanpick." );
   }
   ...
   void condenser_api_impl::set_pending_payout( discussion& d )
   {
      ...
      if( _db.has_hardfork( WEBANPICK_HARDFORK_0_17__774 ) )
         ...
         pot = props.total_reward_fund_webanpick;
      ...
      if( d.parent_author != WEBANPICK_ROOT_POST_PARENT )
...
{
   JSON_RPC_REGISTER_API( WEBANPICK_CONDENSER_API_PLUGIN_NAME );
}
