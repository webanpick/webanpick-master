...
#include <webanpick/utilities/benchmark_dumper.hpp>
...
using fc::flat_map;
using webanpick::chain::block_id_type;
...
const webanpick::chain::database& chain_plugin::db() const { return my->db; }
...
#ifdef IS_TEST_NET
         ("chain-id", bpo::value< std::string >()->default_value( WEBANPICK_CHAIN_ID_NAME ), "chain ID to connect to")
#endif
         ;
}
...
void chain_plugin::plugin_startup()
{
   ...
   webanpick::utilities::benchmark_dumper dumper;

   const auto& abstract_index_cntr = my->db.get_abstract_index_cntr();

   typedef webanpick::utilities::benchmark_dumper::index_memory_details_cntr_t index_memory_details_cntr_t;
   ...
   db_open_args.initial_supply = WEBANPICK_INIT_SUPPLY;
      ...
      const chainbase::database::abstract_index_cntr_t& abstract_index_cntr )
   {
      ...
      {
         typedef webanpick::utilities::benchmark_dumper::database_object_sizeof_cntr_t database_object_sizeof_cntr_t;
      ...
      const webanpick::utilities::benchmark_dumper::measurement& measure =
   ...
   if(my->replay)
   {
      ...
      db_open_args.benchmark = webanpick::chain::database::TBenchmark(my->benchmark_interval, benchmark_lambda);
      last_block_number = my->db.reindex( db_open_args );

      if( my->benchmark_interval > 0 )
      {
         const webanpick::utilities::benchmark_dumper::measurement& total_data = dumper.dump(true, get_indexes_memory_details);
   ...
   {
      db_open_args.benchmark = webanpick::chain::database::TBenchmark(dump_memory_details, benchmark_lambda);
      ...
      catch( const fc::exception& e )
      {
         ...
         catch( webanpick::chain::block_log_exception& )
         {
...
bool chain_plugin::accept_block( const webanpick::chain::signed_block& block, bool currently_syncing, uint32_t skip )
{
...
void chain_plugin::accept_transaction( const webanpick::chain::signed_transaction& trx )
{
   boost::promise< void > prom;
   write_context cxt;
   cxt.req_ptr = &trx;
...
bool chain_plugin::block_is_on_preferred_chain(const webanpick::chain::block_id_type& block_id )
{
   ...
   return db().get_block_id_for_num( webanpick::chain::block_header::num_from_id( block_id ) ) == block_id;
}

void chain_plugin::check_time_in_block( const webanpick::chain::signed_block& block )
{
...
} } } // namespace webanpick::plugis::chain::chain_apis
