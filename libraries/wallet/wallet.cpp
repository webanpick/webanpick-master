...
#include <webanpick/utilities/words.hpp>

#include <webanpick/protocol/base.hpp>
#include <webanpick/wallet/wallet.hpp>
#include <webanpick/wallet/api_documentation.hpp>
#include <webanpick/wallet/reflect_util.hpp>
#include <webanpick/wallet/remote_node_api.hpp>
...
namespace webanpick { namespace wallet {

using webanpick::plugins::condenser_api::legacy_asset;
...
public:
   wallet_api& self;
   wallet_api_impl( wallet_api& s, const wallet_data& initial_data, const webanpick::protocol::chain_id_type& _webanpick_chain_id, fc::api< remote_node_api > rapi )
      : self( s ),
        _remote_api( rapi )
   {
      init_prototype_ops();
      ...
      webanpick_chain_id = _webanpick_chain_id;
   }
   ...
   variant info() const
   {
      ...
      result["post_reward_fund"] = fc::variant(_remote_api->get_reward_fund( WEBANPICK_POST_REWARD_FUND_NAME )).get_object();
      return result;
   }

   variant_object about() const
   {
      string client_version( webanpick::utilities::git_revision_description );
      const size_t pos = client_version.find( '/' );
      if( pos != string::npos && client_version.size() > pos )
         client_version = client_version.substr( pos + 1 );

      fc::mutable_variant_object result;
      result["blockchain_version"]       = WEBANPICK_BLOCKCHAIN_VERSION;
      result["client_version"]           = client_version;
      result["webanpick_revision"]           = webanpick::utilities::git_revision_sha;
      result["webanpick_revision_age"]       = fc::get_approximate_relative_time_string( fc::time_point_sec( webanpick::utilities::git_revision_unix_timestamp ) );
      ...
      try
      {
         ...
         result["server_webanpick_revision"] = v.webanpick_revision;
   ...
   bool import_key(string wif_key)
   {
      ...
      webanpick::chain::public_key_type wif_pub_key = optional_private_key->get_public_key();
   ...
   int find_first_unused_derived_key_index(const fc::ecc::private_key& parent_key)
   {
      ...
      for (int key_index = 0; ; ++key_index)
      {
         ...
         webanpick::chain::public_key_type derived_public_key = derived_private_key.get_public_key();
   ...
   { try {
         ...
         webanpick::chain::public_key_type owner_pubkey = owner_privkey.get_public_key();
         webanpick::chain::public_key_type active_pubkey = active_privkey.get_public_key();
         webanpick::chain::public_key_type memo_pubkey = memo_privkey.get_public_key();
   ...
   {
      FC_ASSERT( tx_expiration_seconds < WEBANPICK_MAX_TIME_UNTIL_EXPIRATION );
   ...
   annotated_signed_transaction sign_transaction(signed_transaction tx, bool broadcast = false)
   {
      ...
      for( const public_key_type& k : minimal_signing_keys )
      {
         ...
         tx.sign( it->second, webanpick_chain_id );
      }
   ...
   std::map<string,std::function<string(fc::variant,const fc::variants&)>> get_result_formatters() const
   {
      ...
      m["list_my_accounts"] = [](variant result, const fc::variants& a ) {
         ...
         asset total_webanpick;
         asset total_vest(0, VESTS_SYMBOL );
         asset total_sbd(0, SBD_SYMBOL );
         for( const auto& a : accounts ) {
            total_webanpick += a.balance.to_asset();
         ...
         out << "-------------------------------------------------------------------------\n";
            out << std::left << std::setw( 17 ) << "TOTAL"
                << std::right << std::setw(18) << legacy_asset::from_asset(total_webanpick).to_string() <<" "
      ...
      m["get_open_orders"] = []( variant result, const fc::variants& a ) {
          ...
          for( const auto& o : orders ) {
             ...
             ss << ' ' << setw( 10 ) << (o.sell_price.base.symbol == WEBANPICK_SYMBOL ? "SELL" : "BUY");
             ss << "\n";
          }
          return ss.str();
      };
         ...
         for( size_t i = 0; i < orders.bids.size() || i < orders.asks.size(); i++ )
         {
            if ( i < orders.bids.size() )
            {
               ...
               ss
                  ...
                  << ' ' << setw( spacing ) << legacy_asset::from_asset( asset( orders.bids[i].webanpick, WEBANPICK_SYMBOL ) ).to_string()
            ...
            if ( i < orders.asks.size() )
            {
               ...
               ss << ' ' << setw( spacing ) << orders.asks[i].real_price
                  << ' ' << setw( spacing ) << legacy_asset::from_asset( asset( orders.asks[i].webanpick, WEBANPICK_SYMBOL ) ).to_string()
   ...
   wallet_data                             _wallet;
   webanpick::protocol::chain_id_type          webanpick_chain_id;
...
} } } // webanpick::wallet::detail



namespace webanpick { namespace wallet {

wallet_api::wallet_api(const wallet_data& initial_data, const webanpick::protocol::chain_id_type& _webanpick_chain_id, fc::api< remote_node_api > rapi)
   : my(new detail::wallet_api_impl(*this, initial_data, _webanpick_chain_id, rapi))
{}
...
brain_key_info wallet_api::suggest_brain_key()const
{
   ...
   {
      fc::bigint choice = entropy % webanpick::words::word_list_size;
      entropy /= webanpick::words::word_list_size;
      if( i > 0 )
         brain_key += " ";
      brain_key += webanpick::words::word_list[ choice.to_int64() ];
   }
...
{ try {
   ...
   op.fee = my->_remote_api->get_chain_properties().account_creation_fee * asset( WEBANPICK_CREATE_ACCOUNT_WITH_WEBANPICK_MODIFIER, WEBANPICK_SYMBOL );
 ...
 *  will be controlable by this wallet.
 */
annotated_signed_transaction wallet_api::create_account_delegated( string creator, asset webanpick_fee, asset delegated_vests, string new_account_name, string json_meta, bool broadcast )
{ try {
   FC_ASSERT( !is_locked() );
   auto owner = suggest_brain_key();
   auto active = suggest_brain_key();
   ...
   import_key( posting.wif_priv_key );
   import_key( memo.wif_priv_key );
   return create_account_with_keys_delegated( creator, webanpick_fee, delegated_vests, new_account_name, json_meta,  owner.pub_key, active.pub_key, posting.pub_key, memo.pub_key, broadcast );
} FC_CAPTURE_AND_RETHROW( (creator)(new_account_name)(json_meta) ) }
...
annotated_signed_transaction wallet_api::claim_reward_balance( string account, asset reward_webanpick, asset reward_sbd, asset reward_vests, bool broadcast )
{
   FC_ASSERT( !is_locked() );
...
annotated_signed_transaction wallet_api::vote( string voter, string author, string permlink, int16_t weight, bool broadcast )
{
   ...
   op.author = author;
   op.permlink = permlink;
   op.weight = weight * WEBANPICK_1_PERCENT;
