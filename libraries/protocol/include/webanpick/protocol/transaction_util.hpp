...
void verify_authority( const vector<AuthContainerType>& auth_containers, const flat_set<public_key_type>& sigs,
                       ...
                       uint32_t max_recursion_depth = WEBANPICK_MAX_SIG_CHECK_DEPTH,
...
{ try {
   ...
   if( required_posting.size() ) {
      ...
      {
         WEBANPICK_ASSERT( s.check_authority(id) ||
   ...
   {
      WEBANPICK_ASSERT( s.check_authority(auth), tx_missing_other_auth, "Missing Authority", ("auth",auth)("sigs",sigs) );
   }
   ...
   {
      WEBANPICK_ASSERT( owner_approvals.find(id) != owner_approvals.end() ||
                       s.check_authority(get_owner(id)),
