...
namespace fc { namespace raw {
   ...
   template< typename Storage >
   void to_variant(   const webanpick::protocol::fixed_string_impl< Storage >& s, variant& v ) { v = std::string( s ); }

   template< typename Storage >
   void from_variant( const variant& v, webanpick::protocol::fixed_string_impl< Storage >& s ) { s = v.as_string(); }
} // fc
