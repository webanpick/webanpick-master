...
#include <webanpick/protocol/types_fwd.hpp>

#define WEBANPICK_ASSET_SYMBOL_PRECISION_BITS    4
#define SMT_MAX_NAI                          99999999
#define SMT_MIN_NAI                          1
#define SMT_MIN_NON_RESERVED_NAI             10000000
#define WEBANPICK_ASSET_SYMBOL_NAI_LENGTH        10
#define WEBANPICK_ASSET_SYMBOL_NAI_STRING_LENGTH ( WEBANPICK_ASSET_SYMBOL_NAI_LENGTH + 2 )

#define WEBANPICK_PRECISION_SBD   (3)
#define WEBANPICK_PRECISION_WEBANPICK (3)
#define WEBANPICK_PRECISION_VESTS (6)
...
#define WEBANPICK_NAI_SBD   (1)
#define WEBANPICK_NAI_WEBANPICK (2)
#define WEBANPICK_NAI_VESTS (3)

#define WEBANPICK_ASSET_NUM_SBD \
  (((SMT_MAX_NAI + WEBANPICK_NAI_SBD)   << WEBANPICK_ASSET_SYMBOL_PRECISION_BITS) | WEBANPICK_PRECISION_SBD)
#define WEBANPICK_ASSET_NUM_WEBANPICK \
  (((SMT_MAX_NAI + WEBANPICK_NAI_WEBANPICK) << WEBANPICK_ASSET_SYMBOL_PRECISION_BITS) | WEBANPICK_PRECISION_WEBANPICK)
#define WEBANPICK_ASSET_NUM_VESTS \
  (((SMT_MAX_NAI + WEBANPICK_NAI_VESTS) << WEBANPICK_ASSET_SYMBOL_PRECISION_BITS) | WEBANPICK_PRECISION_VESTS)

#ifdef IS_TEST_NET

#define VESTS_SYMBOL_U64  (uint64_t('V') | (uint64_t('E') << 8) | (uint64_t('S') << 16) | (uint64_t('T') << 24) | (uint64_t('S') << 32))
#define WEBANPICK_SYMBOL_U64  (uint64_t('T') | (uint64_t('E') << 8) | (uint64_t('S') << 16) | (uint64_t('T') << 24) | (uint64_t('S') << 32))
#define SBD_SYMBOL_U64    (uint64_t('T') | (uint64_t('B') << 8) | (uint64_t('D') << 16))

#else

#define VESTS_SYMBOL_U64  (uint64_t('V') | (uint64_t('E') << 8) | (uint64_t('S') << 16) | (uint64_t('T') << 24) | (uint64_t('S') << 32))
#define WEBANPICK_SYMBOL_U64  (uint64_t('S') | (uint64_t('T') << 8) | (uint64_t('E') << 16) | (uint64_t('E') << 24) | (uint64_t('M') << 32))
...
#define WEBANPICK_SYMBOL_SER  (uint64_t(3) | (WEBANPICK_SYMBOL_U64 << 8)) ///< WEBANPICK|TESTS with 3 digits of precision
#define SBD_SYMBOL_SER    (uint64_t(3) |   (SBD_SYMBOL_U64 << 8)) ///< SBD|TBD with 3 digits of precision

#define WEBANPICK_ASSET_MAX_DECIMALS 12
...
class asset_symbol_type
{
   public:
      ...
      // buf must have space for WEBANPICK_ASSET_SYMBOL_MAX_LENGTH+1
      ...
      {
         char buf[ WEBANPICK_ASSET_SYMBOL_NAI_STRING_LENGTH ];
         to_nai_string( buf );
...
FC_REFLECT(webanpick::protocol::asset_symbol_type, (asset_num))
...
inline void pack( Stream& s, const webanpick::protocol::asset_symbol_type& sym )
{
   switch( sym.space() )
   {
      case webanpick::protocol::asset_symbol_type::legacy_space:
      {
         uint64_t ser = 0;
         switch( sym.asset_num )
         {
            case WEBANPICK_ASSET_NUM_WEBANPICK:
               ser = WEBANPICK_SYMBOL_SER;
               break;
            case WEBANPICK_ASSET_NUM_SBD:
               ser = SBD_SYMBOL_SER;
               break;
            case WEBANPICK_ASSET_NUM_VESTS:
      ...
      case webanpick::protocol::asset_symbol_type::smt_nai_space:
...
inline void unpack( Stream& s, webanpick::protocol::asset_symbol_type& sym )
{
   ...
   switch( ser )
   {
      case WEBANPICK_SYMBOL_SER & 0xFFFFFFFF:
         s.read( ((char*) &ser)+4, 4 );
         FC_ASSERT( ser == WEBANPICK_SYMBOL_SER, "invalid asset bits" );
         sym.asset_num = WEBANPICK_ASSET_NUM_WEBANPICK;
         break;
      case SBD_SYMBOL_SER & 0xFFFFFFFF:
         s.read( ((char*) &ser)+4, 4 );
         FC_ASSERT( ser == SBD_SYMBOL_SER, "invalid asset bits" );
         sym.asset_num = WEBANPICK_ASSET_NUM_SBD;
         break;
      case VESTS_SYMBOL_SER & 0xFFFFFFFF:
         s.read( ((char*) &ser)+4, 4 );
         FC_ASSERT( ser == VESTS_SYMBOL_SER, "invalid asset bits" );
         sym.asset_num = WEBANPICK_ASSET_NUM_VESTS;
         break;
      default:
...
inline void to_variant( const webanpick::protocol::asset_symbol_type& sym, fc::variant& var )
{
   try
   {
...
inline void from_variant( const fc::variant& var, webanpick::protocol::asset_symbol_type& sym )
{
   try
   {
      ...
      sym = webanpick::protocol::asset_symbol_type::from_nai_string( v[1].as< std::string >().c_str(), v[0].as< uint8_t >() );
   } FC_CAPTURE_AND_RETHROW()
}
