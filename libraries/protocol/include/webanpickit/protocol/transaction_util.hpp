#pragma once
#include <webanpickit/protocol/sign_state.hpp>
#include <webanpickit/protocol/exceptions.hpp>

namespace webanpickit { namespace protocol {
...
void verify_authority( const vector<AuthContainerType>& auth_containers, const flat_set<public_key_type>& sigs,
                       ...
                       uint32_t max_recursion_depth = webanpickit_MAX_SIG_CHECK_DEPTH,
...
{ try {
   ...
   if( required_posting.size() ) {
      ...
      {
         webanpickit_ASSERT( s.check_authority(id) ||
   ...
   {
      webanpickit_ASSERT( s.check_authority(auth), tx_missing_other_auth, "Missing Authority", ("auth",auth)("sigs",sigs) );
   }
   ...
   {
      webanpickit_ASSERT( owner_approvals.find(id) != owner_approvals.end() ||
                       s.check_authority(get_owner(id)),
