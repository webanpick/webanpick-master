...
void smt_create_operation::validate()const
{
   ...
   FC_ASSERT( smt_creation_fee.amount <= WEBANPICK_MAX_SHARE_SUPPLY, "Fee must be smaller than WEBANPICK_MAX_SHARE_SUPPLY" );
   FC_ASSERT( is_asset_type( smt_creation_fee, WEBANPICK_SYMBOL ) || is_asset_type( smt_creation_fee, SBD_SYMBOL ), "Fee must be WEBANPICK or SBD" );
   symbol.validate();
...
uint32_t smt_generation_unit::webanpick_unit_sum()const
{
   uint32_t result = 0;
   for(const std::pair< account_name_type, uint16_t >& e : webanpick_unit )
      result += e.second;
   return result;
}
...
{
   FC_ASSERT( webanpick_unit.size() <= SMT_MAX_UNIT_ROUTES );
   for(const std::pair< account_name_type, uint16_t >& e : webanpick_unit )
   {
...
void smt_cap_commitment::validate()const
{
   ...
   FC_ASSERT( upper_bound <= WEBANPICK_MAX_SHARE_SUPPLY );
...
#define SMT_MIN_HARD_CAP_WEBANPICK_UNITS     10000
#define SMT_MIN_SATURATION_WEBANPICK_UNITS    1000
#define SMT_MIN_SOFT_CAP_WEBANPICK_UNITS      1000
...
void smt_capped_generation_policy::validate()const
{
   ...
   FC_ASSERT( soft_cap_percent <= WEBANPICK_100_PERCENT );

   FC_ASSERT( pre_soft_cap_unit.webanpick_unit.size() > 0 );
   FC_ASSERT( pre_soft_cap_unit.token_unit.size() > 0 );

   FC_ASSERT( pre_soft_cap_unit.webanpick_unit.size() <= SMT_MAX_UNIT_COUNT );
   FC_ASSERT( pre_soft_cap_unit.token_unit.size() <= SMT_MAX_UNIT_COUNT );
   FC_ASSERT( post_soft_cap_unit.webanpick_unit.size() <= SMT_MAX_UNIT_COUNT );
   FC_ASSERT( post_soft_cap_unit.token_unit.size() <= SMT_MAX_UNIT_COUNT );

   // TODO : Check account name

   if( soft_cap_percent == WEBANPICK_100_PERCENT )
   {
      FC_ASSERT( post_soft_cap_unit.webanpick_unit.size() == 0 );
   ...
   min_webanpick_units_commitment.validate();
   hard_cap_webanpick_units_commitment.validate();

   FC_ASSERT( min_webanpick_units_commitment.lower_bound <= hard_cap_webanpick_units_commitment.lower_bound );
   FC_ASSERT( min_webanpick_units_commitment.upper_bound <= hard_cap_webanpick_units_commitment.upper_bound );
   ...
   FC_ASSERT( hard_cap_webanpick_units_commitment.lower_bound >= SMT_MIN_HARD_CAP_WEBANPICK_UNITS );

   // we want saturation point to be large enough we don't see quantization effects
   FC_ASSERT( hard_cap_webanpick_units_commitment.lower_bound >= SMT_MIN_SATURATION_WEBANPICK_UNITS * uint64_t( max_unit_ratio ) );

   // this static_assert checks to be sure min_soft_cap / max_soft_cap computation can't overflow uint64_t
   static_assert( uint64_t( WEBANPICK_MAX_SHARE_SUPPLY ) < (std::numeric_limits< uint64_t >::max() / WEBANPICK_100_PERCENT), "Overflow check failed" );
   uint64_t min_soft_cap = (uint64_t( hard_cap_webanpick_units_commitment.lower_bound.value ) * soft_cap_percent) / WEBANPICK_100_PERCENT;
   uint64_t max_soft_cap = (uint64_t( hard_cap_webanpick_units_commitment.upper_bound.value ) * soft_cap_percent) / WEBANPICK_100_PERCENT;

   // we want soft cap to be large enough we don't see quantization effects
   FC_ASSERT( min_soft_cap >= SMT_MIN_SOFT_CAP_WEBANPICK_UNITS );

   // We want to prevent the following from overflowing WEBANPICK_MAX_SHARE_SUPPLY:
   // max_tokens_created = (u1.tt * sc + u2.tt * (hc-sc)) * min_unit_ratio
   // max_webanpick_accepted =  u1.st * sc + u2.st * (hc-sc)

   // hc / max_unit_ratio is the saturation point

   uint128_t sc = max_soft_cap;
   uint128_t hc_sc = hard_cap_webanpick_units_commitment.upper_bound.value - max_soft_cap;

   uint128_t max_tokens_created = (pre_soft_cap_unit.token_unit_sum() * sc + post_soft_cap_unit.token_unit_sum() * hc_sc) * min_unit_ratio;
   uint128_t max_share_supply_u128 = uint128_t( WEBANPICK_MAX_SHARE_SUPPLY );

   FC_ASSERT( max_tokens_created <= max_share_supply_u128 );

   uint128_t max_webanpick_accepted = (pre_soft_cap_unit.webanpick_unit_sum() * sc + post_soft_cap_unit.webanpick_unit_sum() * hc_sc);
   FC_ASSERT( max_webanpick_accepted <= max_share_supply_u128 );
}
...
void smt_setup_emissions_operation::validate()const
{
   ...
   FC_ASSERT( schedule_time > WEBANPICK_GENESIS_TIME );
...
void smt_setup_operation::validate()const
{
   ...
   FC_ASSERT( max_supply <= WEBANPICK_MAX_SHARE_SUPPLY );
   validate_visitor vtor;
   initial_generation_policy.visit( vtor );
   FC_ASSERT( generation_begin_time > WEBANPICK_GENESIS_TIME );
   ...
   // TODO:  Support using WEBANPICK as well
...
void smt_refund_operation::validate()const
{
   ...
   FC_ASSERT( amount.symbol == WEBANPICK_SYMBOL );
}
