...
{
   static_assert( WEBANPICK_ASSET_SYMBOL_NAI_STRING_LENGTH >= 12, "This code will overflow a short buffer" );
   uint32_t x = to_nai();
   buf[11] = '\0';
   buf[10] = ((x%10)+'0');  x /= 10;
   buf[ 9] = ((x%10)+'0');  x /= 10;
...
uint32_t asset_symbol_type::asset_num_from_nai( uint32_t nai, uint8_t decimal_places )
{
   ...
   {
      case WEBANPICK_NAI_WEBANPICK:
         FC_ASSERT( decimal_places == WEBANPICK_PRECISION_WEBANPICK );
         return WEBANPICK_ASSET_NUM_WEBANPICK;
      case WEBANPICK_NAI_SBD:
         FC_ASSERT( decimal_places == WEBANPICK_PRECISION_SBD );
         return WEBANPICK_ASSET_NUM_SBD;
      case WEBANPICK_NAI_VESTS:
         FC_ASSERT( decimal_places == WEBANPICK_PRECISION_VESTS );
         return WEBANPICK_ASSET_NUM_VESTS;
      default:
...
uint32_t asset_symbol_type::to_nai()const
{
   ...
   switch( asset_num )
   {
      case WEBANPICK_ASSET_NUM_WEBANPICK:
         nai_data_digits = WEBANPICK_NAI_WEBANPICK;
         break;
      case WEBANPICK_ASSET_NUM_SBD:
         nai_data_digits = WEBANPICK_NAI_SBD;
         break;
      case WEBANPICK_ASSET_NUM_VESTS:
         nai_data_digits = WEBANPICK_NAI_VESTS;
         break;
      default:
...
asset_symbol_type asset_symbol_type::get_paired_symbol() const
{
   switch( space() )
   {
      case legacy_space:
      {
         ...
         {
            case WEBANPICK_ASSET_NUM_WEBANPICK:
               return from_asset_num( WEBANPICK_ASSET_NUM_VESTS );
            case WEBANPICK_ASSET_NUM_SBD:
               return *this;
            case WEBANPICK_ASSET_NUM_VESTS:
               return from_asset_num( WEBANPICK_ASSET_NUM_WEBANPICK );
            default:
               FC_ASSERT( false, "Unknown asset symbol" );
         }
      }
...
void asset::validate()const
{
   ...
   FC_ASSERT( amount.value <= WEBANPICK_MAX_SATOSHIS );
}
...
DEFINE_PRICE_COMPARISON_OPERATOR( >= )
      ...
      price price::max( asset_symbol_type base, asset_symbol_type quote ) { return asset( share_type(WEBANPICK_MAX_SATOSHIS), base ) / asset( share_type(1), quote); }
      price price::min( asset_symbol_type base, asset_symbol_type quote ) { return asset( 1, base ) / asset( WEBANPICK_MAX_SATOSHIS, quote); }
...
namespace fc {
   void to_variant( const webanpick::protocol::asset& var, fc::variant& vo )
   {
      try
      {
   ...
   void from_variant( const fc::variant& var, webanpick::protocol::asset& vo )
   {
      try
      {
         ...
         vo.symbol = webanpick::protocol::asset_symbol_type::from_nai_string( v[2].as< std::string >().c_str(), v[1].as< uint8_t >() );
      } FC_CAPTURE_AND_RETHROW()
   }
}
