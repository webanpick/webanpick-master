...
webanpick::protocol::transaction_id_type webanpick::protocol::transaction::id() const
{
   auto h = digest();
...
const signature_type& webanpick::protocol::signed_transaction::sign(const private_key_type& key, const chain_id_type& chain_id)
{
...
signature_type webanpick::protocol::signed_transaction::sign(const private_key_type& key, const chain_id_type& chain_id)const
{
   digest_type::encoder enc;
   fc::raw::pack( enc, chain_id );
   fc::raw::pack( enc, *this );
   ...
   ) const
{
   ...
   for( const public_key_type& k : s )
   {
      ...
      try
      {
         webanpick::protocol::verify_authority( operations, result, get_active, get_owner, get_posting, max_recursion );
...
{ try {
   webanpick::protocol::verify_authority( operations, get_signature_keys( chain_id ), get_active, get_owner, get_posting, max_recursion );
