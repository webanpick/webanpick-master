...
namespace webanpick { namespace protocol {
   ...
   void account_create_operation::validate() const
   {
      ...
      FC_ASSERT( is_asset_type( fee, WEBANPICK_SYMBOL ), "Account creation fee must be WEBANPICK" );
      owner.validate();
      active.validate();
      ...
      FC_ASSERT( fee >= asset( 0, WEBANPICK_SYMBOL ), "Account creation fee cannot be negative" );
   }
   ...
   void comment_payout_beneficiaries::validate()const
   {
      ...
      FC_ASSERT( beneficiaries[0].weight <= WEBANPICK_100_PERCENT, "Cannot allocate more than 100% of rewards to one account" );
      sum += beneficiaries[0].weight;
      FC_ASSERT( sum <= WEBANPICK_100_PERCENT, "Cannot allocate more than 100% of rewards to a comment" ); // Have to check incrementally to avoid overflow

      for( size_t i = 1; i < beneficiaries.size(); i++ )
      {
         validate_account_name( beneficiaries[i].account );
         FC_ASSERT( beneficiaries[i].weight <= WEBANPICK_100_PERCENT, "Cannot allocate more than 100% of rewards to one account" );
   ...
   {
      validate_account_name( author );
      FC_ASSERT( percent_webanpick_dollars <= WEBANPICK_100_PERCENT, "Percent cannot exceed 100%" );
   ...
   void vote_operation::validate() const
   {
      ...
      validate_account_name( author );\
      FC_ASSERT( abs(weight) <= WEBANPICK_100_PERCENT, "Weight is not a webanpickit percentage" );
      validate_permlink( permlink );
   }
   ...
   { try {
      ...
      FC_ASSERT( amount.symbol != VESTS_SYMBOL, "transferring of Webanpick Power (STMP) is not allowed." );
      FC_ASSERT( amount.amount > 0, "Cannot transfer a negative amount (aka: stealing)" );
      FC_ASSERT( memo.size() < WEBANPICK_MAX_MEMO_SIZE, "Memo is too large" );
   ...
   {
      validate_account_name( from );
      FC_ASSERT( is_asset_type( amount, WEBANPICK_SYMBOL ), "Amount must be WEBANPICK" );
      if ( to != account_name_type() ) validate_account_name( to );
      FC_ASSERT( amount > asset( 0, WEBANPICK_SYMBOL ), "Must transfer a nonzero amount" );
   }
   ...
   void set_withdraw_vesting_route_operation::validate() const
   {
      ...
      FC_ASSERT( 0 <= percent && percent <= WEBANPICK_100_PERCENT, "Percent must be valid webanpick percent" );
   }
   ...
   void witness_update_operation::validate() const
   {
      ...
      FC_ASSERT( url.size() <= WEBANPICK_MAX_WITNESS_URL_LENGTH, "URL is too long" );

      FC_ASSERT( url.size() > 0, "URL size must be greater than 0" );
      FC_ASSERT( fc::is_utf8( url ), "URL is not valid UTF8" );
      FC_ASSERT( fee >= asset( 0, WEBANPICK_SYMBOL ), "Fee cannot be negative" );
   ...
   void witness_set_properties_operation::validate() const
   {
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( account_creation_fee.symbol == WEBANPICK_SYMBOL, "account_creation_fee must be in WEBANPICK" );
         FC_ASSERT( account_creation_fee.amount >= WEBANPICK_MIN_ACCOUNT_CREATION_FEE , "account_creation_fee smaller than minimum account creation fee" );
      }
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( maximum_block_size >= WEBANPICK_MIN_BLOCK_SIZE_LIMIT, "maximum_block_size smaller than minimum max block size" );
      }
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( sbd_interest_rate <= WEBANPICK_100_PERCENT, "sbd_interest_rate must not exceed 100%" );
      }
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( ( is_asset_type( sbd_exchange_rate.base, SBD_SYMBOL ) && is_asset_type( sbd_exchange_rate.quote, WEBANPICK_SYMBOL ) ),
            "Price feed must be a WEBANPICK/SBD price" );
   ...
   void equihash_pow::create( const block_id_type& recent_block, const account_name_type& account_name, uint32_t nonce )
   {
      ...
      proof = fc::equihash::proof::hash( WEBANPICK_EQUIHASH_N, WEBANPICK_EQUIHASH_K, seed );
   ...
   void equihash_pow::validate() const
   {
      validate_account_name( input.worker_account );
      auto seed = fc::sha256::hash( input );
      FC_ASSERT( proof.n == WEBANPICK_EQUIHASH_N, "proof of work 'n' value is incorrect" );
      FC_ASSERT( proof.k == WEBANPICK_EQUIHASH_K, "proof of work 'k' value is incorrect" );
   ...
   void feed_publish_operation::validate()const
   {
      ...
      FC_ASSERT( ( is_asset_type( exchange_rate.base, WEBANPICK_SYMBOL ) && is_asset_type( exchange_rate.quote, SBD_SYMBOL ) )
         || ( is_asset_type( exchange_rate.base, SBD_SYMBOL ) && is_asset_type( exchange_rate.quote, WEBANPICK_SYMBOL ) ),
         "Price feed must be a WEBANPICK/SBD price" );
   ...
   void limit_order_create_operation::validate()const
   {
      ...
      FC_ASSERT(  ( is_asset_type( amount_to_sell, WEBANPICK_SYMBOL ) && is_asset_type( min_to_receive, SBD_SYMBOL ) )
               || ( is_asset_type( amount_to_sell, SBD_SYMBOL ) && is_asset_type( min_to_receive, WEBANPICK_SYMBOL ) )
               || (
                     amount_to_sell.symbol.space() == asset_symbol_type::smt_nai_space
                     && is_asset_type( min_to_receive, WEBANPICK_SYMBOL )
                  )
               || (
                     is_asset_type( amount_to_sell, WEBANPICK_SYMBOL )
                     && min_to_receive.symbol.space() == asset_symbol_type::smt_nai_space
                  ),
               "Limit order must be for the WEBANPICK:SBD or SMT:(WEBANPICK/SBD) market" );
   ...
   void limit_order_create2_operation::validate()const
   {
      ...
      FC_ASSERT(  ( is_asset_type( amount_to_sell, WEBANPICK_SYMBOL ) && is_asset_type( exchange_rate.quote, SBD_SYMBOL ) )
               || ( is_asset_type( amount_to_sell, SBD_SYMBOL ) && is_asset_type( exchange_rate.quote, WEBANPICK_SYMBOL ) )
               || (
                     amount_to_sell.symbol.space() == asset_symbol_type::smt_nai_space
                     && is_asset_type( exchange_rate.quote, WEBANPICK_SYMBOL )
                  )
               || (
                     is_asset_type( amount_to_sell, WEBANPICK_SYMBOL )
   ...
   {
      validate_account_name( owner );
      /// only allow conversion from SBD to WEBANPICK, allowing the opposite can enable traders to abuse
      /// market fluxuations through converting large quantities without moving the price.
      FC_ASSERT( is_asset_type( amount, SBD_SYMBOL ), "Can only convert SBD to WEBANPICK" );
   ...
   void escrow_transfer_operation::validate()const
   {
      ...
      FC_ASSERT( webanpick_amount.amount >= 0, "webanpick amount cannot be negative" );
      FC_ASSERT( sbd_amount.amount > 0 || webanpick_amount.amount > 0, "escrow must transfer a non-zero amount" );
      FC_ASSERT( from != agent && to != agent, "agent must be a third party" );
      FC_ASSERT( (fee.symbol == WEBANPICK_SYMBOL) || (fee.symbol == SBD_SYMBOL), "fee must be WEBANPICK or SBD" );
      FC_ASSERT( sbd_amount.symbol == SBD_SYMBOL, "sbd amount must contain SBD" );
      FC_ASSERT( webanpick_amount.symbol == WEBANPICK_SYMBOL, "webanpick amount must contain WEBANPICK" );
   ...
   void escrow_release_operation::validate()const
   {
      ...
      FC_ASSERT( sbd_amount.amount >= 0, "sbd amount cannot be negative" );
      FC_ASSERT( webanpick_amount.amount >= 0, "webanpick amount cannot be negative" );
      FC_ASSERT( sbd_amount.amount > 0 || webanpick_amount.amount > 0, "escrow must release a non-zero amount" );
      FC_ASSERT( sbd_amount.symbol == SBD_SYMBOL, "sbd amount must contain SBD" );
      FC_ASSERT( webanpick_amount.symbol == WEBANPICK_SYMBOL, "webanpick amount must contain WEBANPICK" );
   }
   ...
   void transfer_to_savings_operation::validate()const {
      ...
      FC_ASSERT( amount.symbol == WEBANPICK_SYMBOL || amount.symbol == SBD_SYMBOL );
      FC_ASSERT( memo.size() < WEBANPICK_MAX_MEMO_SIZE, "Memo is too large" );
      FC_ASSERT( fc::is_utf8( memo ), "Memo is not UTF8" );
   }
   ...
   {
      validate_account_name( account );
      FC_ASSERT( is_asset_type( reward_webanpick, WEBANPICK_SYMBOL ), "Reward Webanpick must be WEBANPICK" );
      FC_ASSERT( is_asset_type( reward_sbd, SBD_SYMBOL ), "Reward Webanpick must be SBD" );
      FC_ASSERT( is_asset_type( reward_vests, VESTS_SYMBOL ), "Reward Webanpick must be VESTS" );
      FC_ASSERT( reward_webanpick.amount >= 0, "Cannot claim a negative amount" );
      ...
      FC_ASSERT( reward_webanpick.amount > 0 || reward_sbd.amount > 0 || reward_vests.amount > 0, "Must claim something." );
   }
