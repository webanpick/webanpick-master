...
#include <webanpick/chain/block_summary_object.hpp>
#include <webanpick/chain/compound.hpp>
#include <webanpick/chain/custom_operation_interpreter.hpp>
#include <webanpick/chain/database.hpp>
#include <webanpick/chain/database_exceptions.hpp>
#include <webanpick/chain/db_with.hpp>
#include <webanpick/chain/evaluator_registry.hpp>
#include <webanpick/chain/global_property_object.hpp>
#include <webanpick/chain/history_object.hpp>
#include <webanpick/chain/index.hpp>
#include <webanpick/chain/smt_objects.hpp>
#include <webanpick/chain/webanpick_evaluator.hpp>
#include <webanpick/chain/webanpick_objects.hpp>
#include <webanpick/chain/transaction_object.hpp>
#include <webanpick/chain/shared_db_merkle.hpp>
#include <webanpick/chain/operation_notification.hpp>
#include <webanpick/chain/witness_schedule.hpp>

#include <webanpick/chain/util/asset.hpp>
#include <webanpick/chain/util/reward.hpp>
#include <webanpick/chain/util/uint256.hpp>
#include <webanpick/chain/util/reward.hpp>
...
FC_REFLECT( webanpick::chain::object_schema_repr, (space_type)(type) )
FC_REFLECT( webanpick::chain::operation_schema_repr, (id)(type) )
FC_REFLECT( webanpick::chain::db_schema, (types)(object_types)(operation_type)(custom_operation_types) )
...
struct reward_fund_context
{
   ...
   asset       reward_balance = asset( 0, WEBANPICK_SYMBOL );
   share_type  webanpick_awarded = 0;
};
...
{
   set_chain_id( WEBANPICK_CHAIN_ID_NAME );
}
...
uint32_t database::reindex( const open_args& args )
{
   ...
   BOOST_SCOPE_EXIT(this_,&reindex_success,&last_block_number) {
      WEBANPICK_TRY_NOTIFY(this_->_on_reindex_done, reindex_success, last_block_number);
   } BOOST_SCOPE_EXIT_END

   try
   {
      WEBANPICK_TRY_NOTIFY(_on_reindex_start);
      ...
      WEBANPICK_ASSERT( _block_log.head(), block_log_exception, "No blocks in block log. Cannot reindex an empty chain." );
...
{
   return webanpick_chain_id;
}

void database::set_chain_id( const std::string& _chain_id_name )
{
   webanpick_chain_id = generate_chain_id( _chain_id_name );
}
...
{ try {
   if( !has_hardfork( WEBANPICK_HARDFORK_0_6__127 ) )
...
{
   if( has_hardfork( WEBANPICK_HARDFORK_0_17__769 ) || comment.parent_author == WEBANPICK_ROOT_POST_PARENT )
...
{
   return get< reward_fund_object, by_name >( WEBANPICK_POST_REWARD_FUND_NAME );
}
...
uint32_t database::witness_participation_rate()const
{
   ...
   return uint64_t(WEBANPICK_100_PERCENT) * dpo.recent_slots_filled.popcount() / 128;
}
   ...
   )
{
   ...
   auto maximum_block_size = get_dynamic_global_properties().maximum_block_size; //WEBANPICK_MAX_BLOCK_SIZE;
   ...
   if( has_hardfork( WEBANPICK_HARDFORK_0_5__54 ) )
   {
      const auto& witness = get_witness( witness_owner );

      if( witness.running_version != WEBANPICK_BLOCKCHAIN_VERSION )
         pending_block.extensions.insert( block_header_extensions( WEBANPICK_BLOCKCHAIN_VERSION ) );

      const auto& hfp = get_hardfork_property_object();

      if( hfp.current_hardfork_version < WEBANPICK_BLOCKCHAIN_VERSION // Binary is newer hardfork than has been applied
      ...
      else if( hfp.current_hardfork_version == WEBANPICK_BLOCKCHAIN_VERSION // Binary does not know of a new hardfork
         && witness.hardfork_version_vote > WEBANPICK_BLOCKCHAIN_VERSION ) // Voting for hardfork in the future, that we do not know of...
      {
   ...
   {
      FC_ASSERT( fc::raw::pack_size(pending_block) <= WEBANPICK_MAX_BLOCK_SIZE );
   }
...
void database::pop_block()
{
   try
   {
      ...
      WEBANPICK_ASSERT( head_block.valid(), pop_empty_chain, "there are no blocks to pop" );
...
void database::notify_pre_apply_operation( operation_notification& note )
{
   ...
   WEBANPICK_TRY_NOTIFY( pre_apply_operation, note )
}

void database::notify_post_apply_operation( const operation_notification& note )
{
   WEBANPICK_TRY_NOTIFY( post_apply_operation, note )
}
...
{
   WEBANPICK_TRY_NOTIFY( applied_block, block )
}

void database::notify_on_pending_transaction( const signed_transaction& tx )
{
   WEBANPICK_TRY_NOTIFY( on_pending_transaction, tx )
}

void database::notify_on_pre_apply_transaction( const signed_transaction& tx )
{
   WEBANPICK_TRY_NOTIFY( on_pre_apply_transaction, tx )
}

void database::notify_on_applied_transaction( const signed_transaction& tx )
{
   WEBANPICK_TRY_NOTIFY( on_applied_transaction, tx )
}
...
fc::time_point_sec database::get_slot_time(uint32_t slot_num)const
{
   ...
   auto interval = WEBANPICK_BLOCK_INTERVAL;
   const dynamic_global_property_object& dpo = get_dynamic_global_properties();
...
uint32_t database::get_slot_at_time(fc::time_point_sec when)const
{
   ...
   if( when < first_slot_time )
      return 0;
   return (when - first_slot_time).to_seconds() / WEBANPICK_BLOCK_INTERVAL + 1;
}

/**
 *  Converts WEBANPICK into sbd and adds it to to_account while reducing the WEBANPICK supply
 *  by WEBANPICK and increasing the sbd supply by the specified amount.
 */
std::pair< asset, asset > database::create_sbd( const account_object& to_account, asset webanpick, bool to_reward_balance )
{
   std::pair< asset, asset > assets( asset( 0, SBD_SYMBOL ), asset( 0, WEBANPICK_SYMBOL ) );

   try
   {
      if( webanpick.amount == 0 )
         return assets;
      ...
      {
         auto to_sbd = ( gpo.sbd_print_rate * webanpick.amount ) / WEBANPICK_100_PERCENT;
         auto to_webanpick = webanpick.amount - to_sbd;

         auto sbd = asset( to_sbd, WEBANPICK_SYMBOL ) * median_price;

         if( to_reward_balance )
         {
            adjust_reward_balance( to_account, sbd );
            adjust_reward_balance( to_account, asset( to_webanpick, WEBANPICK_SYMBOL ) );
         }
         else
         {
            adjust_balance( to_account, sbd );
            adjust_balance( to_account, asset( to_webanpick, WEBANPICK_SYMBOL ) );
         }

         adjust_supply( asset( -to_sbd, WEBANPICK_SYMBOL ) );
         adjust_supply( sbd );
         assets.first = sbd;
         assets.second = asset( to_webanpick, WEBANPICK_SYMBOL );
      }
      else
      {
         adjust_balance( to_account, webanpick );
         assets.second = webanpick;
      }
   }
   FC_CAPTURE_LOG_AND_RETHROW( (to_account.name)(webanpick) )
 ...
 * @param WEBANPICK - WEBANPICK to be converted to vesting shares
 */
asset database::create_vesting( const account_object& to_account, asset webanpick, bool to_reward_balance )
{
   try
   {
      ...
      /**
       *  The ratio of total_vesting_shares / total_vesting_fund_webanpick should not
       ...
       */
      asset new_vesting = webanpick * ( to_reward_balance ? cprops.get_reward_vesting_share_price() : cprops.get_vesting_share_price() );
      ...
      modify( to_account, [&]( account_object& to )
      {
         if( to_reward_balance )
         {
            ...
            to.reward_vesting_webanpick += webanpick;
         }
         else
      ...
      modify( cprops, [&]( dynamic_global_property_object& props )
      {
         if( to_reward_balance )
         {
            ...
            props.pending_rewarded_vesting_webanpick += webanpick;
         }
         else
         {
            props.total_vesting_fund_webanpick += webanpick;
   ...
   FC_CAPTURE_AND_RETHROW( (to_account.name)(webanpick) )
}
...
uint32_t database::get_pow_summary_target()const
{
   ...
   if( has_hardfork( WEBANPICK_HARDFORK_0_16__551 ) )
...
void database::adjust_proxied_witness_votes( const account_object& a,
                                   const std::array< share_type, WEBANPICK_MAX_PROXY_RECURSION_DEPTH+1 >& delta,
                                   int depth )
{
   if( a.proxy != WEBANPICK_PROXY_TO_SELF_ACCOUNT )
   {
      /// nested proxies are not supported, vote will not propagate
      if( depth >= WEBANPICK_MAX_PROXY_RECURSION_DEPTH )
         return;
      ...
      {
         for( int i = WEBANPICK_MAX_PROXY_RECURSION_DEPTH - depth - 1; i >= 0; --i )
         {
   ...
   else
   {
      ...
      for( int i = WEBANPICK_MAX_PROXY_RECURSION_DEPTH - depth; i >= 0; --i )
         total_delta += delta[i];
...
void database::adjust_witness_vote( const witness_object& witness, share_type delta )
{
   ...
   modify( witness, [&]( witness_object& w )
   {
      ...
      if( has_hardfork( WEBANPICK_HARDFORK_0_2 ) )
         w.virtual_scheduled_time = w.virtual_last_update + (WEBANPICK_VIRTUAL_SCHEDULE_LAP_LENGTH2 - w.virtual_position)/(w.votes.value+1);
      else
         w.virtual_scheduled_time = w.virtual_last_update + (WEBANPICK_VIRTUAL_SCHEDULE_LAP_LENGTH - w.virtual_position)/(w.votes.value+1);

      /** witnesses with a low number of votes could overflow the time field and end up with a scheduled time in the past */
      if( has_hardfork( WEBANPICK_HARDFORK_0_4 ) )
      {
...
void database::clear_witness_votes( const account_object& a )
{
   ...
   if( has_hardfork( WEBANPICK_HARDFORK_0_6__104 ) )
...
{
   if( !has_hardfork( WEBANPICK_HARDFORK_0_14__327 ) ) return;

   const auto& null_account = get_account( WEBANPICK_NULL_ACCOUNT );
   asset total_webanpick( 0, WEBANPICK_SYMBOL );
   asset total_sbd( 0, SBD_SYMBOL );

   if( null_account.balance.amount > 0 )
   {
      total_webanpick += null_account.balance;
      adjust_balance( null_account, -null_account.balance );
   }

   if( null_account.savings_balance.amount > 0 )
   {
      total_webanpick += null_account.savings_balance;
   ...
   {
      const auto& gpo = get_dynamic_global_properties();
      auto converted_webanpick = null_account.vesting_shares * gpo.get_vesting_share_price();

      modify( gpo, [&]( dynamic_global_property_object& g )
      {
         g.total_vesting_shares -= null_account.vesting_shares;
         g.total_vesting_fund_webanpick -= converted_webanpick;
      });
      ...
      total_webanpick += converted_webanpick;
   }

   if( null_account.reward_webanpick_balance.amount > 0 )
   {
      total_webanpick += null_account.reward_webanpick_balance;
      adjust_reward_balance( null_account, -null_account.reward_webanpick_balance );
   }
   ...
   if( null_account.reward_vesting_balance.amount > 0 )
   {
      ...
      total_webanpick += null_account.reward_vesting_webanpick;

      modify( gpo, [&]( dynamic_global_property_object& g )
      {
         g.pending_rewarded_vesting_shares -= null_account.reward_vesting_balance;
         g.pending_rewarded_vesting_webanpick -= null_account.reward_vesting_webanpick;
      });

      modify( null_account, [&]( account_object& a )
      {
         a.reward_vesting_webanpick.amount = 0;
         a.reward_vesting_balance.amount = 0;
      });
   }

   if( total_webanpick.amount > 0 )
      adjust_supply( -total_webanpick );
...
{
   if( head_block_num() >= WEBANPICK_OWNER_AUTH_HISTORY_TRACKING_START_BLOCK_NUM )
   {
...
void database::process_vesting_withdrawals()
{
   ...
   while( current != widx.end() && current->next_vesting_withdrawal <= head_block_time() )
   {
      ...
      */
      ...
      share_type vests_deposited_as_webanpick = 0;
      share_type vests_deposited_as_vests = 0;
      asset total_webanpick_converted = asset( 0, WEBANPICK_SYMBOL );

      // Do two passes, the first for vests, the second for webanpick. Try to maintain as much accuracy for vests as possible.
           ...
           ++itr )
      {
         if( itr->auto_vest )
         {
            share_type to_deposit = ( ( fc::uint128_t ( to_withdraw.value ) * itr->percent ) / WEBANPICK_100_PERCENT ).to_uint64();
           ...
           ++itr )
      {
         if( !itr->auto_vest )
         {
            ...
            share_type to_deposit = ( ( fc::uint128_t ( to_withdraw.value ) * itr->percent ) / WEBANPICK_100_PERCENT ).to_uint64();
            vests_deposited_as_webanpick += to_deposit;
            auto converted_webanpick = asset( to_deposit, VESTS_SYMBOL ) * cprops.get_vesting_share_price();
            total_webanpick_converted += converted_webanpick;

            if( to_deposit > 0 )
            {
               modify( to_account, [&]( account_object& a )
               {
                  a.balance += converted_webanpick;
               });

               modify( cprops, [&]( dynamic_global_property_object& o )
               {
                  o.total_vesting_fund_webanpick -= converted_webanpick;
                  o.total_vesting_shares.amount -= to_deposit;
               });

               push_virtual_operation( fill_vesting_withdraw_operation( from_account.name, to_account.name, asset( to_deposit, VESTS_SYMBOL), converted_webanpick ) );
            }
         }
      }

      share_type to_convert = to_withdraw - vests_deposited_as_webanpick - vests_deposited_as_vests;
      FC_ASSERT( to_convert >= 0, "Deposited more vests than were supposed to be withdrawn" );

      auto converted_webanpick = asset( to_convert, VESTS_SYMBOL ) * cprops.get_vesting_share_price();
      ...
      modify( from_account, [&]( account_object& a )
      {
         ...
         {
            a.next_vesting_withdrawal += fc::seconds( WEBANPICK_VESTING_WITHDRAW_INTERVAL_SECONDS );
         }
      });
      ...
      push_virtual_operation( fill_vesting_withdraw_operation( from_account.name, from_account.name, asset( to_convert, VESTS_SYMBOL ), converted_webanpick ) );
   }
}
      ...
      else if( c.total_vote_weight > 0 )
      {
         ...
         while( itr != cvidx.end() && itr->comment == c.id )
         {
            ...
            if( claim > 0 ) // min_amt is non-zero satoshis
            {
               ...
               auto reward = create_vesting( voter, asset( claim, WEBANPICK_SYMBOL ), has_hardfork( WEBANPICK_HARDFORK_0_17__659 ) );
...
share_type database::cashout_comment_helper( util::comment_reward_context& ctx, const comment_object& comment )
{
   try
   {
      ...
      if( comment.net_rshares > 0 )
      {
         ...
         if( has_hardfork( WEBANPICK_HARDFORK_0_17__774 ) )
         {
         ...
         if( reward_tokens > 0 )
         {
            share_type curation_tokens = ( ( reward_tokens * get_curation_rewards_percent( comment ) ) / WEBANPICK_100_PERCENT ).to_uint64();
            ...
            {
               auto benefactor_tokens = ( author_tokens * b.weight ) / WEBANPICK_100_PERCENT;
               auto vest_created = create_vesting( get_account( b.account ), asset( benefactor_tokens, WEBANPICK_SYMBOL ), has_hardfork( WEBANPICK_HARDFORK_0_17__659 ) );
            ...
            auto sbd_webanpick     = ( author_tokens * comment.percent_webanpick_dollars ) / ( 2 * WEBANPICK_100_PERCENT ) ;
            auto vesting_webanpick = author_tokens - sbd_webanpick;

            const auto& author = get_account( comment.author );
            auto vest_created = create_vesting( author, asset( vesting_webanpick, WEBANPICK_SYMBOL ), has_hardfork( WEBANPICK_HARDFORK_0_17__659 ) );
            auto sbd_payout = create_sbd( author, asset( sbd_webanpick, WEBANPICK_SYMBOL ), has_hardfork( WEBANPICK_HARDFORK_0_17__659 ) );

            adjust_total_payout( comment, sbd_payout.first + to_sbd( sbd_payout.second + asset( vesting_webanpick, WEBANPICK_SYMBOL ) ), to_sbd( asset( curation_tokens, WEBANPICK_SYMBOL ) ), to_sbd( asset( total_beneficiary, WEBANPICK_SYMBOL ) ) );
            ...
            push_virtual_operation( comment_reward_operation( comment.author, to_string( comment.permlink ), to_sbd( asset( claimed_reward, WEBANPICK_SYMBOL ) ) ) );
      ...
      modify( comment, [&]( comment_object& c )
      {
         ...
         */
         ...
         if( has_hardfork( WEBANPICK_HARDFORK_0_17__769 ) )
         {
            c.cashout_time = fc::time_point_sec::maximum();
         }
         else if( c.parent_author == WEBANPICK_ROOT_POST_PARENT )
         {
            if( has_hardfork( WEBANPICK_HARDFORK_0_12__177 ) && c.last_payout == fc::time_point_sec::min() )
               c.cashout_time = head_block_time() + WEBANPICK_SECOND_CASHOUT_WINDOW;
            else
               c.cashout_time = fc::time_point_sec::maximum();
         }
      ...
      while( vote_itr != vote_idx.end() && vote_itr->comment == comment.id )
      {
         ...
         ++vote_itr;
         if( !has_hardfork( WEBANPICK_HARDFORK_0_12__177 ) || calculate_discussion_payout_time( comment ) != fc::time_point_sec::maximum() )
         {
...
void database::process_comment_cashout()
{
   ...
   if( !has_hardfork( WEBANPICK_FIRST_CASHOUT_TIME ) )
      return;

   const auto& gpo = get_dynamic_global_properties();
   util::comment_reward_context ctx;
   ctx.current_webanpick_price = get_feed_history().current_median_history;

   vector< reward_fund_context > funds;
   vector< share_type > webanpick_awarded;
   ...
   for( auto itr = reward_idx.begin(); itr != reward_idx.end(); ++itr )
   {
      ...
      modify( *itr, [&]( reward_fund_object& rfo )
      {
         ...
         if( has_hardfork( WEBANPICK_HARDFORK_0_19__1051 ) )
            decay_time = WEBANPICK_RECENT_RSHARES_DECAY_TIME_HF19;
         else
            decay_time = WEBANPICK_RECENT_RSHARES_DECAY_TIME_HF17;
   ...
   if( has_hardfork( WEBANPICK_HARDFORK_0_17__771 ) )
   {
   ...
   while( current != cidx.end() && current->cashout_time <= head_block_time() )
   {
      if( has_hardfork( WEBANPICK_HARDFORK_0_17__771 ) )
      {
         ...
         ctx.total_reward_fund_webanpick = funds[ fund_id ].reward_balance;
         funds[ fund_id ].webanpick_awarded += cashout_comment_helper( ctx, *current );
      }
      else
      {
         ...
         while( itr != com_by_root.end() && itr->root_comment == current->root_comment )
         {
            ...
            ctx.total_reward_fund_webanpick = gpo.total_reward_fund_webanpick;
            ...
            if( reward > 0 )
            {
               ...
               {
                  p.total_reward_fund_webanpick.amount -= reward;
               });
            }
         }
      }
   ...
   if( funds.size() )
   {
      for( size_t i = 0; i < funds.size(); i++ )
      {
         modify( get< reward_fund_object, by_id >( reward_fund_id_type( i ) ), [&]( reward_fund_object& rfo )
         {
            ...
            rfo.reward_balance -= asset( funds[ i ].webanpick_awarded, WEBANPICK_SYMBOL );
         });
      }
   }
}
...
/**
 *  Overall the network has an inflation rate of 102% of virtual webanpick per year
...
void database::process_funds()
{
   ...
   if( has_hardfork( WEBANPICK_HARDFORK_0_16__551) )
   {
       ...
       */
      int64_t start_inflation_rate = int64_t( WEBANPICK_INFLATION_RATE_START_PERCENT );
      int64_t inflation_rate_adjustment = int64_t( head_block_num() / WEBANPICK_INFLATION_NARROWING_PERIOD );
      int64_t inflation_rate_floor = int64_t( WEBANPICK_INFLATION_RATE_STOP_PERCENT );

      // below subtraction cannot underflow int64_t because inflation_rate_adjustment is <2^32
      int64_t current_inflation_rate = std::max( start_inflation_rate - inflation_rate_adjustment, inflation_rate_floor );

      auto new_webanpick = ( props.virtual_supply.amount * current_inflation_rate ) / ( int64_t( WEBANPICK_100_PERCENT ) * int64_t( WEBANPICK_BLOCKS_PER_YEAR ) );
      auto content_reward = ( new_webanpick * WEBANPICK_CONTENT_REWARD_PERCENT ) / WEBANPICK_100_PERCENT;
      if( has_hardfork( WEBANPICK_HARDFORK_0_17__774 ) )
         content_reward = pay_reward_funds( content_reward ); /// 75% to content creator
      auto vesting_reward = ( new_webanpick * WEBANPICK_VESTING_FUND_PERCENT ) / WEBANPICK_100_PERCENT; /// 15% to vesting fund
      auto witness_reward = new_webanpick - content_reward - vesting_reward; /// Remaining 10% to witness pay

      const auto& cwit = get_witness( props.current_witness );
      witness_reward *= WEBANPICK_MAX_WITNESSES;
      ...
      new_webanpick = content_reward + vesting_reward + witness_reward;

      modify( props, [&]( dynamic_global_property_object& p )
      {
         p.total_vesting_fund_webanpick += asset( vesting_reward, WEBANPICK_SYMBOL );
         if( !has_hardfork( WEBANPICK_HARDFORK_0_17__774 ) )
            p.total_reward_fund_webanpick  += asset( content_reward, WEBANPICK_SYMBOL );
         p.current_supply           += asset( new_webanpick, WEBANPICK_SYMBOL );
         p.virtual_supply           += asset( new_webanpick, WEBANPICK_SYMBOL );
      });

      const auto& producer_reward = create_vesting( get_account( cwit.owner ), asset( witness_reward, WEBANPICK_SYMBOL ) );
   ...
   else
   {
      ...
      if( props.head_block_number < WEBANPICK_START_VESTING_BLOCK )
      ...
      modify( props, [&]( dynamic_global_property_object& p )
      {
          p.total_vesting_fund_webanpick += vesting_reward;
          p.total_reward_fund_webanpick  += content_reward;
...
{
   if( has_hardfork( WEBANPICK_HARDFORK_0_12__178 ) )
      return asset( 0, WEBANPICK_SYMBOL );

   const auto& props = get_dynamic_global_properties();
   static_assert( WEBANPICK_LIQUIDITY_REWARD_PERIOD_SEC == 60*60, "this code assumes a 1 hour time interval" );
   asset percent( protocol::calc_percent_reward_per_hour< WEBANPICK_LIQUIDITY_APR_PERCENT >( props.virtual_supply.amount ), WEBANPICK_SYMBOL );
   return std::max( percent, WEBANPICK_MIN_LIQUIDITY_REWARD );
}

asset database::get_content_reward()const
{
   const auto& props = get_dynamic_global_properties();
   static_assert( WEBANPICK_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   asset percent( protocol::calc_percent_reward_per_block< WEBANPICK_CONTENT_APR_PERCENT >( props.virtual_supply.amount ), WEBANPICK_SYMBOL );
   return std::max( percent, WEBANPICK_MIN_CONTENT_REWARD );
}
...
asset database::get_curation_reward()const
{
   const auto& props = get_dynamic_global_properties();
   static_assert( WEBANPICK_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   asset percent( protocol::calc_percent_reward_per_block< WEBANPICK_CURATE_APR_PERCENT >( props.virtual_supply.amount ), WEBANPICK_SYMBOL);
   return std::max( percent, WEBANPICK_MIN_CURATE_REWARD );
}
...
asset database::get_producer_reward()
{
   const auto& props = get_dynamic_global_properties();
   static_assert( WEBANPICK_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   asset percent( protocol::calc_percent_reward_per_block< WEBANPICK_PRODUCER_APR_PERCENT >( props.virtual_supply.amount ), WEBANPICK_SYMBOL);
   auto pay = std::max( percent, WEBANPICK_MIN_PRODUCER_REWARD );
   const auto& witness_account = get_account( props.current_witness );

   /// pay witness in vesting shares
   if( props.head_block_number >= WEBANPICK_START_MINER_VOTING_BLOCK || (witness_account.vesting_shares.amount.value == 0) ) {
...
#ifndef IS_TEST_NET
   /// 0 block rewards until at least WEBANPICK_MAX_WITNESSES have produced a POW
   if( props.num_pow_witnesses < WEBANPICK_MAX_WITNESSES && props.head_block_number < WEBANPICK_START_VESTING_BLOCK )
      return asset( 0, WEBANPICK_SYMBOL );
#endif

   static_assert( WEBANPICK_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   static_assert( WEBANPICK_MAX_WITNESSES == 21, "this code assumes 21 per round" );
   asset percent( calc_percent_reward_per_round< WEBANPICK_POW_APR_PERCENT >( props.virtual_supply.amount ), WEBANPICK_SYMBOL);
   return std::max( percent, WEBANPICK_MIN_POW_REWARD );
}
...
void database::pay_liquidity_reward()
{
   ...
   if( (head_block_num() % WEBANPICK_LIQUIDITY_REWARD_BLOCKS) == 0 )
   {
...
uint16_t database::get_curation_rewards_percent( const comment_object& c ) const
{
   ...
   else if( has_hardfork( WEBANPICK_HARDFORK_0_8__116 ) )
      return WEBANPICK_1_PERCENT * 25;
   else
      return WEBANPICK_1_PERCENT * 50;
}
...
share_type database::pay_reward_funds( share_type reward )
{
   ...
   for( auto itr = reward_idx.begin(); itr != reward_idx.end(); ++itr )
   {
      ...
      auto r = ( reward * itr->percent_content_rewards ) / WEBANPICK_100_PERCENT;

      modify( *itr, [&]( reward_fund_object& rfo )
      {
         rfo.reward_balance += asset( r, WEBANPICK_SYMBOL );
      });

      used_rewards += r;

      // Sanity check to ensure we aren't printing more WEBANPICK than has been allocated through inflation
 ...
 *  the head block time and then converts them to/from webanpick/sbd at the
...
void database::process_conversions()
{
   ...
   asset net_webanpick( 0, WEBANPICK_SYMBOL );
   ...
   modify( props, [&]( dynamic_global_property_object& p )
   {
       p.current_supply += net_webanpick;
       p.current_sbd_supply -= net_sbd;
       p.virtual_supply += net_webanpick;
       p.virtual_supply -= net_sbd * get_feed_history().current_median_history;
   } );
}

asset database::to_sbd( const asset& webanpick )const
{
   return util::to_sbd( get_feed_history().current_median_history, webanpick );
}

asset database::to_webanpick( const asset& sbd )const
{
   return util::to_webanpick( get_feed_history().current_median_history, sbd );
}
...
void database::account_recovery_processing()
{
   ...
   while( hist != hist_idx.end() && time_point_sec( hist->last_valid_time + WEBANPICK_OWNER_AUTH_RECOVERY_PERIOD ) < head_block_time() )
   {
      remove( *hist );
      hist = hist_idx.begin();
   }
...
void database::expire_escrow_ratification()
{
   ...
   while( escrow_itr != escrow_idx.end() && !escrow_itr->is_approved() && escrow_itr->ratification_deadline <= head_block_time() )
   {
      ...
      adjust_balance( old_escrow.from, old_escrow.webanpick_balance );
...
void database::process_decline_voting_rights()
{
   ...
   while( itr != request_idx.end() && itr->effective_date <= head_block_time() )
   {
      ...
      std::array<share_type, WEBANPICK_MAX_PROXY_RECURSION_DEPTH+1> delta;
      delta[0] = -account.vesting_shares.amount;
      for( int i = 0; i < WEBANPICK_MAX_PROXY_RECURSION_DEPTH; ++i )
      ...
      {
         a.can_vote = false;
         a.proxy = WEBANPICK_PROXY_TO_SELF_ACCOUNT;
      });
...
void database::init_genesis( uint64_t init_supply )
{
   try
   {
      ...
      public_key_type      init_public_key(WEBANPICK_INIT_PUBLIC_KEY);

      create< account_object >( [&]( account_object& a )
      {
         a.name = WEBANPICK_MINER_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = WEBANPICK_MINER_ACCOUNT;
      ...
      create< account_object >( [&]( account_object& a )
      {
         a.name = WEBANPICK_NULL_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = WEBANPICK_NULL_ACCOUNT;
         auth.owner.weight_threshold = 1;
         auth.active.weight_threshold = 1;
      });
      ...
      create< account_object >( [&]( account_object& a )
      {
         a.name = WEBANPICK_TEMP_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = WEBANPICK_TEMP_ACCOUNT;
         auth.owner.weight_threshold = 0;
         auth.active.weight_threshold = 0;
      });

      for( int i = 0; i < WEBANPICK_NUM_INIT_MINERS; ++i )
      {
         create< account_object >( [&]( account_object& a )
         {
            a.name = WEBANPICK_INIT_MINER_NAME + ( i ? fc::to_string( i ) : std::string() );
            a.memo_key = init_public_key;
            a.balance  = asset( i ? 0 : init_supply, WEBANPICK_SYMBOL );
         } );

         create< account_authority_object >( [&]( account_authority_object& auth )
         {
            auth.account = WEBANPICK_INIT_MINER_NAME + ( i ? fc::to_string( i ) : std::string() );
         ...
         {
            w.owner        = WEBANPICK_INIT_MINER_NAME + ( i ? fc::to_string(i) : std::string() );
      ...
      {
         p.current_witness = WEBANPICK_INIT_MINER_NAME;
         p.time = WEBANPICK_GENESIS_TIME;
         p.recent_slots_filled = fc::uint128::max_value();
         p.participation_count = 128;
         p.current_supply = asset( init_supply, WEBANPICK_SYMBOL );
         p.virtual_supply = p.current_supply;
         p.maximum_block_size = WEBANPICK_MAX_BLOCK_SIZE;
      } );
      ...
      {
         hpo.processed_hardforks.push_back( WEBANPICK_GENESIS_TIME );
      } );

      // Create witness scheduler
      create< witness_schedule_object >( [&]( witness_schedule_object& wso )
      {
         wso.current_shuffled_witnesses[0] = WEBANPICK_INIT_MINER_NAME;
      } );
   }
   FC_CAPTURE_AND_RETHROW()
}
...
void database::notify_changed_objects()
{
   try
   {
      ...
      get_changed_ids( ids );
      WEBANPICK_TRY_NOTIFY( changed_objects, ids )*/
      /*
      if( _undo_db.enabled() )
      {
         ...
         WEBANPICK_TRY_NOTIFY( changed_objects, changed_ids )
      }
      */
   }
   FC_CAPTURE_AND_RETHROW()
...
void database::check_free_memory( bool force_print, uint32_t current_block_num )
{
   ...
   if( BOOST_UNLIKELY( _shared_file_full_threshold != 0 && _shared_file_scale_rate != 0 && free_mem < ( ( uint128_t( WEBANPICK_100_PERCENT - _shared_file_full_threshold ) * max_mem ) / WEBANPICK_100_PERCENT ).to_uint64() ) )
   {
      uint64_t new_max = ( uint128_t( max_mem * _shared_file_scale_rate ) / WEBANPICK_100_PERCENT ).to_uint64() + max_mem;
...
{ try {
   ...
   if( BOOST_UNLIKELY( next_block_num == 1 ) )
   {
      ...
      uint32_t n;
      for( n=0; n<WEBANPICK_NUM_HARDFORKS; n++ )
      {
   ...
   if( has_hardfork( WEBANPICK_HARDFORK_0_12 ) )
   {
   ...
   if( block_size < WEBANPICK_MIN_BLOCK_SIZE )
   {
      elog( "Block size is too small",
         ("next_block_num",next_block_num)("block_size", block_size)("min",WEBANPICK_MIN_BLOCK_SIZE)
      );
   }
   ...
   if( has_hardfork( WEBANPICK_HARDFORK_0_5__54 ) ) // Cannot remove after hardfork
   {
...
try {
   if( (head_block_num() % WEBANPICK_FEED_INTERVAL_BLOCKS) != 0 )
      return;
   ...
   for( int i = 0; i < wso.num_scheduled_witnesses; i++ )
   {
      ...
      if( has_hardfork( WEBANPICK_HARDFORK_0_19__822 ) )
      {
         if( now < wit.last_sbd_exchange_update + WEBANPICK_MAX_FEED_AGE_SECONDS
            && !wit.sbd_exchange_rate.is_null() )
         {
            feeds.push_back( wit.sbd_exchange_rate );
         }
      }
      else if( wit.last_sbd_exchange_update < now + WEBANPICK_MAX_FEED_AGE_SECONDS &&
          !wit.sbd_exchange_rate.is_null() )
      {
         feeds.push_back( wit.sbd_exchange_rate );
      }
   ...
   if( feeds.size() >= WEBANPICK_MIN_FEEDS )
   {
      ...
      modify( get_feed_history(), [&]( feed_history_object& fho )
      {
         ...
         size_t webanpick_feed_history_window = WEBANPICK_FEED_HISTORY_WINDOW_PRE_HF_16;
         if( has_hardfork( WEBANPICK_HARDFORK_0_16__551) )
            webanpick_feed_history_window = WEBANPICK_FEED_HISTORY_WINDOW;

         if( fho.price_history.size() > webanpick_feed_history_window )
         ...
         if( fho.price_history.size() )
         {
            ...
            if( skip_price_feed_limit_check )
               return;
#endif
            if( has_hardfork( WEBANPICK_HARDFORK_0_14__230 ) )
            {
               const auto& gpo = get_dynamic_global_properties();
...
{ try {
   ...
   if( !(skip & (skip_transaction_signatures | skip_authority_check) ) )
   {
      ...
      try
      {
         trx.verify_authority( chain_id, get_active, get_owner, get_posting, WEBANPICK_MAX_SIG_CHECK_DEPTH );
      }
   ...
   if( BOOST_LIKELY(head_block_num() > 0) )
   {
      if( !(skip & skip_tapos_check) )
      {
         ...
         WEBANPICK_ASSERT( trx.ref_block_prefix == tapos_block_summary.block_id._hash[1], transaction_tapos_exception,
      ...
      WEBANPICK_ASSERT( trx.expiration <= now + fc::seconds(WEBANPICK_MAX_TIME_UNTIL_EXPIRATION), transaction_expiration_exception,
                  "", ("trx.expiration",trx.expiration)("now",now)("max_til_exp",WEBANPICK_MAX_TIME_UNTIL_EXPIRATION));
      if( has_hardfork( WEBANPICK_HARDFORK_0_9 ) ) // Simple solution to pending trx bug when now == trx.expiration
         WEBANPICK_ASSERT( now < trx.expiration, transaction_expiration_exception, "", ("now",now)("trx.exp",trx.expiration) );
      WEBANPICK_ASSERT( now <= trx.expiration, transaction_expiration_exception, "", ("now",now)("trx.exp",trx.expiration) );
   }
...
{ try {
   ...
   if( head_block_time() != fc::time_point_sec() )
   {
      ...
      for( uint32_t i = 0; i < missed_blocks; ++i )
      {
         ...
         if(  witness_missed.owner != b.witness )
         {
            ...
            {
               w.total_missed++;
               if( has_hardfork( WEBANPICK_HARDFORK_0_14__278 ) )
               {
                  if( head_block_num() - w.last_confirmed_block_num  > WEBANPICK_BLOCKS_PER_DAY )
                  {
   ...
   {
      WEBANPICK_ASSERT( _dgp.head_block_number - _dgp.last_irreversible_block_num  < WEBANPICK_MAX_UNDO_HISTORY, undo_database_exception,
                 "The database does not have enough undo history to support a blockchain with so many missed blocks. "
                 "Please add a checkpoint if you would like to continue applying blocks beyond this point.",
                 ("last_irreversible_block_num",_dgp.last_irreversible_block_num)("head", _dgp.head_block_number)
                 ("max_undo",WEBANPICK_MAX_UNDO_HISTORY) );
   }
} FC_CAPTURE_AND_RETHROW() }
...
{ try {
   modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& dgp )
   {
      dgp.virtual_supply = dgp.current_supply
         + ( get_feed_history().current_median_history.is_null() ? asset( 0, WEBANPICK_SYMBOL ) : dgp.current_sbd_supply * get_feed_history().current_median_history );

      auto median_price = get_feed_history().current_median_history;

      if( !median_price.is_null() && has_hardfork( WEBANPICK_HARDFORK_0_14__230 ) )
      {
         auto percent_sbd = uint16_t( ( ( fc::uint128_t( ( dgp.current_sbd_supply * get_feed_history().current_median_history ).amount.value ) * WEBANPICK_100_PERCENT )
            / dgp.virtual_supply.amount.value ).to_uint64() );

         if( percent_sbd <= WEBANPICK_SBD_START_PERCENT )
            dgp.sbd_print_rate = WEBANPICK_100_PERCENT;
         else if( percent_sbd >= WEBANPICK_SBD_STOP_PERCENT )
            dgp.sbd_print_rate = 0;
         else
            dgp.sbd_print_rate = ( ( WEBANPICK_SBD_STOP_PERCENT - percent_sbd ) * WEBANPICK_100_PERCENT ) / ( WEBANPICK_SBD_STOP_PERCENT - WEBANPICK_SBD_START_PERCENT );
      }
   });
} FC_CAPTURE_AND_RETHROW() }
...
{ try {
    ...
    *
    */
   if( head_block_num() < WEBANPICK_START_MINER_VOTING_BLOCK )
   {
      modify( dpo, [&]( dynamic_global_property_object& _dpo )
      {
         if ( head_block_num() > WEBANPICK_MAX_WITNESSES )
            _dpo.last_irreversible_block_num = head_block_num() - WEBANPICK_MAX_WITNESSES;
      } );
   }
   else
   {
      ...
      static_assert( WEBANPICK_IRREVERSIBLE_THRESHOLD > 0, "irreversible threshold must be nonzero" );

      // 1 1 1 2 2 2 2 2 2 2 -> 2     .7*10 = 7
      // 1 1 1 1 1 1 1 2 2 2 -> 1
      // 3 3 3 3 3 3 3 3 3 3 -> 3

      size_t offset = ((WEBANPICK_100_PERCENT - WEBANPICK_IRREVERSIBLE_THRESHOLD) * wit_objs.size() / WEBANPICK_100_PERCENT);
...
{
   bool has_hf_20__1815 = has_hardfork( WEBANPICK_HARDFORK_0_20__1815 );

#pragma message( "TODO:  Remove if(), do assert unconditionally after HF20 occurs" )
   if( has_hf_20__1815 )
   {
      WEBANPICK_ASSERT( new_order.sell_price.quote.symbol == old_order.sell_price.base.symbol,
      ...
      WEBANPICK_ASSERT( new_order.sell_price.base.symbol  == old_order.sell_price.quote.symbol,
         order_match_exception, "error matching orders: ${new_order} ${old_order} ${match_price}",
         ("new_order", new_order)("old_order", old_order)("match_price", match_price) );
      WEBANPICK_ASSERT( new_order.for_sale > 0 && old_order.for_sale > 0,
         order_match_exception, "error matching orders: ${new_order} ${old_order} ${match_price}",
         ("new_order", new_order)("old_order", old_order)("match_price", match_price) );
      WEBANPICK_ASSERT( match_price.quote.symbol == new_order.sell_price.base.symbol,
         order_match_exception, "error matching orders: ${new_order} ${old_order} ${match_price}",
         ("new_order", new_order)("old_order", old_order)("match_price", match_price) );
      WEBANPICK_ASSERT( match_price.base.symbol == old_order.sell_price.base.symbol,
         order_match_exception, "error matching orders: ${new_order} ${old_order} ${match_price}",
         ("new_order", new_order)("old_order", old_order)("match_price", match_price) );
   }
...
#pragma message( "TODO:  Remove if(), do assert unconditionally after HF20 occurs" )
   if( has_hf_20__1815 )
   {
      WEBANPICK_ASSERT( new_order_pays == new_order.amount_for_sale() ||
   ...
   if( !has_hardfork( WEBANPICK_HARDFORK_0_12__178 ) &&
       ( (age >= WEBANPICK_MIN_LIQUIDITY_REWARD_PERIOD_SEC && !has_hardfork( WEBANPICK_HARDFORK_0_10__149)) ||
       (age >= WEBANPICK_MIN_LIQUIDITY_REWARD_PERIOD_SEC_HF10 && has_hardfork( WEBANPICK_HARDFORK_0_10__149) ) ) )
   {
      if( old_order_receives.symbol == WEBANPICK_SYMBOL )
      {
...
void database::adjust_liquidity_reward( const account_object& owner, const asset& volume, bool is_sdb )
{
   ...
   if( itr != ridx.end() )
   {
      ...
      {
         if( head_block_time() - r.last_update >= WEBANPICK_LIQUIDITY_TIMEOUT_SEC )
         {
            r.sbd_volume = 0;
            r.webanpick_volume = 0;
            r.weight = 0;
         }
         ...
         if( is_sdb )
            ...
            r.webanpick_volume += volume.amount.value;

         r.update_weight( has_hardfork( WEBANPICK_HARDFORK_0_10__141 ) );
   ...
   else
   {
      create<liquidity_reward_balance_object>( [&](liquidity_reward_balance_object& r )
      {
         ...
         r.update_weight( has_hardfork( WEBANPICK_HARDFORK_0_9__141 ) );
         r.last_update = head_block_time();
      } );
   }
}
...
bool database::fill_order( const limit_order_object& order, const asset& pays, const asset& receives )
{
   try
   {
      WEBANPICK_ASSERT( order.amount_for_sale().symbol == pays.symbol,
         order_fill_exception, "error filling orders: ${order} ${pays} ${receives}",
         ("order", order)("pays", pays)("receives", receives) );
      WEBANPICK_ASSERT( pays.symbol != receives.symbol,
         order_fill_exception, "error filling orders: ${order} ${pays} ${receives}",
         ("order", order)("pays", pays)("receives", receives) );
      ...
      {
#pragma message( "TODO:  Remove if(), do assert unconditionally after HF20 occurs" )
         if( has_hardfork( WEBANPICK_HARDFORK_0_20__1815 ) )
         {
            WEBANPICK_ASSERT( pays < order.amount_for_sale(),
              order_fill_exception, "error filling orders: ${order} ${pays} ${receives}",
              ("order", order)("pays", pays)("receives", receives) );
         }
...
void database::modify_balance( const account_object& a, const asset& delta, bool check_balance )
{
   modify( a, [&]( account_object& acnt )
   {
      ...
      {
         case WEBANPICK_ASSET_NUM_WEBANPICK:
            acnt.balance += delta;
            if( check_balance )
            {
               FC_ASSERT( acnt.balance.amount.value >= 0, "Insufficient WEBANPICK funds" );
            }
            break;
         case WEBANPICK_ASSET_NUM_SBD:
            if( a.sbd_seconds_last_update != head_block_time() )
            {
               ...
               if( acnt.sbd_seconds > 0 &&
                   (acnt.sbd_seconds_last_update - acnt.sbd_last_interest_payment).to_seconds() > WEBANPICK_SBD_INTEREST_COMPOUND_INTERVAL_SEC )
               {
                  auto interest = acnt.sbd_seconds / WEBANPICK_SECONDS_PER_YEAR;
                  interest *= get_dynamic_global_properties().sbd_interest_rate;
                  interest /= WEBANPICK_100_PERCENT;
...
void database::modify_reward_balance( const account_object& a, const asset& delta, bool check_balance )
{
   modify( a, [&]( account_object& acnt )
   {
      ...
      {
         case WEBANPICK_ASSET_NUM_WEBANPICK:
            acnt.reward_webanpick_balance += delta;
            if( check_balance )
            {
               FC_ASSERT( acnt.reward_webanpick_balance.amount.value >= 0, "Insufficient reward WEBANPICK funds" );
            }
            break;
         case WEBANPICK_ASSET_NUM_SBD:
...
{
   bool check_balance = has_hardfork( WEBANPICK_HARDFORK_0_20__1811 );
   ...
   modify( a, [&]( account_object& acnt )
   {
      switch( delta.symbol.asset_num )
      {
         case WEBANPICK_ASSET_NUM_WEBANPICK:
            ...
            if( check_balance )
            {
               FC_ASSERT( acnt.savings_balance.amount.value >= 0, "Insufficient savings WEBANPICK funds" );
            }
            break;
         case WEBANPICK_ASSET_NUM_SBD:
            if( a.savings_sbd_seconds_last_update != head_block_time() )
            {
               ...
               if( acnt.savings_sbd_seconds > 0 &&
                   (acnt.savings_sbd_seconds_last_update - acnt.savings_sbd_last_interest_payment).to_seconds() > WEBANPICK_SBD_INTEREST_COMPOUND_INTERVAL_SEC )
               {
                  auto interest = acnt.savings_sbd_seconds / WEBANPICK_SECONDS_PER_YEAR;
                  interest *= get_dynamic_global_properties().sbd_interest_rate;
                  interest /= WEBANPICK_100_PERCENT;
                  asset interest_paid(interest.to_uint64(), SBD_SYMBOL);
...
void database::adjust_supply( const asset& delta, bool adjust_vesting )
{
   ...
   bool check_supply = has_hardfork( WEBANPICK_HARDFORK_0_20__1811 );

   const auto& props = get_dynamic_global_properties();
   if( props.head_block_number < WEBANPICK_BLOCKS_PER_DAY*7 )
   ...
   modify( props, [&]( dynamic_global_property_object& props )
   {
      switch( delta.symbol.asset_num )
      {
         case WEBANPICK_ASSET_NUM_WEBANPICK:
         {
            asset new_vesting( (adjust_vesting && delta.amount > 0) ? delta.amount * 9 : 0, WEBANPICK_SYMBOL );
            props.current_supply += delta + new_vesting;
            props.virtual_supply += delta + new_vesting;
            props.total_vesting_fund_webanpick += new_vesting;
            if( check_supply )
            {
...
{
   _hardfork_times[ 0 ] = fc::time_point_sec( WEBANPICK_GENESIS_TIME );
   _hardfork_versions[ 0 ] = hardfork_version( 0, 0 );
   FC_ASSERT( WEBANPICK_HARDFORK_0_1 == 1, "Invalid hardfork configuration" );
   _hardfork_times[ WEBANPICK_HARDFORK_0_1 ] = fc::time_point_sec( WEBANPICK_HARDFORK_0_1_TIME );
   _hardfork_versions[ WEBANPICK_HARDFORK_0_1 ] = WEBANPICK_HARDFORK_0_1_VERSION;
   FC_ASSERT( WEBANPICK_HARDFORK_0_2 == 2, "Invlaid hardfork configuration" );
   _hardfork_times[ WEBANPICK_HARDFORK_0_2 ] = fc::time_point_sec( WEBANPICK_HARDFORK_0_2_TIME );
   _hardfork_versions[ WEBANPICK_HARDFORK_0_2 ] = WEBANPICK_HARDFORK_0_2_VERSION;
   FC_ASSERT( WEBANPICK_HARDFORK_0_3 == 3, "Invalid hardfork configuration" );
   _hardfork_times[ WEBANPICK_HARDFORK_0_3 ] = fc::time_point_sec( WEBANPICK_HARDFORK_0_3_TIME );
   _hardfork_versions[ WEBANPICK_HARDFORK_0_3 ] = WEBANPICK_HARDFORK_0_3_VERSION;
   FC_ASSERT( WEBANPICK_HARDFORK_0_4 == 4, "Invalid hardfork configuration" );
   _hardfork_times[ WEBANPICK_HARDFORK_0_4 ] = fc::time_point_sec( WEBANPICK_HARDFORK_0_4_TIME );
   _hardfork_versions[ WEBANPICK_HARDFORK_0_4 ] = WEBANPICK_HARDFORK_0_4_VERSION;
   FC_ASSERT( WEBANPICK_HARDFORK_0_5 == 5, "Invalid hardfork configuration" );
   _hardfork_times[ WEBANPICK_HARDFORK_0_5 ] = fc::time_point_sec( WEBANPICK_HARDFORK_0_5_TIME );
   _hardfork_versions[ WEBANPICK_HARDFORK_0_5 ] = WEBANPICK_HARDFORK_0_5_VERSION;
   FC_ASSERT( WEBANPICK_HARDFORK_0_6 == 6, "Invalid hardfork configuration" );
   _hardfork_times[ WEBANPICK_HARDFORK_0_6 ] = fc::time_point_sec( WEBANPICK_HARDFORK_0_6_TIME );
   _hardfork_versions[ WEBANPICK_HARDFORK_0_6 ] = WEBANPICK_HARDFORK_0_6_VERSION;
   FC_ASSERT( WEBANPICK_HARDFORK_0_7 == 7, "Invalid hardfork configuration" );
   _hardfork_times[ WEBANPICK_HARDFORK_0_7 ] = fc::time_point_sec( WEBANPICK_HARDFORK_0_7_TIME );
   _hardfork_versions[ WEBANPICK_HARDFORK_0_7 ] = WEBANPICK_HARDFORK_0_7_VERSION;
   FC_ASSERT( WEBANPICK_HARDFORK_0_8 == 8, "Invalid hardfork configuration" );
   _hardfork_times[ WEBANPICK_HARDFORK_0_8 ] = fc::time_point_sec( WEBANPICK_HARDFORK_0_8_TIME );
   _hardfork_versions[ WEBANPICK_HARDFORK_0_8 ] = WEBANPICK_HARDFORK_0_8_VERSION;
   FC_ASSERT( WEBANPICK_HARDFORK_0_9 == 9, "Invalid hardfork configuration" );
   _hardfork_times[ WEBANPICK_HARDFORK_0_9 ] = fc::time_point_sec( WEBANPICK_HARDFORK_0_9_TIME );
   _hardfork_versions[ WEBANPICK_HARDFORK_0_9 ] = WEBANPICK_HARDFORK_0_9_VERSION;
   FC_ASSERT( WEBANPICK_HARDFORK_0_10 == 10, "Invalid hardfork configuration" );
   _hardfork_times[ WEBANPICK_HARDFORK_0_10 ] = fc::time_point_sec( WEBANPICK_HARDFORK_0_10_TIME );
   _hardfork_versions[ WEBANPICK_HARDFORK_0_10 ] = WEBANPICK_HARDFORK_0_10_VERSION;
   FC_ASSERT( WEBANPICK_HARDFORK_0_11 == 11, "Invalid hardfork configuration" );
   _hardfork_times[ WEBANPICK_HARDFORK_0_11 ] = fc::time_point_sec( WEBANPICK_HARDFORK_0_11_TIME );
   _hardfork_versions[ WEBANPICK_HARDFORK_0_11 ] = WEBANPICK_HARDFORK_0_11_VERSION;
   FC_ASSERT( WEBANPICK_HARDFORK_0_12 == 12, "Invalid hardfork configuration" );
   _hardfork_times[ WEBANPICK_HARDFORK_0_12 ] = fc::time_point_sec( WEBANPICK_HARDFORK_0_12_TIME );
   _hardfork_versions[ WEBANPICK_HARDFORK_0_12 ] = WEBANPICK_HARDFORK_0_12_VERSION;
   FC_ASSERT( WEBANPICK_HARDFORK_0_13 == 13, "Invalid hardfork configuration" );
   _hardfork_times[ WEBANPICK_HARDFORK_0_13 ] = fc::time_point_sec( WEBANPICK_HARDFORK_0_13_TIME );
   _hardfork_versions[ WEBANPICK_HARDFORK_0_13 ] = WEBANPICK_HARDFORK_0_13_VERSION;
   FC_ASSERT( WEBANPICK_HARDFORK_0_14 == 14, "Invalid hardfork configuration" );
   _hardfork_times[ WEBANPICK_HARDFORK_0_14 ] = fc::time_point_sec( WEBANPICK_HARDFORK_0_14_TIME );
   _hardfork_versions[ WEBANPICK_HARDFORK_0_14 ] = WEBANPICK_HARDFORK_0_14_VERSION;
   FC_ASSERT( WEBANPICK_HARDFORK_0_15 == 15, "Invalid hardfork configuration" );
   _hardfork_times[ WEBANPICK_HARDFORK_0_15 ] = fc::time_point_sec( WEBANPICK_HARDFORK_0_15_TIME );
   _hardfork_versions[ WEBANPICK_HARDFORK_0_15 ] = WEBANPICK_HARDFORK_0_15_VERSION;
   FC_ASSERT( WEBANPICK_HARDFORK_0_16 == 16, "Invalid hardfork configuration" );
   _hardfork_times[ WEBANPICK_HARDFORK_0_16 ] = fc::time_point_sec( WEBANPICK_HARDFORK_0_16_TIME );
   _hardfork_versions[ WEBANPICK_HARDFORK_0_16 ] = WEBANPICK_HARDFORK_0_16_VERSION;
   FC_ASSERT( WEBANPICK_HARDFORK_0_17 == 17, "Invalid hardfork configuration" );
   _hardfork_times[ WEBANPICK_HARDFORK_0_17 ] = fc::time_point_sec( WEBANPICK_HARDFORK_0_17_TIME );
   _hardfork_versions[ WEBANPICK_HARDFORK_0_17 ] = WEBANPICK_HARDFORK_0_17_VERSION;
   FC_ASSERT( WEBANPICK_HARDFORK_0_18 == 18, "Invalid hardfork configuration" );
   _hardfork_times[ WEBANPICK_HARDFORK_0_18 ] = fc::time_point_sec( WEBANPICK_HARDFORK_0_18_TIME );
   _hardfork_versions[ WEBANPICK_HARDFORK_0_18 ] = WEBANPICK_HARDFORK_0_18_VERSION;
   FC_ASSERT( WEBANPICK_HARDFORK_0_19 == 19, "Invalid hardfork configuration" );
   _hardfork_times[ WEBANPICK_HARDFORK_0_19 ] = fc::time_point_sec( WEBANPICK_HARDFORK_0_19_TIME );
   _hardfork_versions[ WEBANPICK_HARDFORK_0_19 ] = WEBANPICK_HARDFORK_0_19_VERSION;
#ifdef IS_TEST_NET
   FC_ASSERT( WEBANPICK_HARDFORK_0_20 == 20, "Invalid hardfork configuration" );
   _hardfork_times[ WEBANPICK_HARDFORK_0_20 ] = fc::time_point_sec( WEBANPICK_HARDFORK_0_20_TIME );
   _hardfork_versions[ WEBANPICK_HARDFORK_0_20 ] = WEBANPICK_HARDFORK_0_20_VERSION;
   FC_ASSERT( WEBANPICK_HARDFORK_0_21 == 21, "Invalid hardfork configuration" );
   _hardfork_times[ WEBANPICK_HARDFORK_0_21 ] = fc::time_point_sec( WEBANPICK_HARDFORK_0_21_TIME );
   _hardfork_versions[ WEBANPICK_HARDFORK_0_21 ] = WEBANPICK_HARDFORK_0_21_VERSION;
#endif


   const auto& hardforks = get_hardfork_property_object();
   FC_ASSERT( hardforks.last_hardfork <= WEBANPICK_NUM_HARDFORKS, "Chain knows of more hardforks than configuration", ("hardforks.last_hardfork",hardforks.last_hardfork)("WEBANPICK_NUM_HARDFORKS",WEBANPICK_NUM_HARDFORKS) );
   FC_ASSERT( _hardfork_versions[ hardforks.last_hardfork ] <= WEBANPICK_BLOCKCHAIN_VERSION, "Blockchain version is older than last applied hardfork" );
   FC_ASSERT( WEBANPICK_BLOCKCHAIN_HARDFORK_VERSION >= WEBANPICK_BLOCKCHAIN_VERSION );
   FC_ASSERT( WEBANPICK_BLOCKCHAIN_HARDFORK_VERSION == _hardfork_versions[ WEBANPICK_NUM_HARDFORKS ] );
}
...
void database::process_hardforks()
{
   try
   {
      ...
      if( has_hardfork( WEBANPICK_HARDFORK_0_5__54 ) )
      {
         ...
         {
            if( hardforks.last_hardfork < WEBANPICK_NUM_HARDFORKS ) {
      ...
      {
         while( hardforks.last_hardfork < WEBANPICK_NUM_HARDFORKS
               && _hardfork_times[ hardforks.last_hardfork + 1 ] <= head_block_time()
               && hardforks.last_hardfork < WEBANPICK_HARDFORK_0_5__54 )
         {
            apply_hardfork( hardforks.last_hardfork + 1 );
         }
      }
...
void database::set_hardfork( uint32_t hardfork, bool apply_now )
{
   ...
   for( uint32_t i = hardforks.last_hardfork + 1; i <= hardfork && i <= WEBANPICK_NUM_HARDFORKS; i++ )
   {
      if( i <= WEBANPICK_HARDFORK_0_5__54 )
   ...
   switch( hardfork )
   {
      case WEBANPICK_HARDFORK_0_1:
...
#ifdef IS_TEST_NET
         {
            ...
            test_op.required_auths.insert( WEBANPICK_INIT_MINER_NAME );
...
#endif
         break;
      case WEBANPICK_HARDFORK_0_2:
         retally_witness_votes();
         break;
      case WEBANPICK_HARDFORK_0_3:
         retally_witness_votes();
         break;
      case WEBANPICK_HARDFORK_0_4:
         reset_virtual_schedule_time(*this);
         break;
      case WEBANPICK_HARDFORK_0_5:
         break;
      case WEBANPICK_HARDFORK_0_6:
         retally_witness_vote_counts();
         retally_comment_children();
         break;
      case WEBANPICK_HARDFORK_0_7:
         break;
      case WEBANPICK_HARDFORK_0_8:
         retally_witness_vote_counts(true);
         break;
      case WEBANPICK_HARDFORK_0_9:
         {
         ...
         break;
      case WEBANPICK_HARDFORK_0_10:
         retally_liquidity_weight();
         break;
      case WEBANPICK_HARDFORK_0_11:
         break;
      case WEBANPICK_HARDFORK_0_12:
         {
            ...
            for( auto itr = comment_idx.begin(); itr != comment_idx.end(); ++itr )
            {
               ...
               if( itr->parent_author == WEBANPICK_ROOT_POST_PARENT )
               {
                  ...
                  if( itr->last_payout == fc::time_point_sec::min() && itr->cashout_time == fc::time_point_sec::maximum() )
                  {
                     ...
                     {
                        c.cashout_time = head_block_time() + WEBANPICK_CASHOUT_WINDOW_SECONDS_PRE_HF17;
                     });
                  }
                  ...
                  else if( itr->last_payout > fc::time_point_sec() )
                  {
                     modify( *itr, [&]( comment_object& c )
                     {
                        c.cashout_time = c.last_payout + WEBANPICK_SECOND_CASHOUT_WINDOW;
                     });
                  }
               }
            }

            modify( get< account_authority_object, by_account >( WEBANPICK_MINER_ACCOUNT ), [&]( account_authority_object& auth )
            {
               auth.posting = authority();
               auth.posting.weight_threshold = 1;
            });

            modify( get< account_authority_object, by_account >( WEBANPICK_NULL_ACCOUNT ), [&]( account_authority_object& auth )
            {
               auth.posting = authority();
               auth.posting.weight_threshold = 1;
            });

            modify( get< account_authority_object, by_account >( WEBANPICK_TEMP_ACCOUNT ), [&]( account_authority_object& auth )
            {
               auth.posting = authority();
               auth.posting.weight_threshold = 1;
            });
         ...
         break;
      case WEBANPICK_HARDFORK_0_13:
         break;
      case WEBANPICK_HARDFORK_0_14:
         break;
      case WEBANPICK_HARDFORK_0_15:
         break;
      case WEBANPICK_HARDFORK_0_16:
         {
            modify( get_feed_history(), [&]( feed_history_object& fho )
            {
               while( fho.price_history.size() > WEBANPICK_FEED_HISTORY_WINDOW )
                  fho.price_history.pop_front();
            });
         }
         break;
      case WEBANPICK_HARDFORK_0_17:
         {
            static_assert(
               WEBANPICK_MAX_VOTED_WITNESSES_HF0 + WEBANPICK_MAX_MINER_WITNESSES_HF0 + WEBANPICK_MAX_RUNNER_WITNESSES_HF0 == WEBANPICK_MAX_WITNESSES,
               "HF0 witness counts must add up to WEBANPICK_MAX_WITNESSES" );
            static_assert(
               WEBANPICK_MAX_VOTED_WITNESSES_HF17 + WEBANPICK_MAX_MINER_WITNESSES_HF17 + WEBANPICK_MAX_RUNNER_WITNESSES_HF17 == WEBANPICK_MAX_WITNESSES,
               "HF17 witness counts must add up to WEBANPICK_MAX_WITNESSES" );

            modify( get_witness_schedule_object(), [&]( witness_schedule_object& wso )
            {
               wso.max_voted_witnesses = WEBANPICK_MAX_VOTED_WITNESSES_HF17;
               wso.max_miner_witnesses = WEBANPICK_MAX_MINER_WITNESSES_HF17;
               wso.max_runner_witnesses = WEBANPICK_MAX_RUNNER_WITNESSES_HF17;
            });
            ...
            {
               rfo.name = WEBANPICK_POST_REWARD_FUND_NAME;
               rfo.last_update = head_block_time();
               rfo.content_constant = WEBANPICK_CONTENT_CONSTANT_HF0;
               rfo.percent_curation_rewards = WEBANPICK_1_PERCENT * 25;
               rfo.percent_content_rewards = WEBANPICK_100_PERCENT;
               rfo.reward_balance = gpo.total_reward_fund_webanpick;
#ifndef IS_TEST_NET
               rfo.recent_claims = WEBANPICK_HF_17_RECENT_CLAIMS;
#endif
            ...
            modify( gpo, [&]( dynamic_global_property_object& g )
            {
               g.total_reward_fund_webanpick = asset( 0, WEBANPICK_SYMBOL );
            ...
            */
            ...
            root_posts.reserve( WEBANPICK_HF_17_NUM_POSTS );
            vector< const comment_object* > replies;
            replies.reserve( WEBANPICK_HF_17_NUM_REPLIES );
            ...
            for( const auto& itr : root_posts )
            {
               modify( *itr, [&]( comment_object& c )
               {
                  c.cashout_time = std::max( c.created + WEBANPICK_CASHOUT_WINDOW_SECONDS, c.cashout_time );
               });
            }
            ...
            for( const auto& itr : replies )
            {
               modify( *itr, [&]( comment_object& c )
               {
                  c.cashout_time = std::max( calculate_discussion_payout_time( c ), c.created + WEBANPICK_CASHOUT_WINDOW_SECONDS );
               });
            }
         }
         break;
      case WEBANPICK_HARDFORK_0_18:
         break;
      case WEBANPICK_HARDFORK_0_19:
         {
            modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
            {
               gpo.vote_power_reserve_rate = WEBANPICK_REDUCED_VOTE_POWER_RATE;
            });

            modify( get< reward_fund_object, by_name >( WEBANPICK_POST_REWARD_FUND_NAME ), [&]( reward_fund_object &rfo )
            {
#ifndef IS_TEST_NET
               rfo.recent_claims = WEBANPICK_HF_19_RECENT_CLAIMS;
#endif
         ...
         break;
#ifdef IS_TEST_NET
      case WEBANPICK_HARDFORK_0_20:
         break;
      case WEBANPICK_HARDFORK_0_21:
         break;
#endif
      default:
         break;
...
void database::validate_invariants()const
{
   try
   {
      ...
      asset total_supply = asset( 0, WEBANPICK_SYMBOL );
      asset total_sbd = asset( 0, SBD_SYMBOL );
      asset total_vesting = asset( 0, VESTS_SYMBOL );
      asset pending_vesting_webanpick = asset( 0, WEBANPICK_SYMBOL );
      ...
      for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
      {
         ...
         total_supply += itr->reward_webanpick_balance;
         ...
         pending_vesting_webanpick += itr->reward_vesting_webanpick;
         total_vsf_votes += ( itr->proxy == WEBANPICK_PROXY_TO_SELF_ACCOUNT ?
                                 itr->witness_vote_weight() :
                                 ( WEBANPICK_MAX_PROXY_RECURSION_DEPTH > 0 ?
                                      itr->proxied_vsf_votes[WEBANPICK_MAX_PROXY_RECURSION_DEPTH - 1] :
      ...
      {
         if( itr->amount.symbol == WEBANPICK_SYMBOL )
      ...
      {
         if( itr->sell_price.base.symbol == WEBANPICK_SYMBOL )
         {
            total_supply += asset( itr->for_sale, WEBANPICK_SYMBOL );
         }
      ...
      {
         total_supply += itr->webanpick_balance;
         total_sbd += itr->sbd_balance;

         if( itr->pending_fee.symbol == WEBANPICK_SYMBOL )
            total_supply += itr->pending_fee;
         else if( itr->pending_fee.symbol == SBD_SYMBOL )
            total_sbd += itr->pending_fee;
         else
            FC_ASSERT( false, "found escrow pending fee that is not SBD or WEBANPICK" );
      }
      ...
      for( auto itr = savings_withdraw_idx.begin(); itr != savings_withdraw_idx.end(); ++itr )
      {
         if( itr->amount.symbol == WEBANPICK_SYMBOL )
            ...
            FC_ASSERT( false, "found savings withdraw that is not SBD or WEBANPICK" );
      }
      ...
      total_supply += gpo.total_vesting_fund_webanpick + gpo.total_reward_fund_webanpick + gpo.pending_rewarded_vesting_webanpick;
      ...
      FC_ASSERT( gpo.pending_rewarded_vesting_webanpick == pending_vesting_webanpick, "", ("pending_rewarded_vesting_webanpick",gpo.pending_rewarded_vesting_webanpick)("pending_vesting_webanpick", pending_vesting_webanpick));
   ...
   try
   {
      ...
      for( const auto& account : get_index<account_index>().indices() )
      {
         modify( account, [&]( account_object& a )
         {
            ...
            a.vesting_withdraw_rate  = asset( a.to_withdraw / WEBANPICK_VESTING_WITHDRAW_INTERVALS_PRE_HF_16, VESTS_SYMBOL );
            if( a.vesting_withdraw_rate.amount == 0 )
               a.vesting_withdraw_rate.amount = 1;

            for( uint32_t i = 0; i < WEBANPICK_MAX_PROXY_RECURSION_DEPTH; ++i )
...
// Low memory nodes only need immediate child count, full nodes track total children
         ...
         while( parent )
         {
            ...
            if( parent->parent_author != WEBANPICK_ROOT_POST_PARENT )
...
void database::retally_witness_votes()
{
   ...
   for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
   {
      if( itr->proxy != WEBANPICK_PROXY_TO_SELF_ACCOUNT ) continue;
...
void database::retally_witness_vote_counts( bool force )
{
   ...
   for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
   {
      ...
      if( force || (a.proxy != WEBANPICK_PROXY_TO_SELF_ACCOUNT  ) )
      {
        const auto& vidx = get_index< witness_vote_index >().indices().get< by_account_witness >();
        auto wit_itr = vidx.lower_bound( boost::make_tuple( a.name, account_name_type() ) );
        while( wit_itr != vidx.end() && wit_itr->account == a.name )
