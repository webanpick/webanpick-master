...
using webanpick::protocol::transaction_id_type;
using webanpick::protocol::chain_id_type;
...
{
   class variant;
   inline void to_variant( const webanpick::chain::shared_string& s, variant& var )
   {
      var = fc::string( webanpick::chain::to_string( s ) );
   }

   inline void from_variant( const variant& var, webanpick::chain::shared_string& s )
   {
...
FC_REFLECT_ENUM( webanpick::chain::object_type,
                 (dynamic_global_property_object_type)
                 (account_object_type)
                 (account_authority_object_type)
                 (witness_object_type)
...
FC_REFLECT_TYPENAME( webanpick::chain::shared_string )

FC_REFLECT_ENUM( webanpick::chain::bandwidth_type, (post)(forum)(market) )
