...
share_type calc_percent_reward( share_type current_supply )
{
   ...
   static_assert( multiply_constant < UINT64_MAX / (10 * uint64_t( WEBANPICK_100_PERCENT )), "multiply constant is too large, we may be in danger of overflow" );
   static_assert( (percent == 0) || (percent > WEBANPICK_1_PERCENT), "percent is smaller than 1%, re-check your arguments" );
   static_assert( percent <= WEBANPICK_100_PERCENT, "percent is implausibly large, re-check your arguments (if you really mean to do this, you should revise the overflow check above accordingly)" );
...
{
   return calc_percent_reward< percent, WEBANPICK_APR_PERCENT_MULTIPLY_PER_HOUR, WEBANPICK_APR_PERCENT_SHIFT_PER_HOUR >( current_supply );
}

template< uint16_t percent >
inline share_type calc_percent_reward_per_block( share_type current_supply )
{
   return calc_percent_reward< percent, WEBANPICK_APR_PERCENT_MULTIPLY_PER_BLOCK, WEBANPICK_APR_PERCENT_SHIFT_PER_BLOCK >( current_supply );
}

template< uint16_t percent >
inline share_type calc_percent_reward_per_round( share_type current_supply )
{
   return calc_percent_reward< percent, WEBANPICK_APR_PERCENT_MULTIPLY_PER_ROUND, WEBANPICK_APR_PERCENT_SHIFT_PER_ROUND >( current_supply );
}
