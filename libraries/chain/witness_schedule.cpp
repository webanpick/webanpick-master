...
void reset_virtual_schedule_time( database& db )
{
   ...
   for( const auto& witness : idx )
   {
      db.modify( witness, [&]( witness_object& wobj )
      {
         ...
         wobj.virtual_scheduled_time = WEBANPICK_VIRTUAL_SCHEDULE_LAP_LENGTH2 / (wobj.votes.value+1);
      } );
   }
}
...
void update_witness_schedule4( database& db )
{
   ...
   active_witnesses.reserve( WEBANPICK_MAX_WITNESSES );
   ...
   for( auto itr = widx.begin();
        ...
        ++itr )
   {
      if( db.has_hardfork( WEBANPICK_HARDFORK_0_14__278 ) && (itr->signing_key == public_key_type()) )
         continue;
   ...
   while( mitr != pow_idx.end() && selected_miners.size() < wso.max_miner_witnesses )
   {
      ...
      if( selected_voted.find(mitr->id) == selected_voted.end() )
      {
         ...
         if( !( db.has_hardfork( WEBANPICK_HARDFORK_0_14__278 ) && db.get_witness( mitr->owner ).signing_key == public_key_type() ) )
         {
   ...
   for( auto witness_count = selected_voted.size() + selected_miners.size();
        sitr != schedule_idx.end() && witness_count < WEBANPICK_MAX_WITNESSES;
        ++sitr )
   {
      new_virtual_time = sitr->virtual_scheduled_time; /// everyone advances to at least this time
      processed_witnesses.push_back(sitr);

      if( db.has_hardfork( WEBANPICK_HARDFORK_0_14__278 ) && sitr->signing_key == public_key_type() )
   ...
   {
      auto new_virtual_scheduled_time = new_virtual_time + WEBANPICK_VIRTUAL_SCHEDULE_LAP_LENGTH2 / ((*itr)->votes.value+1);
   ...
   size_t expected_active_witnesses = std::min( size_t(WEBANPICK_MAX_WITNESSES), widx.size() );
   FC_ASSERT( active_witnesses.size() == expected_active_witnesses, "number of active witnesses does not equal expected_active_witnesses=${expected_active_witnesses}",
                                       ("active_witnesses.size()",active_witnesses.size()) ("WEBANPICK_MAX_WITNESSES",WEBANPICK_MAX_WITNESSES) ("expected_active_witnesses", expected_active_witnesses) );

   auto majority_version = wso.majority_version;

   if( db.has_hardfork( WEBANPICK_HARDFORK_0_5__54 ) )
   {
   ...
   db.modify( wso, [&]( witness_schedule_object& _wso )
   {
      ...
      for( size_t i = active_witnesses.size(); i < WEBANPICK_MAX_WITNESSES; i++ )
      {
...
{
   if( (db.head_block_num() % WEBANPICK_MAX_WITNESSES) == 0 ) //wso.next_shuffle_block_num )
   {
      if( db.has_hardfork(WEBANPICK_HARDFORK_0_4) )
      {
      ...
      /// only use vote based scheduling after the first 1M WEBANPICK is created or if there is no POW queued
      if( props.num_pow_witnesses == 0 || db.head_block_num() > WEBANPICK_START_MINER_VOTING_BLOCK )
      {
         const auto& widx = db.get_index<witness_index>().indices().get<by_vote_name>();

         for( auto itr = widx.begin(); itr != widx.end() && (active_witnesses.size() < (WEBANPICK_MAX_WITNESSES-2)); ++itr )
         {
            if( itr->pow_worker )
               continue;
         ...
         if( sitr != schedule_idx.end() )
         {
            ...
            db.modify( *sitr, [&]( witness_object& wo )
            {
               ...
               if( db.has_hardfork( WEBANPICK_HARDFORK_0_2 ) )
                  wo.virtual_scheduled_time += WEBANPICK_VIRTUAL_SCHEDULE_LAP_LENGTH2 / (wo.votes.value+1);
               else
                  wo.virtual_scheduled_time += WEBANPICK_VIRTUAL_SCHEDULE_LAP_LENGTH / (wo.votes.value+1);
            } );
         }
      }
      ...
      if( props.num_pow_witnesses > WEBANPICK_MAX_WITNESSES )
      {
         if( itr != pow_idx.end() )
         {
            db.modify( *itr, [&](witness_object& wit )
      ...
      while( itr != pow_idx.end() )
      {
         ...
         if( db.head_block_num() > WEBANPICK_START_MINER_VOTING_BLOCK || active_witnesses.size() >= WEBANPICK_MAX_WITNESSES )
            break;
         ++itr;
      }
      ...
      /*
         ...
         for( const string& w : active_witnesses )
            ...
            */
         // active witnesses has exactly WEBANPICK_MAX_WITNESSES elements, asserted above
         for( size_t i = 0; i < active_witnesses.size(); i++ )
         {
            _wso.current_shuffled_witnesses[i] = active_witnesses[i];
         }
