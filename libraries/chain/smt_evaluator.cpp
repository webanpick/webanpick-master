...
{
   FC_ASSERT( _db.has_hardfork( WEBANPICK_SMT_HARDFORK ), "SMT functionality not enabled until hardfork ${hf}", ("hf", WEBANPICK_SMT_HARDFORK) );
   ...
   FC_ASSERT( dgpo.smt_creation_fee.symbol == WEBANPICK_SYMBOL || dgpo.smt_creation_fee.symbol == SBD_SYMBOL,
      "Unexpected internal error - wrong symbol ${s} of SMT creation fee.", ("s", dgpo.smt_creation_fee.symbol) );
   FC_ASSERT( o.smt_creation_fee.symbol == WEBANPICK_SYMBOL || o.smt_creation_fee.symbol == SBD_SYMBOL,
      "Asset fee must be WEBANPICK or SBD, was ${s}", ("s", o.smt_creation_fee.symbol) );
   ...
   else
   {
      ...
      if( o.smt_creation_fee.symbol == WEBANPICK_SYMBOL )
      {
         effective_elevation_fee = _db.to_sbd( o.smt_creation_fee );
      }
      else
      {
         effective_elevation_fee = _db.to_webanpick( o.smt_creation_fee );         
      }
   }
   ...
   _db.adjust_balance( WEBANPICK_NULL_ACCOUNT,  o.smt_creation_fee );
