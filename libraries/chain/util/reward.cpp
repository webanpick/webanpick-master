...
uint64_t get_rshare_reward( const comment_reward_context& ctx )
{
   ...
   u256 rf(ctx.total_reward_fund_webanpick.amount.value);
   ...
   claim = ( claim * ctx.reward_weight ) / WEBANPICK_100_PERCENT;

   u256 payout_u256 = ( rf * claim ) / total_claims;
   FC_ASSERT( payout_u256 <= u256( uint64_t( std::numeric_limits<int64_t>::max() ) ) );
   uint64_t payout = static_cast< uint64_t >( payout_u256 );

   if( is_comment_payout_dust( ctx.current_webanpick_price, payout ) )
      payout = 0;

   asset max_webanpick = to_webanpick( ctx.current_webanpick_price, ctx.max_sbd );

   payout = std::min( payout, uint64_t( max_webanpick.amount.value ) );
