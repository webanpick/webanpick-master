...
#include <webanpick/chain/witness_objects.hpp>
...
{
   FC_ASSERT( permlink.size() > WEBANPICK_MIN_PERMLINK_LENGTH && permlink.size() < WEBANPICK_MAX_PERMLINK_LENGTH, "Permlink is not a valid size." );
...
void witness_update_evaluator::do_apply( const witness_update_operation& o )
{
   ...
   if ( _db.has_hardfork( WEBANPICK_HARDFORK_0_14__410 ) )
   {
   ...
   if( _db.is_producing() )
   {
      FC_ASSERT( o.props.maximum_block_size <= WEBANPICK_SOFT_MAX_BLOCK_SIZE, "Max block size cannot be more than 2MiB" );
   }
...
{
   FC_ASSERT( _db.has_hardfork( WEBANPICK_HARDFORK_0_20__1620 ), "witness_set_properties_evaluator not enabled until HF 20" );
...
void account_create_evaluator::do_apply( const account_create_operation& o )
{
   ...
   if( _db.has_hardfork( WEBANPICK_HARDFORK_0_19__987) )
   {
      const witness_schedule_object& wso = _db.get_witness_schedule_object();
      FC_ASSERT( o.fee >= asset( wso.median_props.account_creation_fee.amount * WEBANPICK_CREATE_ACCOUNT_WITH_WEBANPICK_MODIFIER, WEBANPICK_SYMBOL ), "Insufficient Fee: ${f} required, ${p} provided.",
                 ("f", wso.median_props.account_creation_fee * asset( WEBANPICK_CREATE_ACCOUNT_WITH_WEBANPICK_MODIFIER, WEBANPICK_SYMBOL ) )
                 ("p", o.fee) );
   }
   else if( _db.has_hardfork( WEBANPICK_HARDFORK_0_1 ) )
   {
      const witness_schedule_object& wso = _db.get_witness_schedule_object();
   ...
   if( _db.has_hardfork( WEBANPICK_HARDFORK_0_15__465 ) )
   {
   ...
   const auto& new_account = _db.create< account_object >( [&]( account_object& acc )
   {
      ...
      if( !_db.has_hardfork( WEBANPICK_HARDFORK_0_11__169 ) )
         acc.recovery_account = "webanpick";
      else
...
{
   FC_ASSERT( !_db.has_hardfork( WEBANPICK_HARDFORK_0_20__1760 ), "Account creation with delegation is deprecated as of Hardfork 20" );
   ...
   auto target_delegation = asset( wso.median_props.account_creation_fee.amount * WEBANPICK_CREATE_ACCOUNT_WITH_WEBANPICK_MODIFIER * WEBANPICK_CREATE_ACCOUNT_DELEGATION_RATIO, WEBANPICK_SYMBOL ) * props.get_vesting_share_price();

   auto current_delegation = asset( o.fee.amount * WEBANPICK_CREATE_ACCOUNT_DELEGATION_RATIO, WEBANPICK_SYMBOL ) * props.get_vesting_share_price() + o.delegation;
   ...
   if( o.delegation.amount > 0 || !_db.has_hardfork( WEBANPICK_HARDFORK_0_19__997 ) )
   {
      _db.create< vesting_delegation_object >( [&]( vesting_delegation_object& vdo )
      {
         ...
         vdo.min_delegation_time = _db.head_block_time() + WEBANPICK_CREATE_ACCOUNT_DELEGATION_TIME;
      });
   }
...
{
   if( _db.has_hardfork( WEBANPICK_HARDFORK_0_1 ) ) FC_ASSERT( o.account != WEBANPICK_TEMP_ACCOUNT, "Cannot update temp account." );

   if( ( _db.has_hardfork( WEBANPICK_HARDFORK_0_15__465 ) ) && o.posting )
      o.posting->validate();
   ...
   {
#ifndef IS_TEST_NET
      if( _db.has_hardfork( WEBANPICK_HARDFORK_0_11 ) )
         FC_ASSERT( _db.head_block_time() - account_auth.last_owner_update > WEBANPICK_OWNER_UPDATE_LIMIT, "Owner authority can only be updated once an hour." );
#endif
   ...
   if( o.active && ( _db.has_hardfork( WEBANPICK_HARDFORK_0_15__465 ) ) )
      verify_authority_accounts_exist( _db, *o.active, o.account, authority::active );
   if( o.posting && ( _db.has_hardfork( WEBANPICK_HARDFORK_0_15__465 ) ) )
...
void delete_comment_evaluator::do_apply( const delete_comment_operation& o )
{
   ...
   if( _db.has_hardfork( WEBANPICK_HARDFORK_0_19__876 ) )
      FC_ASSERT( comment.cashout_time != fc::time_point_sec::maximum() );

   if( _db.has_hardfork( WEBANPICK_HARDFORK_0_19__977 ) )
   ...
   if( _db.has_hardfork( WEBANPICK_HARDFORK_0_6__80 ) && comment.parent_author != WEBANPICK_ROOT_POST_PARENT )
   {
...
void comment_options_evaluator::do_apply( const comment_options_operation& o )
{
   ...
   FC_ASSERT( comment.percent_webanpick_dollars >= o.percent_webanpick_dollars, "A comment cannot accept a greater percent SBD." );

   _db.modify( comment, [&]( comment_object& c ) {
       c.max_accepted_payout   = o.max_accepted_payout;
       c.percent_webanpick_dollars = o.percent_webanpick_dollars;
...
{ try {
   if( _db.has_hardfork( WEBANPICK_HARDFORK_0_5__55 ) )
   ...
   if( o.parent_author != WEBANPICK_ROOT_POST_PARENT )
   {
      parent = &_db.get_comment( o.parent_author, o.parent_permlink );
      if( !_db.has_hardfork( WEBANPICK_HARDFORK_0_17__767 ) )
         FC_ASSERT( parent->depth < WEBANPICK_MAX_COMMENT_DEPTH_PRE_HF17, "Comment is nested ${x} posts deep, maximum depth is ${y}.", ("x",parent->depth)("y",WEBANPICK_MAX_COMMENT_DEPTH_PRE_HF17) );
      else
         FC_ASSERT( parent->depth < WEBANPICK_MAX_COMMENT_DEPTH, "Comment is nested ${x} posts deep, maximum depth is ${y}.", ("x",parent->depth)("y",WEBANPICK_MAX_COMMENT_DEPTH) );
   }
   ...
   if ( itr == by_permlink_idx.end() )
   {
      if( o.parent_author != WEBANPICK_ROOT_POST_PARENT )
      {
         ...
         if( _db.has_hardfork( WEBANPICK_HARDFORK_0_12__177 ) && !_db.has_hardfork( WEBANPICK_HARDFORK_0_17__869 ) )
            FC_ASSERT( _db.calculate_discussion_payout_time( *parent ) != fc::time_point_sec::maximum(), "Discussion is frozen." );
      }

      if( _db.has_hardfork( WEBANPICK_HARDFORK_0_12__176 ) )
      {
         if( o.parent_author == WEBANPICK_ROOT_POST_PARENT )
             FC_ASSERT( ( now - auth.last_root_post ) > WEBANPICK_MIN_ROOT_COMMENT_INTERVAL, "You may only post once every 5 minutes.", ("now",now)("last_root_post", auth.last_root_post) );
         else
             FC_ASSERT( (now - auth.last_post) > WEBANPICK_MIN_REPLY_INTERVAL, "You may only comment once every 20 seconds.", ("now",now)("auth.last_post",auth.last_post) );
      }
      else if( _db.has_hardfork( WEBANPICK_HARDFORK_0_6__113 ) )
      {
         if( o.parent_author == WEBANPICK_ROOT_POST_PARENT )
             FC_ASSERT( (now - auth.last_post) > WEBANPICK_MIN_ROOT_COMMENT_INTERVAL, "You may only post once every 5 minutes.", ("now",now)("auth.last_post",auth.last_post) );
         else
             FC_ASSERT( (now - auth.last_post) > WEBANPICK_MIN_REPLY_INTERVAL, "You may only comment once every 20 seconds.", ("now",now)("auth.last_post",auth.last_post) );
      }
      else
      ...
      uint16_t reward_weight = WEBANPICK_100_PERCENT;
      uint64_t post_bandwidth = auth.post_bandwidth;

      if( _db.has_hardfork( WEBANPICK_HARDFORK_0_12__176 ) && !_db.has_hardfork( WEBANPICK_HARDFORK_0_17__733 ) && o.parent_author == WEBANPICK_ROOT_POST_PARENT )
      {
         uint64_t post_delta_time = std::min( _db.head_block_time().sec_since_epoch() - auth.last_root_post.sec_since_epoch(), WEBANPICK_POST_AVERAGE_WINDOW );
         uint32_t old_weight = uint32_t( ( post_bandwidth * ( WEBANPICK_POST_AVERAGE_WINDOW - post_delta_time ) ) / WEBANPICK_POST_AVERAGE_WINDOW );
         post_bandwidth = ( old_weight + WEBANPICK_100_PERCENT );
         reward_weight = uint16_t( std::min( ( WEBANPICK_POST_WEIGHT_CONSTANT * WEBANPICK_100_PERCENT ) / ( post_bandwidth * post_bandwidth ), uint64_t( WEBANPICK_100_PERCENT ) ) );
      }
      ...
      {
         if( _db.has_hardfork( WEBANPICK_HARDFORK_0_1 ) )
         {
         ...
         if ( o.parent_author == WEBANPICK_ROOT_POST_PARENT )
         {
            ...
            com.cashout_time = _db.has_hardfork( WEBANPICK_HARDFORK_0_12__177 ) ?
               _db.head_block_time() + WEBANPICK_CASHOUT_WINDOW_SECONDS_PRE_HF17 :
         ...
         if( _db.has_hardfork( WEBANPICK_HARDFORK_0_17__769 ) )
         {
            com.cashout_time = com.created + WEBANPICK_CASHOUT_WINDOW_SECONDS;
         }
      });
...
/// this loop can be skiped for validate-only nodes as it is merely gathering stats for indicies
   ...
   else // start edit case
   {
      ...
      if( !_db.has_hardfork( WEBANPICK_HARDFORK_0_17__772 ) )
      {
         if( _db.has_hardfork( WEBANPICK_HARDFORK_0_14__306 ) )
            FC_ASSERT( _db.calculate_discussion_payout_time( comment ) != fc::time_point_sec::maximum(), "The comment is archived." );
         else if( _db.has_hardfork( WEBANPICK_HARDFORK_0_10 ) )
...
void escrow_transfer_evaluator::do_apply( const escrow_transfer_operation& o )
{
   try
   {
      ...
      asset webanpick_spent = o.webanpick_amount;
      asset sbd_spent = o.sbd_amount;
      if( o.fee.symbol == WEBANPICK_SYMBOL )
         webanpick_spent += o.fee;
      else
         sbd_spent += o.fee;

      FC_ASSERT( from_account.balance >= webanpick_spent, "Account cannot cover WEBANPICK costs of escrow. Required: ${r} Available: ${a}", ("r",webanpick_spent)("a",from_account.balance) );
      ...
      _db.adjust_balance( from_account, -webanpick_spent );
      ...
      _db.create<escrow_object>([&]( escrow_object& esc )
      {
         ...
         esc.webanpick_balance          = o.webanpick_amount;
...
void escrow_approve_evaluator::do_apply( const escrow_approve_operation& o )
{
   try
   {
      ...
      if( reject_escrow )
      {
         _db.adjust_balance( o.from, escrow.webanpick_balance );
...
void escrow_release_evaluator::do_apply( const escrow_release_operation& o )
{
   try
   {
      ...
      const auto& e = _db.get_escrow( o.from, o.escrow_id );
      FC_ASSERT( e.webanpick_balance >= o.webanpick_amount, "Release amount exceeds escrow balance. Amount: ${a}, Balance: ${b}", ("a", o.webanpick_amount)("b", e.webanpick_balance) );
      ...
      _db.adjust_balance( o.receiver, o.webanpick_amount );
      ...
      if( e.webanpick_balance.amount == 0 && e.sbd_balance.amount == 0 )
      {
         _db.remove( e );
      }
   }
...
void transfer_to_vesting_evaluator::do_apply( const transfer_to_vesting_operation& o )
{
   ...
   FC_ASSERT( _db.get_balance( from_account, WEBANPICK_SYMBOL) >= o.amount, "Account does not have sufficient WEBANPICK for transfer." );
...
void withdraw_vesting_evaluator::do_apply( const withdraw_vesting_operation& o )
{
   ...
   FC_ASSERT( account.vesting_shares >= asset( 0, VESTS_SYMBOL ), "Account does not have sufficient Webanpick Power for withdraw." );
   FC_ASSERT( account.vesting_shares - account.delegated_vesting_shares >= o.vesting_shares, "Account does not have sufficient Webanpick Power for withdraw." );

   if( !account.mined && _db.has_hardfork( WEBANPICK_HARDFORK_0_1 ) )
   {
      const auto& props = _db.get_dynamic_global_properties();
      const witness_schedule_object& wso = _db.get_witness_schedule_object();
      ...
      FC_ASSERT( account.vesting_shares > min_vests || ( _db.has_hardfork( WEBANPICK_HARDFORK_0_16__562 ) && o.vesting_shares.amount == 0 ),
                 "Account registered by another account requires 10x account creation fee worth of Webanpick Power before it can be powered down." );
   }

   if( o.vesting_shares.amount == 0 )
   {
      if( _db.has_hardfork( WEBANPICK_HARDFORK_0_5__57 ) )
   ...
   {
      int vesting_withdraw_intervals = WEBANPICK_VESTING_WITHDRAW_INTERVALS_PRE_HF_16;
      if( _db.has_hardfork( WEBANPICK_HARDFORK_0_16__551 ) )
         vesting_withdraw_intervals = WEBANPICK_VESTING_WITHDRAW_INTERVALS; /// 13 weeks = 1 quarter of a year
      ...
      _db.modify( account, [&]( account_object& a )
      {
         ...
         a.next_vesting_withdrawal = _db.head_block_time() + fc::seconds(WEBANPICK_VESTING_WITHDRAW_INTERVAL_SECONDS);
...
void set_withdraw_vesting_route_evaluator::do_apply( const set_withdraw_vesting_route_operation& o )
{
   ...
   if( itr == wd_idx.end() )
   {
      ...
      FC_ASSERT( from_account.withdraw_routes < WEBANPICK_MAX_WITHDRAW_ROUTES, "Account already has the maximum number of routes." );
   ...
   FC_ASSERT( total_percent <= WEBANPICK_100_PERCENT, "More than 100% of vesting withdrawals allocated to destinations." );
   }
...
void account_witness_proxy_evaluator::do_apply( const account_witness_proxy_operation& o )
{
   ...
   if( o.proxy.size() ) {
      ...
      proxy_chain.reserve( WEBANPICK_MAX_PROXY_RECURSION_DEPTH + 1 );
      ...
      while( cprox->proxy.size() != 0 ) {
         ...
         cprox = &next_proxy;
         FC_ASSERT( proxy_chain.size() <= WEBANPICK_MAX_PROXY_RECURSION_DEPTH, "Proxy chain is too long." );
      }
...
void account_witness_vote_evaluator::do_apply( const account_witness_vote_operation& o )
{
   ...
   if( itr == by_account_witness_idx.end() ) {
      ...
      if ( _db.has_hardfork( WEBANPICK_HARDFORK_0_2 ) )
      {
         FC_ASSERT( voter.witnesses_voted_for < WEBANPICK_MAX_ACCOUNT_WITNESS_VOTES, "Account has voted for too many witnesses." ); // TODO: Remove after hardfork 2
         ...
         if( _db.has_hardfork( WEBANPICK_HARDFORK_0_3 ) ) {
...
{ try {
   ...
   if( _db.has_hardfork( WEBANPICK_HARDFORK_0_12__177 ) && _db.calculate_discussion_payout_time( comment ) == fc::time_point_sec::maximum() )
   {
#ifndef CLEAR_VOTES
   ...
   if( _db.has_hardfork( WEBANPICK_HARDFORK_0_11 ) )
      FC_ASSERT( elapsed_seconds >= WEBANPICK_MIN_VOTE_INTERVAL_SEC, "Can only vote once every 3 seconds." );

   int64_t regenerated_power = (WEBANPICK_100_PERCENT * elapsed_seconds) / WEBANPICK_VOTE_REGENERATION_SECONDS;
   int64_t current_power     = std::min( int64_t(voter.voting_power + regenerated_power), int64_t(WEBANPICK_100_PERCENT) );
   ...
   int64_t  used_power  = ((current_power * abs_weight) / WEBANPICK_100_PERCENT) * (60*60*24);

   const dynamic_global_property_object& dgpo = _db.get_dynamic_global_properties();

   // The second multiplication is rounded up as of HF 259
   int64_t max_vote_denom = dgpo.vote_power_reserve_rate * WEBANPICK_VOTE_REGENERATION_SECONDS;
   FC_ASSERT( max_vote_denom > 0 );

   if( !_db.has_hardfork( WEBANPICK_HARDFORK_0_14__259 ) )
   {
   ...
   int64_t abs_rshares    = ((uint128_t(voter.effective_vesting_shares().amount.value) * used_power) / (WEBANPICK_100_PERCENT)).to_uint64();
   if( !_db.has_hardfork( WEBANPICK_HARDFORK_0_14__259 ) && abs_rshares == 0 ) abs_rshares = 1;

   if( _db.has_hardfork( WEBANPICK_HARDFORK_0_20__1764 ) )
   {
      abs_rshares -= WEBANPICK_VOTE_DUST_THRESHOLD;
      abs_rshares = std::max( int64_t(0), abs_rshares );
   }
   if( _db.has_hardfork( WEBANPICK_HARDFORK_0_14__259 ) )
   {
      FC_ASSERT( abs_rshares > WEBANPICK_VOTE_DUST_THRESHOLD || o.weight == 0, "Voting weight is too small, please accumulate more voting power or webanpick power." );
   }
   else if( _db.has_hardfork( WEBANPICK_HARDFORK_0_13__248 ) )
   {
      FC_ASSERT( abs_rshares > WEBANPICK_VOTE_DUST_THRESHOLD || abs_rshares == 1, "Voting weight is too small, please accumulate more voting power or webanpick power." );
   }
   ...
   {
      if( _db.has_hardfork( WEBANPICK_HARDFORK_0_12__177 ) )
   ...
   if( itr == comment_vote_idx.end() )
   {
      ...
      if( rshares > 0 )
      {
         if( _db.has_hardfork( WEBANPICK_HARDFORK_0_17__900 ) )
            FC_ASSERT( _db.head_block_time() < comment.cashout_time - WEBANPICK_UPVOTE_LOCKOUT_HF17, "Cannot increase payout within last twelve hours before payout." );
         else if( _db.has_hardfork( WEBANPICK_HARDFORK_0_7 ) )
            FC_ASSERT( _db.head_block_time() < _db.calculate_discussion_payout_time( comment ) - WEBANPICK_UPVOTE_LOCKOUT_HF7, "Cannot increase payout within last minute before payout." );
      }
      ...
      if( !_db.has_hardfork( WEBANPICK_HARDFORK_0_17__769 ) )
      {
         ...
         if( _db.has_hardfork( WEBANPICK_HARDFORK_0_12__177 ) && !_db.has_hardfork( WEBANPICK_HARDFORK_0_13__257)  )
            new_cashout_time_sec = _db.head_block_time().sec_since_epoch() + WEBANPICK_CASHOUT_WINDOW_SECONDS_PRE_HF17;
         else
            new_cashout_time_sec = _db.head_block_time().sec_since_epoch() + WEBANPICK_CASHOUT_WINDOW_SECONDS_PRE_HF12;
      ...
      _db.modify( comment, [&]( comment_object& c ){
         ...
         if( rshares > 0 )
            ...
            c.net_votes--;
         if( !_db.has_hardfork( WEBANPICK_HARDFORK_0_6__114 ) && c.net_rshares == -c.abs_rshares) FC_ASSERT( c.net_votes < 0, "Comment has negative net votes?" );
      });
      ...
      _db.modify( root, [&]( comment_object& c )
      {
         ...
         if( !_db.has_hardfork( WEBANPICK_HARDFORK_0_17__769 ) )
         {
            if( _db.has_hardfork( WEBANPICK_HARDFORK_0_12__177 ) && c.last_payout > fc::time_point_sec::min() )
            ...
            if( c.max_cashout_time == fc::time_point_sec::maximum() )
               c.max_cashout_time = _db.head_block_time() + fc::seconds( WEBANPICK_MAX_CASHOUT_WINDOW_SECONDS );
         }
      });
      ...
      **/
      _db.create<comment_vote_object>( [&]( comment_vote_object& cv ){
         ...
         if( curation_reward_eligible && _db.has_hardfork( WEBANPICK_HARDFORK_0_17__774 ) )
            curation_reward_eligible = _db.get_curation_rewards_percent( comment ) > 0;

         if( curation_reward_eligible )
         {
            if( comment.created < fc::time_point_sec(WEBANPICK_HARDFORK_0_6_REVERSE_AUCTION_TIME) ) {
               ...
               if( _db.has_hardfork( WEBANPICK_HARDFORK_0_17__774 ) )
               {
                  const auto& reward_fund = _db.get_reward_fund( comment );
                  auto curve = !_db.has_hardfork( WEBANPICK_HARDFORK_0_19__1052 ) && comment.created > WEBANPICK_HF_19_SQRT_PRE_CALC
            ...
            if( _db.head_block_time() > fc::time_point_sec(WEBANPICK_HARDFORK_0_6_REVERSE_AUCTION_TIME) )  /// start enforcing this prior to the hardfork
            {
               /// discount weight by time
               uint128_t w(max_vote_weight);
               uint64_t delta_t = std::min( uint64_t((cv.last_update - comment.created).to_seconds()), uint64_t(WEBANPICK_REVERSE_AUCTION_WINDOW_SECONDS) );

               w *= delta_t;
               w /= WEBANPICK_REVERSE_AUCTION_WINDOW_SECONDS;
   ...
   {
      FC_ASSERT( itr->num_changes < WEBANPICK_MAX_VOTE_CHANGES, "Voter has used the maximum number of vote changes on this comment." );

      if( _db.has_hardfork( WEBANPICK_HARDFORK_0_6__112 ) )
      ...
      if( !_db.has_hardfork( WEBANPICK_HARDFORK_0_17__769 ) )
      {
         ...
         if( _db.has_hardfork( WEBANPICK_HARDFORK_0_14__259 ) && abs_rshares == 0 )
...
{
   database& d = db();
   FC_ASSERT( d.has_hardfork( WEBANPICK_HARDFORK_0_14__317 ) );
...
void pow_apply( database& db, Operation o )
{
   ...
   if( db.has_hardfork( WEBANPICK_HARDFORK_0_5__59 ) )
   {
   ...
   if(itr == accounts_by_name.end())
   {
      db.create< account_object >( [&]( account_object& acc )
      {
         ...
         if( !db.has_hardfork( WEBANPICK_HARDFORK_0_11__169 ) )
            acc.recovery_account = "webanpick";
         else
   ...
   if( db.has_hardfork( WEBANPICK_HARDFORK_0_13__256 ) )
   ...
   if( db.head_block_num() < WEBANPICK_START_MINER_VOTING_BLOCK )
      pow_reward.amount *= WEBANPICK_MAX_WITNESSES;
...
void pow_evaluator::do_apply( const pow_operation& o ) {
   FC_ASSERT( !db().has_hardfork( WEBANPICK_HARDFORK_0_13__256 ), "pow is deprecated. Use pow2 instead" );
   pow_apply( db(), o );
}
...
{
   database& db = this->db();
   FC_ASSERT( !db.has_hardfork( WEBANPICK_HARDFORK_0_17__770 ), "mining is now disabled" );

   const auto& dgp = db.get_dynamic_global_properties();
   uint32_t target_pow = db.get_pow_summary_target();
   account_name_type worker_account;

   if( db.has_hardfork( WEBANPICK_HARDFORK_0_16__551 ) )
   {
   ...
   FC_ASSERT( o.props.maximum_block_size >= WEBANPICK_MIN_BLOCK_SIZE_LIMIT * 2, "Voted maximum block size is too small." );
...
{
   if( _db.has_hardfork( WEBANPICK_HARDFORK_0_20__409 ) )
      FC_ASSERT( is_asset_type( o.exchange_rate.base, SBD_SYMBOL ) && is_asset_type( o.exchange_rate.quote, WEBANPICK_SYMBOL ),
            "Price feed must be a SBD/WEBANPICK price" );
...
void convert_evaluator::do_apply( const convert_operation& o )
{
  ...
  auto webanpick_conversion_delay = WEBANPICK_CONVERSION_DELAY_PRE_HF_16;
  if( _db.has_hardfork( WEBANPICK_HARDFORK_0_16__551) )
     webanpick_conversion_delay = WEBANPICK_CONVERSION_DELAY;
  ...
  _db.create<convert_request_object>( [&]( convert_request_object& obj )
  {
      ...
      obj.amount          = o.amount;
      obj.conversion_date = _db.head_block_time() + webanpick_conversion_delay;
  });
...
{
   FC_ASSERT( !_db.has_hardfork( WEBANPICK_HARDFORK_0_4 ), "report_over_production_operation is disabled." );
}
...
void request_account_recovery_evaluator::do_apply( const request_account_recovery_operation& o )
{
   ...
   if( request == recovery_request_idx.end() ) // New Request
   {
      ...
      _db.create< account_recovery_request_object >( [&]( account_recovery_request_object& req )
      {
         ...
         req.expires = _db.head_block_time() + WEBANPICK_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD;
      });
   }
...
void recover_account_evaluator::do_apply( const recover_account_operation& o )
{
   ...
   if( _db.has_hardfork( WEBANPICK_HARDFORK_0_12 ) )
      FC_ASSERT( _db.head_block_time() - account.last_account_recovery > WEBANPICK_OWNER_UPDATE_LIMIT, "Owner authority can only be updated once an hour." );
...
void change_recovery_account_evaluator::do_apply( const change_recovery_account_operation& o )
{
   ...
   if( request == change_recovery_idx.end() ) // New request
   {
      _db.create< change_recovery_account_request_object >( [&]( change_recovery_account_request_object& req )
      {
         ...
         req.effective_on = _db.head_block_time() + WEBANPICK_OWNER_AUTH_RECOVERY_PERIOD;
      });
   }
...
void transfer_from_savings_evaluator::do_apply( const transfer_from_savings_operation& op )
{
   ...
   FC_ASSERT( from.savings_withdraw_requests < WEBANPICK_SAVINGS_WITHDRAW_REQUEST_LIMIT, "Account has reached limit for pending withdraw requests." );
   ...
   _db.create<savings_withdraw_object>( [&]( savings_withdraw_object& s ) {
      ...
      s.complete = _db.head_block_time() + WEBANPICK_SAVINGS_WITHDRAW_TIME;
   });
...
{
   FC_ASSERT( _db.has_hardfork( WEBANPICK_HARDFORK_0_14__324 ) );
   ...
   if( o.decline )
   {
      ...
      _db.create< decline_voting_rights_request_object >( [&]( decline_voting_rights_request_object& req )
      {
         ...
         req.effective_date = _db.head_block_time() + WEBANPICK_OWNER_AUTH_RECOVERY_PERIOD;
      });
   }
   else
   {
...
void claim_reward_balance_evaluator::do_apply( const claim_reward_balance_operation& op )
{
   ...
   FC_ASSERT( op.reward_webanpick <= acnt.reward_webanpick_balance, "Cannot claim that much WEBANPICK. Claim: ${c} Actual: ${a}",
      ("c", op.reward_webanpick)("a", acnt.reward_webanpick_balance) );
   ...
   asset reward_vesting_webanpick_to_move = asset( 0, WEBANPICK_SYMBOL );
   if( op.reward_vests == acnt.reward_vesting_balance )
      reward_vesting_webanpick_to_move = acnt.reward_vesting_webanpick;
   else
      reward_vesting_webanpick_to_move = asset( ( ( uint128_t( op.reward_vests.amount.value ) * uint128_t( acnt.reward_vesting_webanpick.amount.value ) )
         / uint128_t( acnt.reward_vesting_balance.amount.value ) ).to_uint64(), WEBANPICK_SYMBOL );

   _db.adjust_reward_balance( acnt, -op.reward_webanpick );
   _db.adjust_reward_balance( acnt, -op.reward_sbd );
   _db.adjust_balance( acnt, op.reward_webanpick );
   ...
   _db.modify( acnt, [&]( account_object& a )
   {
      ...
      a.reward_vesting_webanpick -= reward_vesting_webanpick_to_move;
   });

   _db.modify( _db.get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
   {
      gpo.total_vesting_shares += op.reward_vests;
      gpo.total_vesting_fund_webanpick += reward_vesting_webanpick_to_move;

      gpo.pending_rewarded_vesting_shares -= op.reward_vests;
      gpo.pending_rewarded_vesting_webanpick -= reward_vesting_webanpick_to_move;
   });
      ...
      else
      {
         ...
         if( token.symbol == VESTS_SYMBOL)
         {
            ...
            if( token == a->reward_vesting_balance )
               reward_vesting_webanpick_to_move = a->reward_vesting_webanpick;
            else
               reward_vesting_webanpick_to_move = asset( ( ( uint128_t( token.amount.value ) * uint128_t( a->reward_vesting_webanpick.amount.value ) )
                  / uint128_t( a->reward_vesting_balance.amount.value ) ).to_uint64(), WEBANPICK_SYMBOL );
         ...
         else if( token.symbol == WEBANPICK_SYMBOL || token.symbol == SBD_SYMBOL )
         {
            FC_ASSERT( is_asset_type( token, WEBANPICK_SYMBOL ) == false || token <= a->reward_webanpick_balance,
                       "Cannot claim that much WEBANPICK. Claim: ${c} Actual: ${a}", ("c", token)("a", a->reward_webanpick_balance) );
...
void delegate_vesting_shares_evaluator::do_apply( const delegate_vesting_shares_operation& op )
{
   ...
   auto min_delegation = asset( wso.median_props.account_creation_fee.amount * 10, WEBANPICK_SYMBOL ) * gpo.get_vesting_share_price();
   ...
   else if( op.vesting_shares >= delegation->vesting_shares )
   {
      ...
      FC_ASSERT( delta >= min_update, "Webanpick Power increase is not enough of a difference. min_update: ${min}", ("min", min_update) );
   ...
   else /* delegation->vesting_shares > op.vesting_shares */
   {
      ...
      {
         FC_ASSERT( delta >= min_update, "Webanpick Power decrease is not enough of a difference. min_update: ${min}", ("min", min_update) );
      ...
      _db.create< vesting_delegation_expiration_object >( [&]( vesting_delegation_expiration_object& obj )
      {
         ...
         obj.expiration = std::max( _db.head_block_time() + WEBANPICK_CASHOUT_WINDOW_SECONDS, delegation->min_delegation_time );
      });
