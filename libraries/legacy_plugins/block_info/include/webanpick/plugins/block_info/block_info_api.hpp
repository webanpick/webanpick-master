...
{
   public:
      block_info_api( const webanpick::app::api_context& ctx );
...
FC_REFLECT( webanpick::plugin::block_info::get_block_info_args,
   (start_block_num)
   (count)
   )

FC_API( webanpick::plugin::block_info::block_info_api,
   (get_block_info)
