...
{
   public:
      raw_block_api( const webanpick::app::api_context& ctx );
...
FC_REFLECT( webanpick::plugin::raw_block::get_raw_block_args,
   (block_num)
   )

FC_REFLECT( webanpick::plugin::raw_block::get_raw_block_result,
   (block_id)
   (previous)
   (timestamp)
   (raw_block)
...
FC_API( webanpick::plugin::raw_block::raw_block_api,
   (get_raw_block)
   (push_raw_block)
   )
