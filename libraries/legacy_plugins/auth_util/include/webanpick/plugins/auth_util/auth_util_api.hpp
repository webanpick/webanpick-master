...
{
   public:
      auth_util_api( const webanpick::app::api_context& ctx );
...
FC_REFLECT( webanpick::plugin::auth_util::check_authority_signature_params,
   (account_name)
   (level)
   (dig)
   (sigs)
   )
FC_REFLECT( webanpick::plugin::auth_util::check_authority_signature_result,
   (keys)
   )

FC_API( webanpick::plugin::auth_util::auth_util_api,
