...
{
   webanpick::protocol::transaction     tx;
   std::string                      wif;
};
...
struct tx_signing_result
{
   ...
   webanpick::protocol::public_key_type key;
   webanpick::protocol::signature_type  sig;
};
...
int main(int argc, char** argv, char** envp)
{
   ...
   while( std::cin )
   {
      ...
      try
      {
         ...
         auto priv_key = webanpick::utilities::wif_to_key( sreq.wif );

         if(priv_key)
         {
            sres.sig = priv_key->sign_compact( sres.sig_digest );
            sres.key = webanpick::protocol::public_key_type( priv_key->get_public_key() );
