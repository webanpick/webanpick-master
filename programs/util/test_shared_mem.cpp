...
struct book
{
     ...
     auth( bip::allocator<webanpick::chain::shared_authority, bip::managed_mapped_file::segment_manager>( al.get_segment_manager() )),
     ...
     int32_t                          pages;
     int32_t                          prize;
     webanpick::chain::shared_authority auth;
...
int main(int argc, char** argv, char** envp)
{
   ...
   */
   ...
   //b.pages = pbc->size();
   //b.auth = webanpick::chain::authority( 1, "dan", pbc->size() );
   pbc->emplace( [&]( book& b ) {
                 b.name = "emplace name";
                 b.pages = pbc->size();
