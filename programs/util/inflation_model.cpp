...
using webanpick::protocol::calc_percent_reward_per_block;
using webanpick::protocol::calc_percent_reward_per_round;
using webanpick::protocol::calc_percent_reward_per_hour;
...
rvec shows total number of WEBANPICK satoshis created since genesis for:
...
- Missed blocks not modeled (lowers WEBANPICK supply, small)
- Miner queue length very approximately modeled (assumed to go to 100 during the first blocks and then stay there) (may lower or raise WEBANPICK supply, very small)
- Creation / destruction of WEBANPICK used to back SBD not modeled (moves WEBANPICK supply in direction opposite to changes in dollar value of 1 WEBANPICK, large)
- Interest paid to SBD not modeled (raises WEBANPICK supply, medium)
- Lost / forgotten private keys / wallets and deliberate burning of WEBANPICK not modeled (lowers WEBANPICK supply, unknown but likely small)
...
#define WEBANPICK_FIRST_CASHOUT_TIME              (fc::time_point_sec(1467590400))  /// July 4th
*/
   ...
   {
      uint32_t vesting_factor = (block_num < WEBANPICK_START_VESTING_BLOCK) ? 0 : 9;

      share_type curate_reward   = calc_percent_reward_per_block< WEBANPICK_CURATE_APR_PERCENT >( current_supply );
      reward_delta[ CURATE_OFF ] = std::max( curate_reward, WEBANPICK_MIN_CURATE_REWARD.amount );
      reward_delta[ VCURATE_OFF ] = reward_delta[ CURATE_OFF ] * vesting_factor;

      share_type content_reward  = calc_percent_reward_per_block< WEBANPICK_CONTENT_APR_PERCENT >( current_supply );
      reward_delta[ CONTENT_OFF ] = std::max( content_reward, WEBANPICK_MIN_CONTENT_REWARD.amount );
      reward_delta[ VCONTENT_OFF ] = reward_delta[ CONTENT_OFF ] * vesting_factor;

      share_type producer_reward = calc_percent_reward_per_block< WEBANPICK_PRODUCER_APR_PERCENT >( current_supply );
      reward_delta[ PRODUCER_OFF ] = std::max( producer_reward, WEBANPICK_MIN_PRODUCER_REWARD.amount );
      ...
      if( (block_num % WEBANPICK_MAX_WITNESSES) == 0 )
         ++pow_deficit;

      if( pow_deficit > 0 )
      {
         pow_reward = calc_percent_reward_per_round< WEBANPICK_POW_APR_PERCENT >( current_supply );
         pow_reward = std::max( pow_reward, WEBANPICK_MIN_POW_REWARD.amount );
         if( block_num < WEBANPICK_START_MINER_VOTING_BLOCK )
            pow_reward *= WEBANPICK_MAX_WITNESSES;
         --pow_deficit;
      }
      ...
      if( (block_num > liquidity_begin_block) && ((block_num % WEBANPICK_LIQUIDITY_REWARD_BLOCKS) == 0) )
      {
         liquidity_reward = calc_percent_reward_per_hour< WEBANPICK_LIQUIDITY_APR_PERCENT >( current_supply );
         liquidity_reward = std::max( liquidity_reward, WEBANPICK_MIN_LIQUIDITY_REWARD.amount );
      }
   ...
   for( uint32_t b=1; b<10*WEBANPICK_BLOCKS_PER_YEAR; b++ )
   {
