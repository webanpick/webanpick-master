...
target_link_libraries( {plugin_provider}_{plugin_name} webanpickit_app webanpickit_chain webanpickit_protocol )
...
{{
   public:
      {plugin_name}_api( const webanpick::app::api_context& ctx );
...
class {plugin_name}_plugin : public webanpick::app::plugin
{{
   public:
      {plugin_name}_plugin( webanpick::app::application* app );
...
"{plugin_name}_api.cpp" :
"""
#include <webanpick/app/api_context.hpp>
#include <webanpick/app/application.hpp>
...
{{
   public:
      {plugin_name}_api_impl( webanpick::app::application& _app );

      std::shared_ptr< {plugin_provider}::plugin::{plugin_name}::{plugin_name}_plugin > get_plugin();

      webanpick::app::application& app;
}};

{plugin_name}_api_impl::{plugin_name}_api_impl( webanpick::app::application& _app ) : app( _app )
{{}}
...
{plugin_name}_api::{plugin_name}_api( const webanpick::app::api_context& ctx )
{{
...
{{
   public:
      {plugin_name}_plugin_impl( webanpick::app::application& app );
...
{plugin_name}_plugin_impl::{plugin_name}_plugin_impl( webanpick::app::application& app )
  : _app(app) {{}}
...
{plugin_name}_plugin::{plugin_name}_plugin( webanpick::app::application* app )
   : plugin(app)
{{
...
WEBANPICK_DEFINE_PLUGIN( {plugin_name}, {plugin_provider}::plugin::{plugin_name}::{plugin_name}_plugin )
""",
}
...
def main(argv):
    ...
    parser.add_argument("provider", help="Name of plugin provider (webanpick for plugins developed by webanpickit)")
