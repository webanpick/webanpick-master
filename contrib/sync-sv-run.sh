...
if [[ ! $? -eq 0 ]]; then
  echo NOTIFYALERT! webanpickdsync has quit unexpectedly, checking for coredump and then starting a new instance..
  sleep 30
  ...
  if [[ -e /tmp/core.$SAVED_PID ]]; then
    ...
    echo NOTIFYALERT! webanpickdsync stacktrace from coredump:
...
if [[ ! -z "$BLOCKCHAIN_TIME" ]]; then
  ...
  if [[ ${BLOCK_AGE} -le 10 ]]; then
    WEBANPICKD_PID=`pgrep -f p2p-endpoint`
    kill -SIGINT $WEBANPICKD_PID
    echo webanpickdsync: waiting for webanpickd to exit cleanly
    while [ -e /proc/$WEBANPICKD_PID ]; do sleep 0.1; done
    echo webanpickdsync: starting a new blockchainstate upload operation
    cd ${COMPRESSPATH:-$HOME}
    echo webanpickdsync: compressing blockchainstate...
    if [[ "$USE_RAMDISK" ]]; then
    ...
    if [[ ! $? -eq 0 ]]; then
      echo NOTIFYALERT! webanpickdsync was unable to compress shared memory file, check the logs.
      exit 1
    fi
    ...
    echo webanpickdsync: uploading $FILE_NAME to $S3_BUCKET
    aws s3 cp blockchain.tar.bz2 s3://$S3_BUCKET/$FILE_NAME
    if [[ ! $? -eq 0 ]]; then
    	echo NOTIFYALERT! webanpickdsync was unable to upload $FILE_NAME to s3://$S3_BUCKET
    	exit 1
    fi
    echo webanpickdsync: replacing current version of blockchain state with $FILE_NAME
    if [[ "$IS_BROADCAST_NODE" ]]; then
    ...
    if [[ ! $? -eq 0 ]]; then
    	echo NOTIFYALERT! webanpickdsync was unable to overwrite the current blockchainstate with $FILE_NAME
    	exit 1
    fi
    ...
    echo webanpickdsync: stopping the container after a sync operation
    if [[ -e /tmp/isnewsync ]]; then
      echo notifywebanpickdsync: webanpickdsync: successfully generated and uploaded new blockchain-$VERSION-latest.tar.bz2 to s3://$S3_BUCKET
    fi
    RUN_SV_PID=`pgrep -f /etc/service/webanpickd`
    kill -9 $RUN_SV_PID
  fi
